 ...
 return newNotficationUid;
	...
	public String getRootExtensionTxt(String formCode, int seqNum,  String caseNumber, String caseLocalId) throws  javax.ejb.EJBException,java.rmi.RemoteException, NEDSSSystemException{
		...
		if (propertyUtil.getDatabaseServerType() != null &&
			...
			assAuthQuery = WumSqlQuery.GET_ASSIGNING_AUTH_STATE_CASE_ORACLE;
		else
		...
		}else
		{
			if (propertyUtil.getDatabaseServerType() != null &&
			          propertyUtil.
			          getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
				assAuthQuery = WumSqlQuery.CREATE_ASSIGNING_AUTH_STATE_CASE_ORACLE;
			else
			...
			if(map!=null && map.values().size()>0){ 
				...
				while(it.hasNext()){
					...
					String value= map.get(dataLocation).toString();
						if(it.hasNext()){
							if(dbType.equalsIgnoreCase(NEDSSConstants.ORACLE_ID) && dataLocation.indexOf("time")>0 ){
								sqlQueryBuff.append(dataLocation).append("=to_date(\'"+value+"\', 'mm/dd/yyyy') and ");
							}else
								sqlQueryBuff.append(dataLocation).append("=\'"+value+"\' and ");
						}else{
							if(dbType.equalsIgnoreCase(NEDSSConstants.ORACLE_ID) && dataLocation.indexOf("time")>0 ){
								sqlQueryBuff.append(dataLocation).append("=to_date(\'"+value+"\', 'mm/dd/yyyy')  ");
							}else
			 ...
			 logger.fatal("Exception occured in PamProxyEJB.getAggregateSummary, dbType: " + dbType + ", nbsSecurityObj.getFullName: " + nbsSecurityObj.getFullName() + ", " + e.getMessage(), e);
	         e.printStackTrace();
	         throw new EJBException(e.getMessage(),e);
		}
	...
	javax.ejb.EJBException, javax.ejb.CreateException, NEDSSSystemException, NEDSSConcurrentDataException{
		try {
			...
			if (cTContactProxyVO.isItNew()) {
				...
				cTContactProxyVO.getcTContactVO().setItDelete(false);
				cTContactProxyVO.getcTContactVO().setItDirty(true);
				cTContactProxyVO.getcTContactVO().getcTContactDT().setItDirty(true);
				cTContactProxyVO.getcTContactVO().getcTContactDT().setItDelete(false);
				cTContactProxyVO.getContactPersonVO().getThePersonDT().setItDirty(true);
				cTContactProxyVO.getContactPersonVO().getThePersonDT().setItDelete(false);			
				cTContactProxyVO.getContactPersonVO().setItDirty(true);
				cTContactProxyVO.getContactPersonVO().setItDelete(false);
