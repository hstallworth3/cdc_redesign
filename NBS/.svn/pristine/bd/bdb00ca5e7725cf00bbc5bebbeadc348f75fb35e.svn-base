...
public class MainSessionHolder
{
    ...
    public MainSessionCommand getMainSessionCommand(HttpSession session) throws Exception
    {
        ...
        else
        {
            ...
            msCommand = getMainSessionCommand((String)session.getAttribute(NEDSSConstants.USERNAME), (String)session.getAttribute(NEDSSConstants.PASSWORD), session.getId(), strRemoteAddress, strRemoteHost);
