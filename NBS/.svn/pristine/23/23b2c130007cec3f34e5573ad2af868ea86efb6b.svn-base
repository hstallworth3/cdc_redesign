...
<%
	...
	<html:form>
		...
		<%
		           ArrayList data = (ArrayList) request.getAttribute("CR3");
					if(data!=null && data.size()>0){
				    Iterator dataIterator = data.iterator();
					while (dataIterator.hasNext()) 
					{
					 CR3VO cr3VO = (CR3VO)dataIterator.next();
					 ReportPatientDT rpDT = cr3VO.getPatientDT();
					 ...
					 ArrayList contacts = (ArrayList)cr3VO.getNamed();
			...
			<tr>
				<td>
					<table width="98%">
		             <tr>
		             <td align="center">
						  	<display:table name="contacts" class="dtTable" style="margin-top:0em;"  id="parent">
						  	    <display:column property="interviewDate" title="Ix Date" format="{0,date,MM/dd/yyyy}" style="width:12%;"/>
								<display:column property="contactName" title="Marginal Name" style="width:12%;"/>
								<display:column property="contactReferraCd" title="Basis" style="width:12%;"/>
								<display:column property="gender" title="Gender" style="width:14%;"/>
								<display:column property="contactFirstSexExpoDate" title="1st Expose" format="{0,date,MM/dd/yyyy}" style="width:10%;"/>
