...
public class TreatmentVO
  ...
  public Collection<Object> getTheTreatmentAdministeredDTCollection() {
    return theTreatmentAdministeredDTCollection;
  }
  ...
  public void setTheTreatmentAdministeredDTCollection(Collection<Object> theTreatmentAdministeredDT) {
    this.theTreatmentAdministeredDTCollection  = theTreatmentAdministeredDT;
    setItDirty(true);
  }
  ...
  public TreatmentAdministeredDT getTreatmentAdministeredDT_s(int index) {
    /** @todo move to const */
    if (this.theTreatmentAdministeredDTCollection  == null)
      this.theTreatmentAdministeredDTCollection  = new ArrayList<Object> ();

    int currentSize = this.theTreatmentAdministeredDTCollection.size();

    // check if we have a this many DTs
    if (index < currentSize) {
      try {
        Object[] tempArray = this.theTreatmentAdministeredDTCollection.toArray();
    ...
    for (int i = currentSize; i < index + 1; i++) {
      ...
      tempDT.setItNew(true); /** @todo move to const */
      this.theTreatmentAdministeredDTCollection.add(tempDT);
    }
