...
public class PostalLocatorDAOImpl
  ...
  public static final String JOIN_POSTAL_LOCATORS_FOR_GEOCODING_ORACLE = " FROM Postal_locator, Entity, Entity_locator_participation " +
      "WHERE Entity.entity_uid = Entity_locator_participation.entity_uid AND Entity_locator_participation.locator_uid = Postal_locator.postal_locator_uid " +
  ...
  public Object loadObject(long postalLocatorUID) throws NEDSSDAOSysException,
	...
	public Collection<Object>  findPostalLocatorsNeedingGeoCoding() throws NEDSSSystemException {
		...
		try {
			...
			boolean useOracle = propertyUtil.getDatabaseServerType().equals(NEDSSConstants.ORACLE_ID);
			int blockSize = GeoCodingHelper.getInstance().getBlockSize();

			// Build select string //
			String selectString = "SELECT" + (useOracle ? "" : " TOP " + blockSize) +
					SELECT_POSTAL_LOCATORS_FOR_GEOCODING_BODY +
					(useOracle ? JOIN_POSTAL_LOCATORS_FOR_GEOCODING_ORACLE + GEOCODING_IND_NULL_CLAUSE +
							" AND rownum <= " + blockSize : 
  ...
  private void updatePostalLocator(Object obj) throws NEDSSDAOSysException,
	...
	private void clearGeocodeMatchIndicators(boolean clearErrors) throws NEDSSDAOSysException, NEDSSSystemException {
		...
		String selectString = UPDATE_POSTAL_LOCATOR_CLEAR_GEOCODING_INDS_BODY +
				(useOracle ? JOIN_POSTAL_LOCATORS_FOR_GEOCODING_ORACLE : JOIN_POSTAL_LOCATORS_FOR_GEOCODING_SQLSERVER) +
