...
public class LineList_IndividualCases_ProgramArea_JurisdictionSecurity {
	...
	@Test
	public void Populate_LineList_IndiCase_PA_JS() throws Exception {
		comUtils.setUpDriver(driver);
		log = new Log("LineList_IndiCase_PA_JS_Reports", Log_File_Path);
		df = new DataFile(Data_File_Path, "LineList_IndiCase_PA_JS");
		...
		for(int col=1; col <=colCount; col++){	
			try {
				...
				//Click on TB Report Section under Public Reports (in some environment Template8 is available and in some environments Template? is available)
				int env_index =0;
				for (int i = 1;i<=10;i++){
				...
				//Click on Run - Get the index of the Run button with Text -  TB Case Verification Report
				int index = 0;
				...
				while(itr.hasNext()) {
					index = index+1;
					if (tableRows.get(index).getText().contains("Line List of Individual Cases with Program Area and Jurisdiction Security"))
						break;	
				}
