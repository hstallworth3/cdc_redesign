...
public class DefinedFieldSQLQuery {
 ...
 //select queries
  ...
  public static final String SELECT_ALL_LDF_METADATA_NBS_QUESTION_ORACLE = 
	  "SELECT DISTINCT m.nbs_ui_metadata_uid \"ldfUid\", "  
	  + "m.record_status_cd AS \"activeInd\", "
	  + "m.admin_comment AS \"adminComment\", "
	  + "m.add_time AS \"addTime\", " 
	  ...
	  + "NULL AS \"ldfOid\", "
	  + "m.version_ctrl_nbr AS \"versionCtrlNbr\" " 
	  + "FROM NBS_UI_Metadata m INNER JOIN "
	  + "NBS_Question q ON m.nbs_question_uid = q.nbs_question_uid "
	  + "LEFT OUTER JOIN "
	  + NEDSSConstants.SYSTEM_REFERENCE_TABLE
	  + ".Codeset cs ON q.code_set_group_id = cs.code_set_group_id "
	  + "INNER JOIN "
	  + NEDSSConstants.SYSTEM_REFERENCE_TABLE
	  + ".Condition_code cc ON m.investigation_form_cd = cc.investigation_form_cd "
	  + "WHERE (m.ldf_status_cd IS NOT NULL)";	  
...
*/
  ...
  public static final String SELECT_LDF_PAGE_SET_ORACLE =
      " "
