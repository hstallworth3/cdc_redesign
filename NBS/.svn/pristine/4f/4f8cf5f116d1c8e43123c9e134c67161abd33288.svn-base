...
create table updated_hep_PATIENT as select  
 			P.PATIENT_UID 
from nbs_rdb.D_PATIENT p, NBS_RDB.HEPATITIS_DATAMART
where P.PATIENT_UID=HEPATITIS_DATAMART.PATIENT_UID
AND PATIENT_LAST_CHANGE_TIME>select date_last_ran from HEPATITIS_DATAMART_LAST;
QUIT;
PROC SQL;
...
SELECT CONDITION_CD, CONDITION_DESC, CONDITION_KEY 
...
EPI_OutbreakUnidentified	EPI_OutbreakWaterborne	EPI_RecDrugUse	IPO_SEXUAL_PREF EPI_OutbreakAssoc;
RUN;
DATA D_INV_EPIDEMIOLOGY;
MERGE  F_INV_EPIDEMIOLOGY(IN=in1) nbs_rdb.D_INV_EPIDEMIOLOGY(IN=in2);
BY D_INV_EPIDEMIOLOGY_KEY;
...
INVESTIGATION_key D_INV_CLINICAL_KEY;
RUN;
...
keep MDH_DiabetesDxDate MTH_MotherBornOutsideUS MTH_MotherEthnicity MTH_MotherHBsAgPosPrior MTH_MotherPositiveAfter MTH_MotherRace 
...
keep EXT_DIABETES_DX_DT EXT_MTH_BORN_OUTSIDE_US  EXT_MTH_ETHNICITY EXT_MTH_HBS_AG_PRIOR_POS EXT_MTH_POS_AFTER EXT_MTH_RACE EXT_MTH_BIRTH_COUNTRY
MTH_POS_TEST_DT INVESTIGATION_key D_INV_MOTHER_KEY;
RUN; 
DATA D_INV_MOTHER(sortedby=INVESTIGATION_KEY RENAME=(EXT_DIABETES_DX_DT=DIABETES_DX_DT  EXT_MTH_BORN_OUTSIDE_US=MTH_BORN_OUTSIDE_US  
...
EXT_GLOB_LAST_RECVD_YR= INPUT(VAC_LastIGDose, comma20.);
...
IF In1=1 then output VAC_REPEAT;
...
keep  VAC_VaccinationDate VAC_VaccineDoseNum PAGE_CASE_UID answer_group_seq_nbr D_INVESTIGATION_REPEAT_KEY INVESTIGATION_KEY;
run;

/*

proc sql;
CREATE TABLE VAC_REPEAT AS
SELECT distinct VAC_VaccinationDate, VAC_VaccineDoseNum, PAGE_CASE_UID, answer_group_seq_nbr, PAGE_CASE.D_INVESTIGATION_REPEAT_KEY,PAGE_CASE.INVESTIGATION_KEY  
FROM NBS_RDB.D_INVESTIGATION_REPEAT INNER JOIN 
NBS_RDB.F_PAGE_CASE PAGE_CASE ON D_INVESTIGATION_REPEAT.D_INVESTIGATION_REPEAT_KEY=PAGE_CASE.D_INVESTIGATION_REPEAT_KEY
INNER JOIN F_PAGE_CASE ON F_PAGE_CASE.D_INVESTIGATION_REPEAT_KEY=PAGE_CASE.D_INVESTIGATION_REPEAT_KEY

where VAC_VaccineDoseNum is not null
OR VAC_VaccinationDate is not null
/*
WHERE D_INVESTIGATION_REPEAT_KEY 
IN (SELECT D_INVESTIGATION_REPEAT_KEY FROM NBS_RDB.F_PAGE_CASE
)order by PAGE_CASE_UID, ANSWER_GROUP_SEQ_NBR;
...
VACC_DOSE_NBR_1= INPUT(_1, comma20.);
VACC_DOSE_NBR_2= INPUT(_2, comma20.);
VACC_DOSE_NBR_3= INPUT(_3, comma20.);
VACC_DOSE_NBR_4= INPUT(_4, comma20.);
