...
<html>
    <head>
		...
		function makeMSelects() {
						...
						$j("#recipient").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
						$j("#patient").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
						$j("#comment").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
			}
		...
		<html:form action="/LoadReviewNotifications1.do" >
           ...
           <!-- Page Code Starts here -->
			...
			<a class="removefilerLink" href="javascript:clearFilter()"><font class="hyperLink"> | Remove All Filters/Sorts&nbsp;</font></a>
            	...
            	&nbsp;&nbsp;&nbsp;<img  src="page_white_get.gif"  class="cursorHand" title="Approve"  align="center"  cellspacing="2" cellpadding="3" border="55" onclick="javascript:approveNotifPopUp(
            ...
            <input type='hidden' id='SearchText2' value="<%= request.getAttribute("COMMENT")!=null? request.getAttribute("COMMENT"):""%>"/>
		     </html:form>
		     </div>
