...
function viewClicked(eleid,subSecNm) {		               		    
  ...
  <% String printMode = (request.getAttribute("mode") == null) ? "" : ((String)request.getAttribute("mode"));
    ...
    if (printMode.equals("print")) { %>
        <body class="popup" onload="startCountdown();checkPageDelete();cleanupPatientRacesViewDisplay();disablePrintLinks();populateBatchRecords();stdSpecialViewProcessing();" onunload="return closePrinterFriendlyWindow();selectTabOnSubmit();">
    <% } else { %> 
        <body class="popup" onload="startCountdown();checkPageDelete();cleanupPatientRacesViewDisplay();selectTabOnSubmit();populateBatchRecords();stdSpecialViewProcessing();" onunload="handlePageUnload(false, event); return false;">
    <% } %>
