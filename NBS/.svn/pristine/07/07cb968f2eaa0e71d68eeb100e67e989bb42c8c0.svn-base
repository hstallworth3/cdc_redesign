...
public class InvestigationUtil {
	...
	public static PersonVO getPersonVO(HttpServletRequest request, HttpServletResponse response) {
			...
			if (morbObsSummaryColl != null) {
				...
				while (ite.hasNext()) {
					...
					if (mrsVO.isMorbFromDoc()) {
						...
						mrsVO.setActionLink(actionLink + "<br><img src=\"ind_electronic.gif\" title=\"Electronic Lab\">");
					}
					else
				...
				while (ite.hasNext()) {
					...
					if(treatSumVO.getNbsDocumentUid()!=null){
						...
						treatSumVO.setActionLink(startDate
								+ "<br>"
								+ "<img src=\"ind_electronic.gif\" title=\"Electronic Treatment\">");
					}
					parameterMap = new HashMap<Object,Object>();
				}
			}
	...
	public void populateAssociatedDocument(ManageEventsForm form,Collection<Object>  documentColl, String sPublicHealthCaseUID, HttpServletRequest request){
		...
		if(documentColl != null) {
			...
			while (ite.hasNext()) {
				...
				parameterMap.put("method", "viewManageSubmit");
				doc.setActionLink(getHiperlink(dateLink(doc.getAddTime()), sCurrentTask, parameterMap));
				parameterMap = new HashMap<Object,Object>();
			}
		}
	...
	public static void storeContactContextInformation(HttpSession session, PublicHealthCaseVO publicHealthCaseVO, Collection<Object> personVoCollection){
		  ...
		  NBSContext.store(session, NBSConstantUtil.DSInvestigationPath, sCurrTask);
     	  ...
     	  NBSContext.store(session, NBSConstantUtil.DSConditionCdDescTxt, publicHealthCaseVO.getThePublicHealthCaseDT().getCdDescTxt());
	 ...
	 * @return
	 */
	private static OrganizationVO getOrganization( Long organizationEntityUid,NBSSecurityObj nbsSecurityObj, HttpServletRequest request){
