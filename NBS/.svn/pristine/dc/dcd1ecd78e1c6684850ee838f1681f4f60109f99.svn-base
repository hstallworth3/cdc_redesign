...
public class LineList_HepatitisInvestigations {
	...
	public void Populate_SummaryReport() throws Exception {
		...
		log = new Log("LineList_Hepatitis_PA_JS", Log_File_Path);
		df = new DataFile(Data_File_Path, "LineList_HepatitisInvestigation");		
		...
		for(int col=1; col <=colCount; col++){	
			try {
				...
				while(itr.hasNext()) {
					index = index+1;
					if (tableRows.get(index).getText().contains("Line List of Hepatitis Investigations"))
						break;	
				}
				...
				if (!(comUtils.isElementDisplayed(By.id("error1")))){
					...
					if (comUtils.isElementDisplayed(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td/p"))){
						...
						if (driver.findElement(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td")).getText().contains("Custom Report For Table: "))//Needs to be changed. Have not yet found any Report to display.
