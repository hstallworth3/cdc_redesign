...
d.provider_last_name, a.CC_CLOSED_DT, a.CA_PATIENT_INTV_STATUS
...
proc sql noprint;
select min(CA_INIT_INTVWR_ASSGN_DT), max(CA_INIT_INTVWR_ASSGN_DT), min(CC_CLOSED_DT), max(CC_CLOSED_DT)
into :MIN_CA_INIT_INTVWR_ASSGN_DT , :MAX_CA_INIT_INTVWR_ASSGN_DT,:MIN_CC_CLOSED_DT, :MAX_CC_CLOSED_DT
from STD_HIV_DATAMART1 where INVESTIGATOR_INTERVIEW_KEY ~=1 
; 

proc sql;
create table PA05_IXS_INIT as 
select  distinct a.IX_TYPE, PROVIDER_LAST_NAME, IX_INTERVIEWER_KEY as INVESTIGATOR_INTERVIEW_KEY, 
a.D_INTERVIEW_KEY, d.D_CONTACT_RECORD_KEY, d.CONTACT_INVESTIGATION_KEY, e.CTT_REFERRAL_BASIS,
f.CA_INIT_INTVWR_ASSGN_DT, f.CC_CLOSED_DT
...
where 
b.IX_INTERVIEWER_KEY in (select INVESTIGATOR_INTERVIEW_KEY from STD_HIV_DATAMART1 where INVESTIGATOR_INTERVIEW_KEY ~=1)
...
and datepart(f.CA_INIT_INTVWR_ASSGN_DT) GE datepart(&MIN_CA_INIT_INTVWR_ASSGN_DT)
and datepart(f.CA_INIT_INTVWR_ASSGN_DT) LE datepart(&MAX_CA_INIT_INTVWR_ASSGN_DT)
...
select distinct  a.INV_LOCAL_ID, c.IX_TYPE, e.INV_CASE_STATUS, e.record_status_cd, a.CC_CLOSED_DT,
 a.FL_FUP_INIT_ASSGN_DT, e.CURR_PROCESS_STATE , a.CA_PATIENT_INTV_STATUS,
INVESTIGATOR_INTERVIEW_KEY, INVESTIGATOR_INTERVIEW_QC, datepart(c.IX_DATE)-datepart(a.CA_INIT_INTVWR_ASSGN_DT)  as Days,
...
inner join nbs_rdb.investigation e  on e.investigation_key =a.investigation_KEY
where c.IX_TYPE in ('Initial/Original', 'Re-Interview' ) and datepart(c.IX_DATE) GE datepart(a.CA_INIT_INTVWR_ASSGN_DT)
...
and d_contact_record.RECORD_STATUS_CD~='LOG_DEL'
    where c.IX_TYPE in ('Initial/Original', 'Re-Interview' ) and
d_contact_record.CTT_REFERRAL_BASIS in ('P1 - Partner, Sex', 'P2 - Partner, Needle-Sharing','P3 - Partner, Both') and a.CA_PATIENT_INTV_STATUS in ('I - Interviewed');
  quit;
...
proc sql;
create table r as   
select distinct PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct count_q) as var_r
from pp 
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME  ;
quit;
...
style(column)={font_size=10pt font_face="calibri"};
by PROVIDER_LAST_NAME ;
columns  PROVIDER_LAST_NAME colname colval colpct blank colname2 colval2 colpct2;
