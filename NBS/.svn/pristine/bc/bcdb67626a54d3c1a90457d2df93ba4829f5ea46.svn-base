...
public class SR9_BarGraph_SelectedDisease_ByMonth {

	public WebDriver driver;	
	public Log log;
	DataFile df;
	public String Data_File_Path,Log_File_Path;
	public static String Data_File_Path1;
	...
	@Test
	public void Populate_SR9() throws Exception {
		comUtils.setUpDriver(driver);
		log = new Log("SR9_Reports", Log_File_Path);
		df = new DataFile(Data_File_Path, "SR9");
		...
		for(int col=1; col <=colCount; col++){			
			try {
				...
				//Click on Run - Get the index of the Run button with Text - SR9: Bar Graph of Selected Disease by Month
				int index = 0;
				...
				while(itr.hasNext()) {
					index = index+1;
					if (tableRows.get(index).getText().contains("SR9: Bar Graph of Selected Disease by Month"))
						break;	
				}
				...
				if (!(comUtils.isElementDisplayed(By.id("error1")))){
					...
					if ((windHandle).equals(winHandleBefore)){ 
						...
						//Switch to the new windowHandle 
