...
//    private static final Logger logger = Logger.getLogger(DataSourceDT.class);
    ...
    public void validate() throws IllegalStateException
    {
        ...
        String data_source_name_v = this.getDataSourceName();
        if(data_source_name_v.length() < 9)
        {
            throw new IllegalStateException("Please enter a name that starts with \"nbs_ods.\" or \"nbs_rdb.\"");
        }
        if
        (
            (!data_source_name_v.toLowerCase().startsWith("nbs_ods.")) &&
            (!data_source_name_v.toLowerCase().startsWith("nbs_rdb."))
        )
        {
            throw new IllegalStateException("The name must start with 'nbs_ods' or 'nbs_rdb'");
        }
