...
//
	...
	protected ArrayList<Object> computeSearchCriteriaTypes(PatientSearchVO find) {
		...
		try{
			...
			if(find.getConditionSelected()!=null && !find.getConditionSelected().isEmpty() ||
					find.getProgramArea()!=null && !find.getProgramArea().isEmpty() ||
					find.getJurisdictionSelected()!=null && !find.getJurisdictionSelected().isEmpty() ||
					find.getPregnantSelected()!=null && !find.getPregnantSelected().isEmpty()||
					find.getInvestigationStatusSelected()!=null && !find.getInvestigationStatusSelected().isEmpty()||
					find.getOutbreakNameSelected()!=null && !find.getOutbreakNameSelected().isEmpty()||
					find.getCaseStatusCodedValuesSelected()!=null && !find.getCaseStatusCodedValuesSelected().isEmpty()||
					find.getCurrentProcessCodedValuesSelected()!=null && !find.getCurrentProcessCodedValuesSelected().isEmpty()||
					find.getInvestigatorSelected()!=null && !find.getInvestigatorSelected().isEmpty() ||
							((find.getActType() != null)
									&& find.getActType().equalsIgnoreCase("P10001"))||
							find.getNotificationCodedValuesSelected()!=null && !find.getNotificationCodedValuesSelected().equals("") ||
							(find.getReportType()!=null && find.getReportType().equalsIgnoreCase("I") && find.getEventStatusInitialSelected()!=null && find.getEventStatusInitialSelected().equalsIgnoreCase("true")) ||
							(find.getReportType()!=null && find.getReportType().equalsIgnoreCase("I") && find.getEventStatusUpdateSelected()!=null && find.getEventStatusUpdateSelected().equalsIgnoreCase("true")))
							{
