...
nbs_rdb.MORBIDITY_REPORT where morb_rpt_uid in (select observation_uid from rdbdata.updated_observation_List)
...
nbs_rdb.MORB_RPT_USER_COMMENT where MORB_RPT_UID in (select observation_uid from rdbdata.updated_observation_List);
...
from 	rdbdata.s_updated_lab as updated_lab  inner join nbs_ods.observation obs
...
proc sql;
	...
	select 	root.morb_Rpt_Key,
			root.morb_rpt_uid,
			obs.activity_to_time	'USER_COMMENTS_DT' as user_comments_dt,
			mrb180.add_user_id		'USER_COMMENTS_BY' as user_comments_by,
			TRANSLATE(ovt.value_txt,' ' ,'0D0A'x) 'EXTERNAL_MORB_RPT_COMMENTS' as external_morb_rpt_comments,
			root.record_status_cd 
	from 	Morbidity_Report			as root,
			nbs_ods.act_relationship 	as ar1,
			nbs_ods.observation			as obs,
			nbs_ods.act_relationship 	as ar2,
			nbs_ods.observation			as mrb180,
