...
public class SR11_Cases_SelectedDisease_YearOverTime {
	...
	@Test
	public void Populate_SR11() throws Exception {
		comUtils.setUpDriver(driver);
		log = new Log("SR11_Report", Log_File_Path);
		df = new DataFile(Data_File_Path, "SR11");
		...
		for(int col=1; col <=colCount; col++){	
			try {
				...
				//Click on Run - Get the index of the Run button with Text - SR11: Cases of Selected Diseases By Year Over Time
				int index = 0;
				...
				while(itr.hasNext()) {
					index = index+1;
					if (tableRows.get(index).getText().contains("SR11: Cases of Selected Diseases By Year Over Time"))
						break;	
				}
				...
				log.writeSubSection("There is an exeption generated. Report is NOT executed successfully","",false);
				log.takeSnapshot(driver);
			}
