...
import gov.cdc.nedss.nnd.intermediarymessage.NBSNNDIntermediaryMessageDocument.NBSNNDIntermediaryMessage.MessageElement.DataElement.DtDataType;
...
import gov.cdc.nedss.nnd.intermediarymessage.NBSNNDIntermediaryMessageDocument.NBSNNDIntermediaryMessage.MessageElement.DataElement.CxDataType;
import gov.cdc.nedss.nnd.intermediarymessage.NBSNNDIntermediaryMessageDocument.NBSNNDIntermediaryMessage.MessageElement.DataElement.XpnDataType;
import gov.cdc.nedss.nnd.intermediarymessage.NBSNNDIntermediaryMessageDocument.NBSNNDIntermediaryMessage.MessageElement.DataElement.XtnDataType;
...
public class MessageBuilderHelper {
	...
	public String createNBSNNDIntermediaryMessageXML(Collection<Object> caseNotificationDataDTCollection) throws NEDSSSystemException {
		...
		return nndIntMessageDoc.toString();
	}
	...
	private MessageElement populateIntermediaryMessageWithCaseData(CaseNotificationDataDT caseNotificationDataDT,
		...
		try {
			...
			// Otherwise we have an invalid type code, or a valid HL7 type we've not yet coded for
			} else {
	...
	public void retrieveNonNBSAnswerData(Collection<Object> caseNotificationDataDTCollection, TreeMap<Object, Object> entityUIDMap, Long publicHealthCaseUid, Long notificationUid) {
		...
		while (groupItr.hasNext()) {
			updatedCaseNotificationDataDTCollection  = processGroupDataRetrieval(entityUIDMap, publicHealthCaseUid, (HashMap<?, ?>)groupItr.next(), notificationUid);
	...
	private Collection<Object>  processGroupDataRetrieval(TreeMap<Object, Object> entityUIDMap, Long publicHealthCaseUid, HashMap<?,?> groupColumns, Long notificationUid) {
		...
		if (fromWhereClause != null) {
			...
			String queryString = "SELECT " + selectColumns + fromWhereClause;

			NotificationMessageDAOImpl notificationMessageDAOImpl = new NotificationMessageDAOImpl();
			updatedCaseNotificationDataDTCollection  = notificationMessageDAOImpl.getUpdatedNonAnswerCaseNotificationDataDTs(queryString, groupColumns, tableName);
		}

		return updatedCaseNotificationDataDTCollection;
	}

	private String buildDynamicNonAnswerFromWhereClause(CaseNotificationDataDT currentCaseNotificationDataDT, TreeMap<Object, Object> entityUIDMap, Long publicHealthCaseUid, Long notificationUid) {
