    ...
    public List<Object> getMessageLog(Long assignedToId, NBSSecurityObj nbssecurityObj)
    {
        ...
        try
        {
            ...
            if (propUtil.getDatabaseServerType().equals(NEDSSConstants.ORACLE_ID))
            	 query = WumSqlQuery.SELECT_MESSAGE_LOG_ORACLE  + ( (whereClause != null && whereClause.length() > 0) ? " and " + whereClause : "" );
            else
            	 query = WumSqlQuery.SELECT_MESSAGE_LOG  + ( (whereClause != null && whereClause.length() > 0) ? " and " + whereClause : "" );
