...
select distinct b.D_INTERVIEW_KEY, c.IX_TYPE, a.inv_local_id, a.INVESTIGATION_KEY, provider_last_name,f.CTT_REFERRAL_BASIS, 
...
inner  join nbs_rdb.D_provider 
  on D_provider.provider_key =a.INVESTIGATOR_INTERVIEW_KEY
  inner join nbs_rdb.F_CONTACT_RECORD_CASE e on 
  e.SUBJECT_INVESTIGATION_KEY = a.Investigation_key
  inner join nbs_rdb.d_contact_record f on 
...
or (f.CTT_REFERRAL_BASIS in ('P1 - Partner, Sex','P2 - Partner, Needle-Sharing','P3 - Partner, Both') and e.CONTACT_INVESTIGATION_KEY is null);
quit;
...
create table cluster as 
 select distinct a.INVESTIGATION_KEY, provider_last_name,
 f.FL_FUP_DISPOSITION,  f.inv_local_id, /* changes made - oct 6th*/
...
/*******Cases with No clusters - Y  ****/
proc sql;
create table cluster_No as 
 select distinct a.INVESTIGATION_KEY, provider_last_name,f.CTT_REFERRAL_BASIS, a.inv_local_id,
 INVESTIGATOR_INTERVIEW_KEY, INVESTIGATOR_INTERVIEW_QC
from STD_HIV_DATAMART1 a inner join 
 nbs_rdb.F_INTERVIEW_CASE b on a.INVESTIGATION_KEY = b.INVESTIGATION_KEY
inner join nbs_rdb.D_INTERVIEW c on c.D_INTERVIEW_KEY =b.D_INTERVIEW_KEY and c.RECORD_STATUS_CD~='LOG_DEL'
...
'S1 - Social Contact 1', 'S2 - Social Contact 2', 'S3 - Social Contact 3') 
or (f.CTT_REFERRAL_BASIS in ('A1 - Associate 1','A2 - Associate 2','A3 - Associate 3',
'S1 - Social Contact 1', 'S2 - Social Contact 2', 'S3 - Social Contact 3') and e.CONTACT_INVESTIGATION_KEY is null)
order by INVESTIGATOR_INTERVIEW_KEY;
quit;
...
create table cluster2 as 
 select distinct a.INVESTIGATION_KEY, provider_last_name,f.inv_local_id,
...
create table partner2 as 
 select distinct a.INVESTIGATION_KEY, provider_last_name,f.inv_local_id, a.INIT_FUP_INITIAL_FOLL_UP,
 f.FL_FUP_DISPOSITION,  a.FL_FUP_DISPO_DT, a.FL_FUP_INIT_ASSGN_DT, datepart(a.FL_FUP_DISPO_DT)-datepart(a.FL_FUP_INIT_ASSGN_DT) as days,
 a.INVESTIGATOR_INTERVIEW_KEY, a.INVESTIGATOR_INTERVIEW_QC
from STD_HIV_DATAMART1 a inner join 
 nbs_rdb.F_INTERVIEW_CASE b on a.INVESTIGATION_KEY = b.INVESTIGATION_KEY
...
'P3 - Partner, Both') AND f.FL_FUP_DISPOSITION in ('2 - Prev. Neg, New Pos' , '3 - Prev. Neg, Still Neg' , 
'4 - Prev. Neg, No Test' ,'5 - No Prev Test, New Pos' , '6 - No Prev Test, New Neg' , '7 - No Prev Test, No Test') 
and datepart(a.FL_FUP_DISPO_DT) ge datepart(a.FL_FUP_INIT_ASSGN_DT)
order by INVESTIGATOR_INTERVIEW_KEY ;
quit;
...
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY , count (distinct INV_LOCAL_ID) as Var_H
from pa1_new
...
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct INV_LOCAL_ID) as Var_I
...
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id) as var_k 
from pa1_new 
...
from pa1_new  
where  CA_PATIENT_INTV_STATUS in ('I - Interviewed') and  HIV_900_result in ('1 - Positive/Reactive')
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
quit;
...
create table pix as 
select  distinct 
d.PROVIDER_LAST_NAME,  A.INVESTIGATOR_INTERVIEW_KEY, count(distinct a.inv_local_id) as var_n 
from STD_HIV_DATAMART1 a
inner join nbs_rdb.investigation e  on e.investigation_key = a.investigation_KEY
inner join nbs_rdb.F_CONTACT_RECORD_CASE d on a.INVESTIGATION_KEY = d.SUBJECT_INVESTIGATION_KEY
inner join nbs_rdb.STD_HIV_DATAMART f on d.CONTACT_INVESTIGATION_KEY = f.Investigation_key
...
group by A.INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by A.INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
quit;
...
select  distinct 
d.PROVIDER_LAST_NAME,  A.INVESTIGATOR_INTERVIEW_KEY, count(distinct a.inv_local_id) as var_o 
from STD_HIV_DATAMART1 a
inner join nbs_rdb.investigation e  on e.investigation_key = a.investigation_KEY
inner join nbs_rdb.F_CONTACT_RECORD_CASE d on a.INVESTIGATION_KEY = d.SUBJECT_INVESTIGATION_KEY
inner join nbs_rdb.STD_HIV_DATAMART f on d.CONTACT_INVESTIGATION_KEY = f.Investigation_key
...
proc sql;
create table q as   
select distinct PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, sum(count_q) as var_q
from pp 
where CA_PATIENT_INTV_STATUS in ('I - Interviewed') 
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME  
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME  ;
...
proc sql;
create table R as   
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id)  as var_R
from PP 
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
quit;
...
proc sql;
create table S as   
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id)  as var_S 
from PP  
...
proc sql;
create table t as   
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id)  as var_t 
from PP  
...
proc sql;
create table v as   
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id)  as var_v 
from PART2  
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
quit;
...
proc sql;
create table w as   
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id)  as var_w
from cluster  
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
quit;
...
proc sql;
create table y as   
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id)  as var_y 
from cluster_no  
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
quit;
...
proc sql;
create table pm as 
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY,  count(distinct inv_local_id) as var_pm  
from partner2
where 
...
proc sql;
create table p2 as 
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id) as var_p2 
from partner2  
...
proc sql;
create table p3 as 
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id) as var_p3 
from partner2  
...
proc sql;
create table p4 as 
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id) as var_p4 
from partner2  
...
proc sql;
create table p5 as 
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id) as var_p5
from partner2  
...
proc sql;
create table p6 as 
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id) as var_p6
from partner2  
...
proc sql;
create table p7 as 
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id) as var_p7
from partner2  
...
proc sql;
create table pn as 
 select distinct  c.IX_TYPE, a.INVESTIGATION_KEY, provider_last_name,
...
select INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_pn 
from pn
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
quit;
...
proc sql;
create table pg as 
select INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_pg 
from pn  
...
proc sql;
create table ph as 
select INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_ph 
from pn  
where FL_FUP_DISPOSITION in (
'H - Unable to Locate')
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
...
proc sql;
create table pj as 
select INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_pj 
from pn  
where FL_FUP_DISPOSITION in (
'J - Located, Not Examined and/or Interviewed')
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
...
proc sql;
create table pk as 
select INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_pk 
from pn  
where FL_FUP_DISPOSITION in (
'K - Sent Out Of Jurisdiction')
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
...
proc sql;
create table pl as 
select INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_pl
from pn  
where FL_FUP_DISPOSITION in (
'L - Other')
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
...
proc sql;
create table pv as 
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id) as var_pv
from pn  
where FL_FUP_DISPOSITION in (
'V - Domestic Violence Risk')
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
...
proc sql;
create table px as 
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id) as var_px
from pn  
where FL_FUP_DISPOSITION in (
'X - Patient Deceased')
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
...
sum(calculated STD_PRTNRS_TRNSGNDR_TTL, calculated SOC_PRTNRS_FML_TTL,calculated SOC_PRTNRS_MALE_TTL) as count_Q, c.IX_TYPE,
D_provider.provider_last_name, f.inv_local_id , a.CA_PATIENT_INTV_STATUS, a.INVESTIGATOR_INTERVIEW_KEY, a.INVESTIGATOR_INTERVIEW_QC,
d_contact_record.CTT_PROCESSING_DECISION
from STD_HIV_DATAMART1 a 
  inner join nbs_rdb.F_INTERVIEW_CASE b
  on b.INVESTIGATION_KEY =a.INVESTIGATION_KEY
inner join nbs_rdb.D_INTERVIEW c
...
proc sql;
create table pee as 
select INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_p1
from pp1
...
proc sql;
create table peo as 
select INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_po
from pp1
...
a.FL_FUP_DISPO_DT, a.FL_FUP_INIT_ASSGN_DT,
a.INVESTIGATOR_INTERVIEW_KEY, a.INVESTIGATOR_INTERVIEW_QC,
datepart(a.FL_FUP_DISPO_DT)-datepart(a.FL_FUP_INIT_ASSGN_DT) as days
...
where 
 e.CTT_REFERRAL_BASIS in ('A1 - Associate 1','A2 - Associate 2','A3 - Associate 3',
'S1 - Social Contact 1', 'S2 - Social Contact 2', 'S3 - Social Contact 3') 
AND a.fl_fup_disposition in ('2 - Prev. Neg, New Pos' , '3 - Prev. Neg, Still Neg' , 
'4 - Prev. Neg, No Test' ,'5 - No Prev Test, New Pos' , '6 - No Prev Test, New Neg' , '7 - No Prev Test, No Test');
quit;
...
proc sql;
create table cmm as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_cm
from cm
group by INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
quit;
...
proc sql;
create table c2 as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_c2
from cm
where fl_fup_disposition in ('2 - Prev. Neg, New Pos')
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
proc sql;
create table c3 as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_c3 
from cm  
where fl_fup_disposition in ('3 - Prev. Neg, Still Neg')  
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
proc sql;
create table c4 as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_c4 
from cm  
where fl_fup_disposition in ('4 - Prev. Neg, No Test')  
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
proc sql;
create table c5 as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_c5
from cm  
where fl_fup_disposition in ('5 - No Prev Test, New Pos')  
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
proc sql;
create table c6 as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_c6
from cm  
where fl_fup_disposition in ('6 - No Prev Test, New Neg')  
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
proc sql;
create table c7 as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_c7
from cm  
where fl_fup_disposition in ('7 - No Prev Test, No Test')  
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
proc sql;
create table cn as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_cn
from cluster 
...
proc sql;
create table cg as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_cg
from cluster 
where FL_FUP_DISPOSITION in ('G - Insufficient Info to Begin Investigation')
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
proc sql;
create table ch as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_ch
from cluster 
...
proc sql;
create table cj as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_cj
from cluster 
...
proc sql;
create table ck as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_ck
from cluster 
...
proc sql;
create table cl as 
select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_cl
from cluster 
...
proc sql;
create table cv as 
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id) as var_cv
from cluster  
...
proc sql;
create table cx as 
select PROVIDER_LAST_NAME,  INVESTIGATOR_INTERVIEW_KEY, count(distinct inv_local_id) as var_cx
from cluster  
...
proc sql;
create table c1 as 
 select a.INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct f.inv_local_id) as var_c1
...
proc sql;
create table Co as 
 select INVESTIGATOR_INTERVIEW_KEY,  PROVIDER_LAST_NAME, count(distinct inv_local_id) as var_co
from cluster 
where fl_fup_disposition IS NULL 
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
select INVESTIGATOR_INTERVIEW_KEY , PROVIDER_LAST_NAME, sum(days_count_A) as Var_pr
from partner2dte
where Days le 3
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
select INVESTIGATOR_INTERVIEW_KEY , PROVIDER_LAST_NAME,  sum(days_count_A) as Var_ps
from partner2dte
where Days le 5
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
select INVESTIGATOR_INTERVIEW_KEY , PROVIDER_LAST_NAME,  sum(days_count_A) as Var_pt
from partner2dte
where Days le 7
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
select INVESTIGATOR_INTERVIEW_KEY , PROVIDER_LAST_NAME,  sum(days_count_A) as Var_pu
from partner2dte
where Days le 14
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
select INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME, sum(days_count_A) as Var_cr
from cmdte
where Days le 3
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
select INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME, sum(days_count_A) as Var_cs
from cmdte
where Days le 5
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
select INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME, sum(days_count_A) as Var_ct
from cmdte
where Days le 7
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
select INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME, sum(days_count_A) as Var_cu
from cmdte
where Days le 14
group by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_INTERVIEW_KEY, PROVIDER_LAST_NAME;
...
from pa1_new  
where HIV_900_result in ('1 - Positive/Reactive');
...
data ind1_1 (keep =INVESTIGATOR_INTERVIEW_KEY PROVIDER_LAST_NAME colgroup colname colval colpct );
set hyy;
...
title1 bold f= Calibri h=14pt  j=c "%upcase(&reportTitle)";
TITLE2 bold f= Calibri h=12pt  j=c " 		WORKER: SUMMARY OF #byval(PROVIDER_LAST_NAME)	";
...
style(column)={font_size=9pt font_face="calibri"};
by PROVIDER_LAST_NAME;
columns  PROVIDER_LAST_NAME colgroup colname colval colpct blank colname2 colval2 colpct2;

define PROVIDER_LAST_NAME / order order=data page noprint ;
define colgroup / order order=data noprint;
