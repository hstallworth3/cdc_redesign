...
public class UidGeneratorHelper extends BMPBase
{
   ...
   private final String GetMaxActUid = "select max( act_uid)  from act " ;
   private final String GetMaxEntityUid = "select max( entity_uid)  from entity " ;
   ...
   private long getMaxActUid() throws Exception
   {
        long maxActUid ;
        ...
        try
        {
            ...
            preparedStmt = dbConnection.prepareStatement(GetMaxActUid);
            resultSet = preparedStmt.executeQuery();
            resultSet.next();
            maxActUid = resultSet.getLong(1);
        }
         catch(Exception ex)
        {
   ...
   private long getMaxEntityUid() throws Exception
   {
        long maxEntityUid ;
        Connection dbConnection = null;
        ResultSet resultSet = null;
        ...
        {
            dbConnection = getConnection();
            preparedStmt = dbConnection.prepareStatement(GetMaxEntityUid);
            resultSet = preparedStmt.executeQuery();
            resultSet.next();
            maxEntityUid = resultSet.getLong(1);
        }
         catch(Exception ex)
        {
            logger.fatal("Error getting max Entity UID. ", ex);
            throw new Exception( ex.getMessage(), ex);
        }
        finally
        {
