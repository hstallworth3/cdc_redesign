...
<html>
    <head>
        ...
        function makeMSelects() {
        	$j("#condition").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
        	$j("#code").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
        	$j("#pArea").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
        	$j("#cFamily").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
        	$j("#associatedPage").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
        	$j("#nndCondition").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
        	$j("#status").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
        	$j("#cInfGroup").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
		                                    ...
		                                    <tr>
			                                    <td align="center">
			                                      ...
			                                      answerArrayText(SearchText2) answerArray(PROGRAMAREA) answerArray(ASSOCIATEDPAGE)  answerArray(CONDITIONFAMILY) answerArray(NNDCONDITION) answerArray(STATUS)  method">
                                        				...
                                        				<display:column property="editLink" title="" media="html" style="width:4%;text-align:center;"/>
