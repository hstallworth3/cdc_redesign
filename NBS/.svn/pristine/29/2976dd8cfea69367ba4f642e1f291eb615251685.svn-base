...
public class FileUploadAction extends MappingDispatchAction {
	...
	/**
	 * Method to create a new attachment record Row in the Attachment Section under Supplemental Information
	 * @param dt
	 * @return
	 */
	private static String _makeNewAttachmentRow(CTContactAttachmentDT dt) {
		
		StringBuffer sb = new StringBuffer("<td style=\"text-align: center;\">");
		
		String delLnk = "<a id=\"td_" + dt.getCtContactAttachmentUid() + "\" href=javascript:deleteAttachment(\"" + dt.getCtContactAttachmentUid() + "\")>Delete</a>";
		...
		parameterMap.put("ctContactAttachmentUid", String.valueOf(dt.getCtContactAttachmentUid()));
		parameterMap.put("fileNmTxt", dt.getFileNmTxt());			
		dt.setViewLink(buildHyperLink("DownloadFile.do", parameterMap, null, dt.getFileNmTxt()));
