	  ...
	  function printQueue() {
       ...
       function selectfilterCriteria()
        ...
        function makeMSelects() {
			$j("#uniqueId").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
			$j("#uniqueName").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
			$j("#label").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});	
		     $j("#type").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
		     $j("#grup").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
		     $j("#subgrup").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
				...
				<!-- Confirm Message -->
		           ...
		           <% if (request.getAttribute("searchConfirmMsg") != null) { %>
					  <div class="infoBox messages">
					  	<%=(String)request.getAttribute("searchConfirmMsg")%>
					  </div>    
		           <% } %>
				             ...
				             <tr>
					             <td align="center">
								  	...
								  	excludedParams="answerArrayText(SearchText1) answerArrayText(SearchText2)  answerArrayText(SearchText3) answerArray(TYPE) answerArray(GROUP) answerArray(SUBGROUP) answerArray(STATUS) method">
											...
											<display:column property="viewLink" title="" media="html" style="width:2%;text-align:center;"/>
	                                        <display:column property="editLink" title="" media="html" style="width:2%;text-align:center;"/>
