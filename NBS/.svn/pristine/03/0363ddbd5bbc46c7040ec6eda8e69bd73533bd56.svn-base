...
import gov.cdc.nedss.phdc.HL7DLNType;
...
public class HL7PatientProcessor {
	...
	public static LabResultProxyVO getPatientAndNextOfKin(
	...
	public static LabResultProxyVO getNextOfKinVO(HL7NK1Type hl7NK1Type,
			LabResultProxyVO labResultProxyVO,
			EdxLabInformationDT edxLabInformationDT) throws NEDSSAppException {
		try {
	...
	public static LabResultProxyVO getPatientVO(HL7PIDType hl7PIDType,
			LabResultProxyVO labResultProxyVO,
			EdxLabInformationDT edxLabInformationDT) throws NEDSSAppException {
		...
		try {
			...
			String toCode = dao.findToCode("ELR_LCA_SEX", personVO
...
//			}
			...
			for (int j = 0; j < ethnicArray.length; j++) {
				...
				String ethnicGroupCd = dao.findToCode("ELR_LCA_ETHN_GRP",
				...
				if(!cddv.reverseMap(cddv.getCodedValuesAsTreeMap("P_ETHN_GRP")).containsKey(personEthnicGroupDT.getEthnicGroupCd()))
				...
				for (int j = 0; j < raceArray.length; j++) {
					try {
						...
						raceDT.setPersonUid(personVO.getThePersonDT()
								.getPersonUid());
						String newRaceCat = dao.findToCode("ELR_LCA_RACE",
...
//			}
	...
	public static PersonVO personVO(LabResultProxyVO labResultProxyVO, EdxLabInformationDT edxLabInformationDT) throws NEDSSAppException {
