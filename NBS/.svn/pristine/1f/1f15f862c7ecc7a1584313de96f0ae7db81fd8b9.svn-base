...
public class LabTestLoincDAOImpl extends DAOBase {
	...
	private static final String FIND_EXISTING_LINK_ORA = "SELECT Labtest_loinc.lab_test_cd \"labTestCd\", "
			+ "Labtest_loinc.laboratory_id  \"laboratoryId\", "
			+ "Labtest_loinc.loinc_cd \"loincCd\" "
			+ "FROM "
			+ NEDSSConstants.SYSTEM_REFERENCE_TABLE
	...
	private static final String CREATE_LINK_ORA = "INSERT INTO "
			+ NEDSSConstants.SYSTEM_REFERENCE_TABLE
			+ ".Labtest_loinc(	lab_test_cd, "
			+ "laboratory_id, loinc_cd, status_cd, status_time, effective_from_time) values(?,?,?,'A',?,?)";
	...
	private static final String FIND_LOINC_ORA = " SELECT distinct loinc_cd \"loincCd\", component_name \"componentName\" FROM "
			+ NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".LOINC_code ";
	...
	private static final String FIND_LABTEST_ORA = "SELECT  distinct lab_test_cd \"labTestCd\", lab_test_desc_txt \"labTestDescTxt\", laboratory_id \"laboratoryId\" FROM "
			+ NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".Lab_test where test_type_cd='R' ";
    ...
    private static final String FIND_MANAGE_LOINC_ORA = " SELECT distinct loinc_cd \"loincCd\", component_name \"componentName\", property \"property\",time_aspect \"time_aspect\",system_cd \"system_cd\",scale_type \"scale_type\",method_type \"method_type\",display_nm \"display_name\",related_class_cd \"related_class_cd\",pa_derivation_exclude_cd \"paDerivationExcludeCd\" FROM "
	+ NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".LOINC_code ";
	...
	public Collection<Object> findLabTestLoincLink(String whereClause) throws NEDSSDAOSysException, NEDSSSystemException {
		...
		try {
	        ...
	        if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
				codeSql = FIND_EXISTING_LINK_ORA;
