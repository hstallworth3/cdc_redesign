...
PROC SQL;
CREATE TABLE 
	CODED_TABLE_SNM AS  SELECT CODED.CODE_SET_GROUP_ID, PAGE_CASE_UID, NBS_QUESTION_UID, 
...
PROC SQL; 
CREATE TABLE  
	CODED_TABLE_NONSNM AS  SELECT CODED.CODE_SET_GROUP_ID, PAGE_CASE_UID, NBS_QUESTION_UID,  
...
ON  METADATA.CODE_SET_GROUP_ID=CODED.CODE_SET_GROUP_ID 
	LEFT JOIN NBS_SRT.CODE_VALUE_GENERAL CVG ON  CVG.CODE_SET_NM=METADATA.CODE_SET_NM 
AND CVG.CODE=CODED.ANSWER_TXT where NBS_CASE_ANSWER_UID not in (select NBS_CASE_ANSWER_UID from CODED_TABLE_SNM) 
ORDER BY NBS_CASE_ANSWER_UID, RDB_COLUMN_NM; 
QUIT; 
...
RUN;
DATA CODED_TABLE ; 
  MERGE CODED_TABLE_NONSNM CODED_TABLE_SNM; 
...
RUN;
PROC SORT DATA=CODED_TABLE; BY PAGE_CASE_UID NBS_QUESTION_UID; RUN;
...
PROC SORT DATA=CODED_TABLE_OTH; BY NBS_CASE_ANSWER_UID RDB_COLUMN_NM; RUN;
...
SET NUMERIC_DATA_TRANS;
	...
	/*DROP NBS_QUESTION_UID;
	DROP NBS_CASE_ANSWER_UID;
	DROP CODE_SET_NM;
	DROP CODE;
	DROP UNIT;
	DROP X;*/
...
/*PROC SORT DATA=NUMERIC_DATA_TRANS1 nodupkey; BY PAGE_CASE_UID; RUN;*/
PROC TRANSPOSE 
...
QUIT;
DATA &STAGING_TABLE;
SET &STAGING_TABLE(DROP=_NAME_ _LABEL_);
RUN;
...
CREATE TABLE 
	&DIM AS 
SELECT * 
FROM  NBS_RDB.&LOOKUP LEFT OUTER JOIN NBS_RDB.&SOURCE 
...
PROC SORT DATA=LOOKUP_TABLE_N NODUPKEY; BY &D_KEY;RUN;
PROC SQL;
