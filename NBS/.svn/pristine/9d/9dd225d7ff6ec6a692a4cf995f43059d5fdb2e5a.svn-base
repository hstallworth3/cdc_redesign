...
public class CustomReport_DiseaseCounts_ByCounty {
	...
	@Test
	public void Populate_CustomRpt_DiseaseCount_ByCounty() throws Exception {
		comUtils.setUpDriver(driver);
		log = new Log("CustomRpt_DiseaseCounts_ByCount_Report", Log_File_Path);
		df = new DataFile(Data_File_Path, "CustomRpt_DiseaseCount_ByCounty");		
		...
		for(int col=1; col <=colCount; col++){	
			try {
				...
				String disease = template_Report.getFieldValue("Diseases",col);
				String state = template_Report.getFieldValue("State",col);
				String counties = template_Report.getFieldValue("Counties",col);
				String from_Date = template_Report.getFieldValue("From_Date",col);
				String to_Date = template_Report.getFieldValue("To_Date",col);				

				String disease_selectall = template_Report.getFieldValue("Diseases_SelectAll",col);
				String counties_selectall = template_Report.getFieldValue("Counties_SelectAll",col);
				...
				//Click on Run - Get the index of the Run button with Text -  	Custom Report for Disease Counts by County
				int index = 0;
				...
				while(itr.hasNext()) {
					index = index+1;
					if (tableRows.get(index).getText().contains("Custom Report for Disease Counts by County"))
						break;	
				}
				...
				if (!(comUtils.isElementDisplayed(By.id("error1")))){
					...
					if (comUtils.isElementDisplayed(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td/p"))){											   
						...
						if (driver.findElement(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td")).getText().contains("Custom Report For Table: PHCDemographic"))
