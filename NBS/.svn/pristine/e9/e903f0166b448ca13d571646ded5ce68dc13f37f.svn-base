...
public class NotificationProxyDaoImpl extends DAOBase
{
            ...
            NEDSSSystemException
    {
        ...
        try
        {
            ...
            // selectstatment = WumSqlQuery.NOTIFICATIONS_FOR_APPROVAL +
            // dataAccessWhereClauseNotification;
            if (PropertyUtil.getInstance().getDatabaseServerType() != null
                    && PropertyUtil.getInstance().getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
            {
                selectstatment = WumSqlQuery.NOTIFICATIONS_FOR_APPROVAL_ORACLE + dataAccessWhereClause
            ...
            TreeMap<?, ?> mapPhcClass = cache.getCodedValuesAsTreeMap("PHC_TYPE");
            mapPhcClass = cache.reverseMap(mapPhcClass); // we can add another
                                                         // method that do not
                                                         // do reverse
            ...
            for (Iterator<Object> anIterator = notificationProxyVOCollection.iterator(); anIterator.hasNext();)
            {
                ...
                {
                    notificationSummaryVO.setNotificationCd(notificationSummaryVO.getNotificationCd());
                ...
                {
                    notificationSummaryVO.setCurrSexCd(notificationSummaryVO.getCurrSexCd());
                    notificationSummaryVO.setCurrSexCdDesc(cache.getCurrentSex(notificationSummaryVO.getCurrSexCd()));
                }
                if (notificationSummaryVO.getBirthTimeCalc() != null)
                {
                    notificationSummaryVO.setBirthTimeCalc(notificationSummaryVO.getBirthTimeCalc());
                }

                if (notificationSummaryVO.getPublicHealthCaseUid() != null)
                {
                    notificationSummaryVO.setPublicHealthCaseUid(notificationSummaryVO.getPublicHealthCaseUid());
                } 
            } 
        }
        catch (Exception ex)
            ...
            throws javax.ejb.EJBException, NEDSSSystemException
    {
        ...
        try
        {
            ...
            if (PropertyUtil.getInstance().getDatabaseServerType() != null
                    && PropertyUtil.getInstance().getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
            {
                selectstatment = WumSqlQuery.REJECTED_NOTIFICATIONS_ORACLE + daysLimit + dataAccessWhereClause
