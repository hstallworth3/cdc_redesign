<?xml version="1.0" encoding="UTF-8"?>
     ...
     <script language="JavaScript">
        ...
        function cancelForm()
        {
            ...
            if (confirm(confirmMsg)) {
                document.forms[0].action ='<%=request.getAttribute("cancelButtonHref")%>';
        ...
        function saveForm() 
        {
        	...
        	if (checkForValidationErrorsOnSubmit()) {
                 ...
                 document.forms[0].action ='<%=request.getAttribute("formHref")%>?ContextAction=Submit';
                 document.forms[0].submit();
          }
        }
		...
		var typeDescTxtNode = getElementByIdOrByName('typeID_textbox');
        	...
        	if (asofDateValue == null || asofDateValue == "") {
        		getElementByIdOrByName("DEM209L").style.color="red";
        	...
        	if(censusTract.value!=null && censusTract.value!=''){
	        	...
	        	if(!re.test(censusTract.value)){
	        		getElementByIdOrByName("DEM168L").style.color="red";
        	...
        	var dobValue = jQuery.trim(dobNode.value);
        	if (dobValue != null && dobValue != "") {
        		if (!isDate(dobValue)) {
        			getElementByIdOrByName("DEM115L").style.color="red";
				var errHref = "Please correct or clear the <a href=\"javascript: getElementByIdOrByName('patientDOB').focus()\">DOB value</a>. Must be a valid date.";        		
        			errorLabels.push(errHref);
        		} else getElementByIdOrByName("DEM115L").style.color="black";
        	}
        ...
        function populateFromSearchCriteria()
	{
		...
		if (searchEthnicity != 'null')
			getElementByIdOrByName("DEM155").value = searchEthnicity;
      		...
      		var searchAsian = '<%=request.getAttribute("asianController")%>';
		if (searchAsian != 'null') {
			$j('#asianRace').attr('checked', true);
		}

      		var searchAmericanIndian = '<%=request.getAttribute("americanIndianController")%>';
		if (searchAmericanIndian != 'null') {
			$j('#americanIndianRace').attr('checked', true);
		}
		
  		var searchAfrican = '<%=request.getAttribute("africanAmericanController")%>';
		if (searchAfrican != 'null') {
			$j('#africanRace').attr('checked', true);
		}
		
  		var searchHawaiian = '<%=request.getAttribute("hawaiianController")%>';
		if (searchHawaiian != 'null') {
			$j('#hawaiianRace').attr('checked', true);
		}

  		var searchWhite = '<%=request.getAttribute("whiteController")%>';
		if (searchWhite != 'null') {
			$j('#whiteRace').attr('checked', true);
		}		
  		var searchUnknown = '<%=request.getAttribute("unknownRace")%>';
		if (searchUnknown != 'null') {
       ...
       function disableBatchEntryFields ()
	{
	  	...
	  	$j("#typeIDL").parent().find("span[title]").css("color", "#BBB");
		...
		$j("#assigningAuthorityL").parent().find("span[title]").css("color", "#BBB");
    ...
    $j('#DEM102').parent().find(':input').focus();">
            <div id="pageview"></div>
 ...
 <b> <a name="PatientSubmitErrorMessages_errorMessagesHref"></a> Please fix the following errors:</b> <br/>
   ...
   <table  display="none"> 					   
	...
	<td>
	<html:text name="DSPersonForm" property="patientAsOfDateGeneral" maxlength="10" size="10" styleId="DEM209" onkeyup="DateMask(this,null,event)"/>
	<html:img src="calendar.gif" onclick="getCalDate('DEM209','DEM209Icon'); return false;" styleId="DEM209Icon"></html:img>
	...
	<td>
	<html:textarea style="WIDTH: 500px; HEIGHT: 100px;" name="DSPersonForm" property="person.thePersonDT.description" styleId ="DEM196" onkeyup="checkTextAreaLength(this, 2000)"/>
	</td>
	</tr>
	...
	<td>
	<html:select name="DSPersonForm" property="person.thePersonDT.nmSuffix" styleId="DEM107">
   ...
   <nedss:container id="otherDetails_subsection" name="Other Personal Details" isHidden="F" classType="subSect" >
	...
	<td>
	<html:text  name="DSPersonForm" property="patientBirthTime" maxlength="10" size="10" styleId="patientDOB" onkeyup="DateMask(this,null,event)" onblur="updatePatientDOBInfo(this);" onchange="updatePatientDOBInfo(this);"/>
	...
	Current Sex:</span>
		...
		<td>
		<html:select name="DSPersonForm" property="person.thePersonDT.currSexCd" styleId="DEM113" onchange="pgSelectNextFocus(this);">
	...
	<td>
	<html:select name="DSPersonForm" property="person.thePersonDT.deceasedIndCd" styleId="DEM127" onchange="hideOrShowTarget('DEM127','deceasedDate');pgSelectNextFocus(this);">
	...
	<td><html:text  name="DSPersonForm" styleId="deceasedDate" property="patientDeceasedDate" size="10" maxlength="10" onkeyup="DateMask(this,null,event)"/>
	     <html:img src="calendar.gif" onclick="getCalDate('deceasedDate','deceasedDateIcon'); return false;" styleId="deceasedDateIcon"></html:img>
	</td>
	</tr>
	<!-- Marital -->
	<tr><td class="fieldName">
	...
	<td>
	<html:select name="DSPersonForm" property="person.thePersonDT.maritalStatusCd" styleId="DEM140">
	...
	State:</span></td>
	<td>
	  <html:select name="DSPersonForm" property="address[0].thePostalLocatorDT_s.stateCd" styleId="DEM162" onchange="getDWRPatientCounties(this, 'DEM165');">
	...
	County:</span></td>
	<td>
	  <html:select name="DSPersonForm" property="address[0].thePostalLocatorDT_s.cntyCd" styleId="DEM165">
	...
	Country:</span></td>
	<td>
	  <html:select name="DSPersonForm" property="address[0].thePostalLocatorDT_s.cntryCd" styleId="DEM167">
...
<!--Ethnicity  -->
	...
	<td>
	<html:select name="DSPersonForm" property="person.thePersonDT.ethnicGroupInd" styleId="DEM155" onchange="pgSelectNextFocus(this);">
...
<!--processing Checkbox Coded Question-->
 ...
 <input id="viewIdSubSection" type="image" src="page_white_text.gif" onclick="viewClicked(this.id,'IdSubSection','patternIdSubSection','questionbodyIdSubSection');return false" name="image" align="middle" cellspacing="2" cellpadding="3" border="55" class="cursorHand" title="View">
 </td><td style="width:3%;text-align:center;">
 <input id="editIdSubSection" type="image" src="page_white_edit.gif" onclick="editClicked(this.id,'IdSubSection');return false" name="image" align="middle" cellspacing="2" 		cellpadding="3" border="55" class="cursorHand" title="Edit">
 </td><td style="width:3%;text-align:center;">
 <input id="deleteIdSubSection" type="image" src="cross.gif" onclick="deleteClicked(this.id,'IdSubSection','patternIdSubSection','questionbodyIdSubSection');return false" name="image" align="middle" 	cellspacing="2" cellpadding="3" border="55" class="cursorHand" title="Delete">	</td>
...
<span id="tableidValue"> </span>
 ...
 <td width="5%"> &nbsp; </td>
 	...
 	<td>
 	<html:select name="DSPersonForm" property="idTypeDescCd" styleId="typeID" onchange="unhideBatchImg();hideBatchImg(this,'','');">
 	...
 	<td>
 	<html:select name="DSPersonForm" property="idAssigningAuthorityCd" styleId="assigningAuthority" onchange="unhideBatchImg();hideBatchImg('', this,'');">
