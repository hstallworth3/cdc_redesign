...
public class CaseLab_LineListing {
	...
	@Test
	public void Populate_Case_Lab_Line_Listing() throws Exception {
		comUtils.setUpDriver(driver);
		log = new Log("CaseLabLineListing_Report", Log_File_Path);
		df = new DataFile(Data_File_Path, "Case_Lab_Line_Listing");		
		...
		for(int col=1; col <=colCount; col++){	
			try {
				...
				String includeNulls = template_Report.getFieldValue("Include Nulls",col);
				String from_Date = template_Report.getFieldValue("From_Date",col);
				String to_Date = template_Report.getFieldValue("To_Date",col);
				
				if((includeNulls).equalsIgnoreCase("Yes"))
					log.write("Include Nulls: Selected;"+"   From Date: " +from_Date+";"+"  To Date:  " +to_Date);  
				else
					log.write("Include Nulls: Not Selected;"+"   From Date: " +from_Date+";"+"  To Date:  " +to_Date);  	
				
				//Click on Run - Get the index of the Run button with Text -  Case Lab Line Listing
				int index = 0;
				...
				while(itr.hasNext()) {
					index = index+1;
					if (tableRows.get(index).getText().contains("Case Lab Line Listing"))
						break;	
				}
				...
				if (!(comUtils.isElementDisplayed(By.id("error1")))){
					...
					if (comUtils.isElementDisplayed(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td/p"))){											   
						...
						if (driver.findElement(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td")).getText().contains("Custom Report For Table: INV_SUMM_DATAMART"))
					...
					log.writeSubSection("Report did NOT executed successfully","",false);
					log.takeSnapshot(driver);					
				}
