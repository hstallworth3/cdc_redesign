    ...
    implements javax.ejb.SessionBean
{
		  ...
		  if(labResultProxyVO.isItNew()){
			  try
			  {
			      ...
			      long time1 = date.getTime();
			       ...
			       ObservationVO observationVO = (ObservationVO)obsRootDAO.loadObject(((Long)returnVal.get(NEDSSConstants.SETLAB_RETURN_OBS_UID)).longValue());
			      String LocalId=observationVO.getTheObservationDT().getLocalId();
      ...
      RemoteException, javax.ejb.CreateException, NEDSSConcurrentDataException
  {
    ...
    try
    {
      ...
      List<Object> localIds = this.findLocalUidsFor(patientMprUid, observationUid, securityObj);
      returnVal.put(NEDSSConstants.SETLAB_RETURN_OBS_LOCAL, localIds.get(0));
      returnVal.put(NEDSSConstants.SETLAB_RETURN_MPR_LOCAL, localIds.get(1));
 ...
 */
  ...
  private List<Object> findLocalUidsFor(Long personMprUid, Long observationUid, NBSSecurityObj securityObj)
  {
    ActController actControllerRef = null;
    EntityController entityControllerRef = null;
    List<Object> localIds = null;

    try
    {
      //Find observation local id
      actControllerRef = this.getActControllerRemoteInterface();
      if(localIds == null) localIds = new ArrayList<Object> ();
      localIds.add(0, actControllerRef.getObservationInfo(observationUid, securityObj).getLocalId());

      //Find mpr local id
      entityControllerRef = this.getEntityControllerRemoteInterface();
      if(localIds == null) localIds = new ArrayList<Object> ();
      localIds.add(1, entityControllerRef.getPersonInfo(personMprUid, securityObj).getLocalId());
    }
    catch (Exception ex)
    {
   ...
   * @roseuid 3C9221D902BF
    ...
    */
   public LabResultProxyVO getLabResultProxy(Long observationUID,
                                             NBSSecurityObj securityObj) throws
       javax.ejb.EJBException,
       NEDSSSystemException
     {
       try {
         ...
         return lrpManager.getLabResultProxyVO(observationUID, securityObj);
       } catch(Exception e) {
...

  {
   ...
   Iterator<Object>  anIterator = null;
    ...
    try
    {
      ...
      List<Object> localIds = this.findLocalUidsFor(patientMprUid, observationUid, nbsSecurityObj);
      returnVal.add(this.OBS_LOCAL_UID, localIds.get(0));
      returnVal.add(this.MPR_LOCAL_UID, localIds.get(1));
      ...
      NEDSSConcurrentDataException
  {
    ...
    try
    {
      ...
      else
      {
        ...
        {
          sql = WumSqlQuery.ASSOC_MORB_COUNT_ORACLE;
        }
        else
        {
      ...
      NEDSSConcurrentDataException
  {
    ...
    try
    {
      ...
      if ((observationDT.getRecordStatusCd().equalsIgnoreCase(NEDSSConstants.
          OBS_UNPROCESSED)) || (observationDT.getRecordStatusCd().equalsIgnoreCase(NEDSSConstants.OBS_UNPROCESSED_PREV_D)))
      {
