...
public class CTContactLoadUtil {
	...
	public static void addContactPageLoadUtil(CTContactForm form, HttpServletRequest request, HttpServletResponse response, String personUid) throws Exception {
		...
		try {
			...
			form.setFormFieldMap(reUtils.initiateForm(PageLoadUtil.questionMap,  (BaseForm) form, (ClientVO) form.getcTContactClientVO()));
	...
	public static CTContactProxyVO createHandler(CTContactForm form, HttpServletRequest req) throws Exception {
		...
		try {
			...
			req.setAttribute("ActionMode", form.getActionMode());
		} catch (Exception e) {
						logger.error("Error: "+e.getMessage());
			e.printStackTrace();
		}
	...
	public static CTContactProxyVO createPageHandler(CTContactForm form,
		...
		try {
			...
			Long providerUid = nbsSecurityObj.getTheUserProfile().getTheUser()
					.getProviderUid();
			HttpSession session = req.getSession();
			PageCreateHelper.loadQuestions(form.getPageFormCd());
			PageCreateHelper.handleFormRules((BaseForm) form,
			...
			if (form.getErrorList() == null || form.getErrorList().size() == 0) {
				...
				proxyVO.getcTContactVO()
						.setCtContactAnswerDTMap(PageCreateHelper
				...
				proxyVO.getcTContactVO().setRepeatingAnswerDTMap(
						PageCreateHelper.setRepeatingQuestionsBatch(form,
...
//								 				if(! lotNbr.equals(invVo.getEpiLinkId()))
	...
	private static void setEntitiesForCreateEdit(CTContactForm form, CTContactProxyVO proxyVO, int revisionPatientUID, String versionCtrlNbr, String userId) {
	 ...
	 * @param userid
	 */
	private static void setPatientForEventPageCreate(Long patientUid, int tempID, CTContactProxyVO proxyVO, CTContactForm form, HttpServletRequest req, String userId) {
		PersonVO personVO = new PersonVO();
	...
	private static void setPatientForEventCreate(Long patientUid, int tempID, CTContactProxyVO proxyVO, CTContactForm form, HttpServletRequest req, String userId) {
	...
	private static int setCtContactVOForCreate(int tempID, CTContactForm form, HttpServletRequest req, CTContactProxyVO proxy, String userId) {
	...
	public static void _loadEntities(CTContactForm form, CTContactProxyVO proxyVO, HttpServletRequest request) {
		...
		try {
			...
			if (dispositionPersonVO  != null) {
				...
				QuickEntryEventHelper helper = new QuickEntryEventHelper();
				form.getAttributeMap().put(CTConstants.DispositionedBy+"SearchResult", helper.makePRVDisplayString(dispositionPersonVO ));
			}
			...
			if (otherInfectedPatientUid != null) {
				...
				if (otherInfectedPatient != null) {
					...
					form.getAttributeMap().put(CTConstants.OtherInfectedPatient+"SearchResult", helper.makePatientDisplayString(otherInfectedPatient));
				}
			}
			...
			if (organizationVO != null) {
				...
				QuickEntryEventHelper helper = new QuickEntryEventHelper();
				form.getAttributeMap().put("CON106SearchResult", helper.makeORGDisplayString(organizationVO));
			}
	...
	public static CTContactProxyVO editContactPageHandler(CTContactForm form, HttpServletRequest req) throws Exception {
		...
		try {
			...
			form.setErrorList(new ArrayList<Object>());
			String invFormCd = form.getPageFormCd();
			PageCreateHelper.loadQuestions(invFormCd);
			PageCreateHelper.handleRaceForRules((ClientVO)form.getcTContactClientVO());
			PageCreateHelper.handleFormRules((BaseForm) form,(ClientVO)form.getcTContactClientVO(), NEDSSConstants.EDIT_SUBMIT_ACTION);
			proxyVO = new CTContactProxyVO();
...
//						.putAll(PageCreateHelper.updateNbsAnswersForDirty(oldAnswers, newAnswers));
				...
				//proxyVO.getcTContactVO().setRepeatingAnswerDTMap
				proxyVO.getcTContactVO().setRepeatingAnswerDTMap(
						PageCreateHelper.setRepeatingQuestionsBatch((BaseForm)form,
								form.getPageFormCd(), userId, providerUid));
    ...
    private static void populatePatientSummary(CTContactForm form,CTContactProxyVO proxyVo, PersonVO personVO, String userId, String conditionCd, boolean newPat, HttpServletRequest request)
    {
    	...
    	try {
			if(!newPat)
			{
				...
				form.getAttributeMap().put("UpdatedBy",ctContactDt.getLastChgUserId() == null ? "" : helper.getUserName(ctContactDt.getAddUserId()));
			}
			else
			{
				Date date = new Date();
			...
			while (itr.hasNext()) {
				...
				if (investigation != null
					...
					if (investigation.getInvestigationStatusCd().equals("O")) {
						String status = "<b><font color=\"green\">Open</font></b>";
						dt.setStatus(status);
					} else {
...
//		}
	...
	private static void setCoinfectionData(PageActProxyVO proxyVO,HttpServletRequest req){
		try{
			...
			PageLoadUtil.loadQuestions(investigationFormCd);
			...
			Map<Object, Object> coInfectionQuestionMap = PageLoadUtil.coinfectionQuestionKeyMap;
