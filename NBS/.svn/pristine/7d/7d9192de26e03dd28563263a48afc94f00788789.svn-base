...
public class EDXObservationDAO {
	...
	public static ObservationDT matchingObservationInODS(EdxLabInformationDT edxLabInformationDT) throws NEDSSAppException {
		...
		String addreasonCd = edxLabInformationDT.getAddReasonCd();
		...
		try {
			...
			if (observationVO != null) {
				obsDT = odsObsDAO.matchingObservation(fillerNumber, // Accession number
						addreasonCd, // ReasonCd of the AUT participation
						clia, // Lab's CLIA #
						observationVO.getTheObservationDT().getCd(), // OT test code
						observationVO.getTheObservationDT().getEffectiveFromTime()); // Specimen collection date
			} else {
