...
public class NETSSMessageProcessor {
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			if(args.length == 1){
			    ...
			    {
					System.out.println("Usage :  NETSSMessageProcessor.bat [USER_ID] [Optional Ending Date in format MM/dd/yyyy]");
		...
		if (processNETSSRequest(userName, specifiedEndingDate))
	 ...
	 */
	private static boolean processNETSSRequest(String userName, Date specifiedEndingDate) {
		Short mmwrYear = null;
		Short mmwrWeek = null;
		Boolean priorYear =  null;
		short[] mmwrVals = getMMWRAndPriorYearBoolean(specifiedEndingDate);
		mmwrWeek = mmwrVals[0];
		mmwrYear = mmwrVals[1];
		if (mmwrVals[2] == 0)
			priorYear = false;
	 ...
	 * @return
	 */
	private static short[] getMMWRAndPriorYearBoolean(Date specifiedDate) {
		...
		if (lastSaturday.after(cutoffDate)) {
