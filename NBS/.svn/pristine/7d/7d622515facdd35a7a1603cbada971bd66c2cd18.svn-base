...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.cdc.gov/NEDSS" xmlns:xs="http://www.cdc.gov/NEDSS">
								...
								<xsl:for-each select="xs:ORDER_OBSERVATION">
									...
									<xsl:if test="xs:CommonOrder">
										<div class="bluebarsect" id="orderTest_{position()}">
											<table class="bluebarsectHeader" id="orderTestTable_{position()}">
												<tr>
													<td class="bluebarsectName">
														...
														<table id="ResultedTestTable-{position()}">
															<tbody>
																...
																<tr class="even1">
																	...
																	</xsl:call-template>&nbsp;<xsl:value-of select="../xs:Units/xs:HL7Identifier"/><br/>
																	</xsl:for-each>
																	</td>
																	<td>
																...
																<tr class="even1">
																	...
																	<td>
																		<xsl:choose>
																			<xsl:when test ="xs:ObservationResult/xs:ProducersReference">
																				...
																				<xsl:if test="normalize-space(xs:ObservationResult/xs:ProducersReference/xs:HL7Identifier)!=''">
																					(<xsl:value-of select="xs:ObservationResult/xs:ProducersReference/xs:HL7Identifier"/><xsl:if test="normalize-space(xs:ObservationResult/xs:ProducersReference/xs:HL7NameofCodingSystem)!=''">
																			...
																			<xsl:otherwise>
																				<xsl:if test="normalize-space(xs:ObservationResult/xs:PerformingOrganizationName/xs:HL7OrganizationName)!=''">
																					<xsl:value-of select="xs:ObservationResult/xs:PerformingOrganizationName/xs:HL7OrganizationName"/>
																				</xsl:if>
																				<xsl:if test="normalize-space(xs:ObservationResult/xs:PerformingOrganizationName/xs:HL7OrganizationIdentifier)!=''">
																					(<xsl:value-of select="xs:ObservationResult/xs:PerformingOrganizationName/xs:HL7OrganizationIdentifier"/>)</xsl:if>
																		</xsl:otherwise>
																		</xsl:choose>
																	</td>
																</tr>
	...
	<xsl:template name="ELRPatient">
		...
		<xsl:for-each select="xs:HL7PATIENT_RESULT/xs:PATIENT">
			<div class="bluebarsect" id="patientInfo">
				<table class="bluebarsectHeader" id="patientInfoHeaderTable">
					<tr class="headerRow">
						<td class="bluebarsectName">
	...
	<xsl:template name="dateFormatter">
		<xsl:if test="xs:year!=''">
			<xsl:copy>
				<xsl:if test="xs:month &lt; '10'">0</xsl:if><xsl:value-of select="xs:month"/>/<xsl:if test="xs:day &lt; '10'">0</xsl:if><xsl:value-of select="xs:day"/>/<xsl:value-of select="xs:year"/>
			</xsl:copy>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="dateTimeFormatter">
			...
			<xsl:copy>
				<xsl:if test="xs:month &lt; '10'">0</xsl:if><xsl:value-of select="xs:month"/>/<xsl:if test="xs:day &lt; '10'">0</xsl:if><xsl:value-of select="xs:day"/>/<xsl:value-of select="xs:year"/>&nbsp;<xsl:if test="xs:hours!=''"><xsl:if test="xs:hours &lt; '10'">0</xsl:if><xsl:value-of select="xs:hours"/>:<xsl:if test="xs:minutes &lt; '10'">0</xsl:if><xsl:value-of select="xs:minutes"/></xsl:if>
			</xsl:copy>
	</xsl:template>
	...
	<xsl:template name="NameFormat">
		...
		</xsl:if><xsl:if test="normalize-space(xs:HL7FamilyName/xs:HL7Surname)!=''">&nbsp;<xsl:value-of select="xs:HL7FamilyName/xs:HL7Surname"/>
		</xsl:if><xsl:if test="normalize-space(xs:HL7Suffix)!=''">,&nbsp;<xsl:value-of select="xs:HL7Suffix"/>
		</xsl:if>
	...
	<xsl:template name="NameFormat1">
		...
		<xsl:if test="normalize-space(xs:HL7SecondAndFurtherGivenNamesOrInitialsThereof)!=''">
			&nbsp;<xsl:value-of select="xs:HL7SecondAndFurtherGivenNamesOrInitialsThereof"/>
		</xsl:if><xsl:if test="normalize-space(xs:HL7FamilyName)!=''">&nbsp;<xsl:value-of select="xs:HL7FamilyName"/></xsl:if><xsl:if test="normalize-space(xs:HL7Suffix)!=''">,&nbsp;<xsl:value-of select="xs:HL7Suffix"/>
		</xsl:if>
		<xsl:value-of select="$type"/>
	</xsl:template>
	<xsl:template name="TelephoneFormat">
		...
		<xsl:variable name="localNumber">
			...
			(<xsl:value-of select="normalize-space(xs:HL7AreaCityCode)"/>)&nbsp;<xsl:value-of select="normalize-space(concat(substring($localNumber,1,3),'-',substring($localNumber,4,7)))"/><xsl:if test="normalize-space(xs:HL7Extension)!=''">&nbsp;ext.&nbsp;<xsl:value-of select="normalize-space(xs:HL7Extension)"/>
			</xsl:if>
			<xsl:choose>
				<xsl:when test="$type!=''">
					&nbsp;<xsl:value-of select="$type"/>
	...
	<xsl:template name="AddressFormat">
		...
		</xsl:if><xsl:if test="normalize-space(xs:HL7OtherDesignation)!=''">,&nbsp;<xsl:value-of select="xs:HL7OtherDesignation"/>
		</xsl:if>
