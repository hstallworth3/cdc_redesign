...
<html>
	...
	function makeMSelects() {
	             $j("#fEventType").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
	             $j("#fAlgorithmName").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
	             $j("#fRelatedConditions").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
	             $j("#fAdvancedCriteria").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
	             $j("#fAction").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
	             $j("#fLastUpdated").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
	             $j("#fStatus").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
	}
    ...
    function submitAttachment() {
      ...
      if (jQuery.trim(document.decisionSupportLibraryForm.importFile.value) == "") {
          ...
          hasErrors = true;
          $j("td#chooseFileLabel").css("color", "red");
      }
      else {
    ...
    <body onload="attachIcons();makeMSelects();showCount();displayToolTips();startCountdown();">
		<div id="blockparent"></div>
		  <div id="doc3">
           ...
           <% if(request.getAttribute("error") != null) { %>
            ...
            <div id="addAttachmentBlock" class="boxed">
		        ...
		        <!-- File Attachment -->
		        <tr>
		            <td class="fieldName" id="chooseFileLabel"> <span style="color:red; font-weight:bold;">*</span>Choose File:</td>
		            <td><html:file property="importFile" style="height: 1.8em;" maxlength="70" size= "70"> </html:file></td>	           
		        </tr> 
		         <tr>
			             ...
			             <td align="center">
							  	<display:table name="algorithmList" class="dtTable" style="margin-top:0em;" pagesize="20"  id="parent" requestURI="/ManageDecisionSupport.do?method=loadqueue&existing=true&initLoad=true" 
								   ...
								   answerArray(EVENTTYPE) answerArrayText(SearchText1) answerArrayText(SearchText2) answerArray(ACTION) answerArray(STATUS) answerArray(LASTUPDATED) method">
							  	    ...
							  	    <display:column property="viewLink" title="" media="html" style="width:3%;text-align:center;"/>
                                    <display:column property="editLink" title="" media="html" style="width:3%;text-align:center;"/>
		  ...
		  <jsp:include page="DSMLibraryDropDown.jsp" />		   
		   <input type='hidden' id='SearchText1' value="<%= request.getAttribute("FILTERBYUNIQUEALGORITHMNAME")!=null? request.getAttribute("FILTERBYUNIQUEALGORITHMNAME"):""%>"/>
		   <input type='hidden' id='SearchText2' value="<%= request.getAttribute("FILTERBYCONDITIONTEST")!=null? request.getAttribute("FILTERBYCONDITIONTEST"):""%>"/>
		  
		   <input type='hidden' id='sortSt' value="<%= request.getAttribute("SORTORDERPARAM")!=null? request.getAttribute("SORTORDERPARAM"):""%>"/>
		   <input type='hidden' id='INV111' value="<%= request.getAttribute("SRCHCRITERIAEVENTTYPE")!=null? request.getAttribute("SRCHCRITERIAEVENTTYPE"):""%>"/>
		   <input type='hidden' id='INV222' value="<%= request.getAttribute("SRCHCRITERIAACTION")!=null? request.getAttribute("SRCHCRITERIAACTION"):""%>"/>
		   <input type='hidden' id='INV333' value="<%= request.getAttribute("SRCHCRITERIALASTUPDATED")!=null? request.getAttribute("SRCHCRITERIALASTUPDATED"):""%>"/>
		   <input type='hidden' id='INV444' value="<%= request.getAttribute("SRCHCRITERIASTATUS")!=null? request.getAttribute("SRCHCRITERIASTATUS"):""%>"/>
