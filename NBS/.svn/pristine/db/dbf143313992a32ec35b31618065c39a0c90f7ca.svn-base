...
public class ObservationDAOImpl extends BMPBase
{
   ...
   // private static final String ACT_UID = "ACTIVITY_UID";
    ...
    public static final String ELR_MATCH_OBSERVATION = "Select ob.observation_uid "+
          "from observation ob, (Select act_uid from Act_id where type_cd='FN' "+
          "and root_extension_txt = ?)a , (select obs.observation_uid "+
          "from observation obs, participation par,  entity_id eid "+
    ...
    public ObservationDT matchingObservation (String fillerNbr,
                                              String addReasonCd,
                                              String reportingLabRootExtensionTxt,
                                              String orderedTestCd,
                                              Timestamp specimenCollectionDate) throws NEDSSSystemException
    {
        if (fillerNbr == null)   return null; //no match
        ...
        try
        {
            ...
            preparedStmt = dbConnection.prepareStatement(ELR_MATCH_OBSERVATION);
            preparedStmt.setString(1, fillerNbr);
            preparedStmt.setString(2, addReasonCd);
            preparedStmt.setString(3, reportingLabRootExtensionTxt);
            preparedStmt.setString(4, orderedTestCd);
            preparedStmt.setTimestamp(5, specimenCollectionDate);
            ...
            if (resultSet.next())
            {
                ...
                if (l!=0)
                {
                  matchedUID = new Long (l);
                }
            }
        }
        catch(Exception ex)
        {
            logger.fatal("An Exception occurred when matching an observation with fillerNbr=->" + fillerNbr, ex);
...
*/
    ...
    public Collection<Object> associatedPublicHealthCaseForMprForCondCd (long mprUid, String conditionCode) throws NEDSSSystemException {
    	PublicHealthCaseDT phcDT = new PublicHealthCaseDT();
    	   ArrayList<Object> returnColl =  new ArrayList<Object> ();
    	   Connection dbConnection = null;
    	   PreparedStatement preparedStmt = null;
    	   ...
    	   try
    	   {
    		   String SELECT_ASSOCIATED_PHC_FOR_LAB =" select public_health_case.cd \"cd\", Public_health_case.record_status_cd \"recordStatusCd\", Public_health_case.public_health_case_uid \"publicHealthCaseUid\", Public_health_case.add_time \"addTime\", observation.effective_from_time \"associatedSpecimenCollDate\" "
    		   +" from "
    		   +"  Participation "
    		   +" inner join person "
    		   +" on person.person_uid=Participation.subject_entity_uid "
    		   +" inner join Public_health_case "
    		   +" on Participation.act_uid = Public_health_case.public_health_case_uid "
               ...
               preparedStmt.setLong(2, mprUid);
               resultSet = preparedStmt.executeQuery();
               resultSetMetaData = resultSet.getMetaData();
    		   
    		   
               returnColl = (ArrayList<Object> )resultSetUtils.mapRsToBeanList(resultSet, resultSetMetaData, phcDT.getClass(), returnColl);
    	   }
    	   catch(SQLException sqlException)
    	   {
    		   logger.fatal("ObservationDAOImpl.openPublicHealthCaseForLab sqlException while getting dbConnection for checking for an existing phcDT " ,sqlException);
