...
public class HomePageChartsDAO extends DAOBase {
    ...
    static {
        String strServer = np.getDatabaseServerType();
        if(strServer != null) {
            if(strServer.equalsIgnoreCase(NEDSSConstants.ORACLE_ID)){
                isOracle = true;
            }
        }
    }	
	...
	public Collection<Object> selectOpenInvsPastXDays(NBSSecurityObj nbsSecurityObj, boolean secured, ChartReportMetadataDT dt) throws NEDSSDAOSysException {
		...
		try{
		    ...
		    if(isOracle) {
		    	 sql = 	"SELECT TO_CHAR(TRUNC(t1.myDate),'MM/DD/YYYY') \"hitDate\", NVL(t2.\"hitCount\",0) \"hitCount\"  FROM (SELECT TRUNC(sysdate) - ROWNUM+1 AS mydate FROM ALL_TAB_COLS WHERE ROWNUM<=7) t1 LEFT JOIN (" +
		    		 	" SELECT COUNT(*) \"hitCount\", TRUNC(ADD_TIME) \"hitDate\""+ 
	 			 		" FROM PUBLIC_HEALTH_CASE WHERE (INVESTIGATION_STATUS_CD = 'O' OR INVESTIGATION_STATUS_CD = 'C') AND CASE_TYPE_CD='I' AND RECORD_STATUS_CD <> 'LOG_DEL'" + 
	 			 		" AND ADD_TIME > TRUNC(sysdate)-7";
		    	 if(secured && dataAccessWhereClause.length() > 0)
			    	sql += " AND " + dataAccessWhereClause;
	
		 	   sql+= " GROUP BY TRUNC(ADD_TIME) ORDER BY \"hitDate\"";
		 	   sql+= " ) t2 ON t1.mydate=t2.\"hitDate\" order by t1.myDate";
		    }		
	...
	public Collection<Object> selectLabsPastXDays(NBSSecurityObj nbsSecurityObj, boolean secured, ChartReportMetadataDT dt) throws NEDSSDAOSysException {
		...
		try{
	    	...
	    	if(isOracle) {
	    		sql = 	"SELECT TO_CHAR(TRUNC(t1.myDate),'MM/DD/YYYY') \"hitDate\", NVL(t2.\"hitCount\",0) \"hitCount\", t3.section \"section\"" +   
		    	   		" FROM (SELECT TRUNC(sysdate) - ROWNUM+1 AS mydate FROM ALL_TAB_COLS WHERE ROWNUM<=7) t1" + 
		    	   		" CROSS JOIN (SELECT 'Manual' AS section FROM DUAL UNION ALL SELECT 'ELR' AS section FROM DUAL) t3" +   
		    	   		" LEFT JOIN ( SELECT COUNT(*) \"hitCount\", CASE ELECTRONIC_IND WHEN 'Y' THEN TRUNC(RPT_TO_STATE_TIME) ELSE TRUNC(ADD_TIME) END \"hitDate\",  CASE ELECTRONIC_IND WHEN 'Y' THEN 'ELR' ELSE 'Manual' END \"section\" FROM OBSERVATION WHERE CTRL_CD_DISPLAY_FORM='LabReport' and OBS_DOMAIN_CD_ST_1='Order' AND (ADD_TIME>trunc(sysdate)-7 OR RPT_TO_STATE_TIME> trunc(sysdate)-7) AND RECORD_STATUS_CD <> 'LOG_DEL'";
		    	   
		    	   if(secured && dataAccessWhereClause.length() > 0)
		    		   sql += " AND " + dataAccessWhereClause;
		    	   
	   	    	sql += 	" GROUP BY  CASE ELECTRONIC_IND WHEN 'Y' THEN TRUNC(RPT_TO_STATE_TIME) ELSE TRUNC(ADD_TIME) END, CASE ELECTRONIC_IND WHEN 'Y' THEN 'ELR' ELSE 'Manual' END ORDER BY \"hitDate\" ) t2" + 
	   	    			" ON t1.mydate=t2.\"hitDate\" AND t2.\"section\"=t3.section";
	   	    			//" order by \"hitDate\", t3.section desc";
		    }
	...
	public Collection<Object> selectInvAssignPastXDays(NBSSecurityObj nbsSecurityObj, boolean secured, ChartReportMetadataDT dt) throws NEDSSDAOSysException {
		...
		try{
		    ...
		    if(isOracle) {
		    	sql = "SELECT NVL(PERSON.LAST_NM,'No Last') || ', '  || NVL(PERSON.FIRST_NM,'No First') \"xAxis\", COUNT(*) \"hitCount\"," +
			    " CASE PHC.INVESTIGATION_STATUS_CD WHEN 'C' THEN 'Closed' WHEN 'O' THEN 'Open' END \"section\"" +
			    " FROM PUBLIC_HEALTH_CASE PHC INNER JOIN PARTICIPATION P ON PHC.PUBLIC_HEALTH_CASE_UID = P.ACT_UID INNER JOIN" +
			    " PERSON ON P.SUBJECT_ENTITY_UID = PERSON.PERSON_UID WHERE (P.TYPE_CD = '" + NEDSSConstants.PHC_INVESTIGATOR + "') AND PHC.CASE_TYPE_CD='I'" +
			    " AND NVL(P.FROM_TIME,PHC.INVESTIGATOR_ASSIGNED_TIME)>trunc(sysdate)-14";
		    	if(secured && dataAccessWhereClause.length() > 0)
			    	sql += " AND " + dataAccessWhereClause;
		    	sql+= " GROUP BY NVL(PERSON.LAST_NM,'No Last') || ', '  || NVL(PERSON.FIRST_NM,'No First'),PHC.INVESTIGATION_STATUS_CD ORDER BY \"xAxis\"";
