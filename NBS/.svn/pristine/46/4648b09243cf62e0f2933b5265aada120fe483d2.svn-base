...
public class PersonSearchLoad extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (contextAction.equalsIgnoreCase("GlobalPatient") || contextAction.equalsIgnoreCase("GlobalMP_ManualSearch")
			...
			psForm.setActionMode("FindPatient");
			psForm.setPageTitle("Find Patient",request);
			
			
		//	ArrayList<Object> usrList = new ArrayList<Object>();
				...
				//  logger.info("About to call processRequest.");
				   Object[] oParams = new Object[] {secObj};
				  ArrayList<Object> arr;
				try {
					arr = (ArrayList<Object> )msCommand.processRequest(sBeanJndiName, sMethod, secObj);
					 //logger.debug("Checkpoint - #");
					users =  (Collection)arr.get(0);
				} catch (InvocationTargetException | IllegalAccessException | EJBException | NEDSSAppException
						| CreateException | NamingException e) {
					// TODO Auto-generated catch block
					 logger.fatal("Exception in get User list in Person search Form.");
					e.printStackTrace();
				}
			 ...
			 // util.sortObjectByColumn(sortMethod, users, true);
			  ...
			  if(users!=null && users.size()>0)
			  {
			  	...
			  	while(ite.hasNext()){
	    			...
	    			user.setUserID(secureUserDT.getUserId());
	    		sbUser.append(user.getUserID()).append("$");
		  ...
		  // psForm.setUserCreateList(sbUser.toString());
			        
			/*	ArrayList<Object> returnList = new ArrayList<Object>();
				try {
					RetrieveSummaryVO rsVO = new RetrieveSummaryVO();
					returnList = rsVO.getUserList();
				} catch (NEDSSSystemException nse) {
					logger.fatal("PersonSearchLoad.execute: NEDSSSystemException: " + nse.getMessage(), nse);
					throw new NEDSSSystemException(nse.getMessage(),nse);
				} catch (Exception e) {
					logger.fatal("PersonSearchLoad.execute: Exception: " + e.getMessage(), e);
					throw new EJBException(e.getMessage(),e);
				}
			psForm.setUserCreateList(returnList);
			*/
			...
			request.setAttribute("EIDOp", psVO.getDocOperator());
			request.setAttribute("DateOp", psVO.getDateOperator());
			request.setAttribute("TestOP", psVO.getResultedTestCodeDropdown());
			request.setAttribute("CodedOp", psVO.getCodeResultOrganismDropdown());
	...
	private String getusers(NBSSecurityObj secObj) {
...
private ArrayList<Object> chooseInvestigation(String programAreas)
    {		
