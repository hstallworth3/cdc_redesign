...
public class HIVInvestigation_LabReport {
	...
	public void setUp() throws Exception {
		...
		Data_File_Path = login.Data_File_Path + "investigations\\HIVInvestigation_LabReport.xls";
	...
	public void Generic_LabReport() throws Exception {
		...
		log = new Log("HIVInvestigation_LabReport", Log_File_Path);
		df = new DataFile(Data_File_Path, "Lab_MetaData");
		Create_LabReport HIVLab = new Create_LabReport();
		HIVLab.setUpDriver(driver);
		HIVLab.setUpLog(log);
		HIVLab.setUpDataFile(df);
		...
		for (int col = 1; col <= colCount; col++) {
			try {
				...
				patID = HIVLab.Populate_DE_LabReport(col);
				driver = HIVLab.getDriver();
				log = HIVLab.getLog();
				...
				ReadMetaData HIVInvestigation = new ReadMetaData();
				HIVInvestigation.setUpDriver(driver);
				HIVInvestigation.setUpDataFile(df);

				HIVInvestigation.ReadData(col);

				driver = HIVInvestigation.getDriver();
				df = HIVInvestigation.getDataFile();
				Thread.sleep(1000);
