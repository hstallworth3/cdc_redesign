...
<script language="javascript">
        function showPrintFriendlyPage()
        {
            var o = new Object();
            o.opener = self;
            var date = '<%=(String)request.getAttribute("dateReceivedHidden")%>';
            var URL = '/nbs/viewELRDocument.do?method=printELRDocument&documentUid='+<%=request.getAttribute("documentUid")%>+'&mode=print&dateReceivedHidden='+date;
		...
		<div id="blockParent"></div>
    		<div id="doc3">
    			...
    			<div class="grayButtonBar" style="text-align: right;">
					...
					<div id="bd">
						 <input id="dateReceivedHidden" type="hidden" value="<%=(String)request.getAttribute("dateReceivedHidden")%>"/>
