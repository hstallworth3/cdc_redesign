...
public class NETSSTransportQOutDAO extends DAOBase{
	...
	private static String FROM_CLAUSE_ORA = "FROM NETSS_TransportQ_out) nt ";
	...
	public Collection<Object>  getNETSSTransportQOutDTCollectionForYear(Short mmwrYear, Short mmwrWeek, Boolean includePriorYear) throws NEDSSSystemException{
        ...
        if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
        	if (includePriorYear)
        		sql = SELECT_NETSS_CASE_DATA_COLLECTION + FROM_CLAUSE_ORA + WHERE_CLAUSE_YTD_AND_PRIOR_YEAR;
        	else
        		sql = SELECT_NETSS_CASE_DATA_COLLECTION + FROM_CLAUSE_ORA + WHERE_CLAUSE_YTD;
        else
        	if (includePriorYear)
