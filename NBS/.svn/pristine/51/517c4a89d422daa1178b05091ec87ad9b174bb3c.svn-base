...
public class CnTransportQOutDAOImpl extends DAOBase{
        ...
        try
        {
            ...
            while(resultSet.next()) {
    			...
    			if(propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID)){
    		        java.sql.Blob blob = (java.sql.Blob) resultSet.getBlob("messagePayload");

    		      
    		        byte[] bdata = blob.getBytes(1, (int) blob.length());
    		        String txt = new String(bdata);

    		        cnTransportQOutDT.setMessagePayload(txt);
    			}else     				
	...
	public void insertCnTransportQOutDT (CnTransportQOutDT cnTransportQOutDT) throws NEDSSDAOSysException
    {
	        ...
	        if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
	        	sql = INSERT_CN_TRANSPORTQ_OUT_ORACLE;
	        else
			...
			if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
			{
				try{
				    ...
				    preparedStmt = dbConnection.prepareStatement("SELECT message_payload FROM " + DataTables.CN_TRANSPORTQ_OUT_TABLE + " WHERE notification_uid = ? and record_status_cd = '" + NEDSSConstants.NND_UNPROCESSED_MESSAGE + "' FOR UPDATE");
				    preparedStmt.setLong(1, cnTransportQOutDT.getNotificationUID().longValue()); //1
				    resultSet = preparedStmt.executeQuery();
				    resultSet.next();
	
				    java.sql.Blob blob = (java.sql.Blob) resultSet.getBlob("message_payload");
				    writeBinaryDataToBlob(blob, payload);
				}
				catch(SQLException sqlex)
				{
					String errorMsg = "SQLException while inserting BLOB into CnTransportQOutDT:" + 
									   cnTransportQOutDT.toString() + "\n";
				    logger.fatal(errorMsg, sqlex);
