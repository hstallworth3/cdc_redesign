...
public class DataSourceDAO extends BMPBase
{
    ...
    {
        "SELECT DISTINCT code_set_nm from " + NEDSSConstants.SYSTEM_REFERENCE_TABLE + "..CODE_VALUE_GENERAL ORDER BY code_set_nm",
        "SELECT DISTINCT code_set_nm from " + NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".CODE_VALUE_GENERAL ORDER BY code_set_nm",
    };
    	
    private static final String[] CODE_VALUE_CLINICAL_SRTS = 
    {
        "SELECT DISTINCT code_set_nm from " + NEDSSConstants.SYSTEM_REFERENCE_TABLE + "..CODE_VALUE_CLINICAL",
        "SELECT DISTINCT code_set_nm from " + NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".CODE_VALUE_CLINICAL",
    };
    ...
    public DataSourceDT view(long uid) throws NEDSSDAOSysException
    {
        ...
        try
        {
            ...
            if(rs.next())
            {
                ...
                dt.setDataSourceName(rs.getString("data_source_name").toUpperCase());
                dt.setDataSourceTitle(rs.getString("data_source_title"));
                dt.setDescTxt(rs.getString("desc_txt"));
                dt.setJurisdictionSecurity(rs.getString("jurisdiction_security"));
                dt.setReporting_facility_security(rs.getString("reporting_facility_security"));
            }
        }
        catch(Exception ex)
        {
    ...
    public void validate(DataSourceDT dt) throws NEDSSDAOSysException
    {
        ...
        try
        {
	        if(strDatabase.equalsIgnoreCase("nbs_rdb")) {
	        ...
	        sbDSN.append(strTable);
	        String strDSN = sbDSN.toString();
	        // Assemble SQL.
	        StringBuffer sbSQL = new StringBuffer();
	        sbSQL.append("select * from ");
	        sbSQL.append(strDSN);
	        sbSQL.append(" where 1 = 0");
        ...
        catch(Exception ex)
        {
            ...
            {
                throw new NEDSSDAOSysException("There is no data mart by that name.  Please enter a name that matches the name of an existing data mart.");
            }
