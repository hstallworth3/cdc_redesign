...
public class TriggerCodeDAOImpl extends DAOBase {
	...
	private static final String RETRIEVE_CODE_DESC_FROM_CODE_VALUE_GENERAL_ORA="SELECT "
	...
	public ArrayList<Object>  getAlgorithmTriggersforAlgrthm(Long algUid)throws NEDSSSystemException {
		...
		//  logic to check if code has separate table
		if (NEDSSConstants.ORACLE_ID.equalsIgnoreCase(propertyUtil.getDatabaseServerType())){
			//Need to include the Oracle Query 
	...
	public Collection<Object> getTriggerCodes()throws NEDSSSystemException {
		...
		//  logic to check if code has separate table
		if (NEDSSConstants.ORACLE_ID.equalsIgnoreCase(propertyUtil.getDatabaseServerType())){
			sqlQuery = WumSqlQuery.SELECT_TRIGGER_CODE_ORA;
		}
		else{
		...
		try {
			...
			resultSet = preparedStmt.executeQuery();
			ArrayList<Object>  triggerCodesList = new ArrayList<Object> ();
			if(resultSet!=null){
				resultSetMetaData = resultSet.getMetaData();
	...
	public ExportReceivingFacilityDT getReceivingFacilityByShortNm(String shortNm)throws NEDSSSystemException {
		...
		//  logic to check if code has separate table
		if (NEDSSConstants.ORACLE_ID.equalsIgnoreCase(propertyUtil.getDatabaseServerType())){
			sqlQuery = WumSqlQuery.SELECT_RECEIVING_FACILITY_ORACLE + WHERE_CLAUSE;
		}
		else{
	...
	public Collection<Object> getExportTriggerFields()throws NEDSSSystemException{
		...
		//  logic to check if code has separate table
		if (NEDSSConstants.ORACLE_ID.equalsIgnoreCase(propertyUtil.getDatabaseServerType())){
			sqlQuery = WumSqlQuery.SELECT_EXPORT_TRIGGERFIELDS_SQL;
		}
		else{
	...
	public Map<Object, Object> updateTriggerCode(TriggerCodesDT dt) throws NEDSSDAOSysException, NEDSSSystemException {
		...
		if( isError!=true){
			try {
				...
				if (NEDSSConstants.ORACLE_ID.equalsIgnoreCase(propertyUtil.getDatabaseServerType())){
					sqlQuery = WumSqlQuery.UPDATE_TRIGGER_CODE_ORA;
				}else
	...
	public void getConditionFromTriggerCode(Coded coded)throws NEDSSSystemException {
		...
		String whereClause = "where code_system_cd = '" + coded.getCodeSystemCd()+"' AND code ='"+ coded.getCode() +"'";
		try {
			dbConnection = getConnection();
		} catch (NEDSSSystemException nsex) {
			logger.fatal("SQLException while obtaining database connection "
		...
		//  logic to check if code has separate table
		if (NEDSSConstants.ORACLE_ID.equalsIgnoreCase(propertyUtil.getDatabaseServerType())){
			sqlQuery = WumSqlQuery.SELECT_ALLTRIGGER_CODE_ORA + whereClause ;
		}
		else{
		...
		try {
			...
			if(resultSet!=null){
				...
				logger.debug("returned triggercode list");
				 Iterator<Object>  it  = triggerCodesList.iterator();
					while (it.hasNext()) {
...
//			return triggerCodesList;
	...
	public int getMaxNbsUid(TriggerCodesDT dt){
		...
		if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
			codeSql = "select max(nbs_uid) from NBS_SRTE.code_to_condition";
...
public String getCodeSystemCdFromCodeValueGeneral(TriggerCodesDT dt, String code)throws NEDSSDAOSysException, NEDSSSystemException
{
	...
	ArrayList<Object> arrayCodeValueGeneral = new ArrayList<Object>();
	String codeSql = "";
	ArrayList<Object>  paramList = new ArrayList<Object> ();
	...
	if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
		codeSql = RETRIEVE_CODE_DESC_FROM_CODE_VALUE_GENERAL_ORA;
	try {			
	...
	NEDSSSystemException
	{
		...
		ResultSet resultSet = null;
		String nndInd = null;
		String codeSql ="SELECT NND_IND from "+NEDSSConstants.SYSTEM_REFERENCE_TABLE+"..condition_code where condition_cd = ?";
		if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
			codeSql = "SELECT NND_IND from "+NEDSSConstants.SYSTEM_REFERENCE_TABLE+".condition_code where condition_cd = ?";
		try
		{
			dbConnection = getConnection();
			preparedStmt = dbConnection.prepareStatement(codeSql);
