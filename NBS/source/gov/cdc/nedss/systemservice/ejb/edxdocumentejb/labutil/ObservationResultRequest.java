...
public class ObservationResultRequest {
	...
	public static LabResultProxyVO getObservationResultRequest(HL7OBSERVATIONType[] observationRequestArray,  LabResultProxyVO labResultProxyVO, EdxLabInformationDT edxLabInformationDT) throws NEDSSAppException{
	...
	private static ObservationVO getObsReqNotes(HL7NTEType[] noteArray, ObservationVO observationVO) throws NEDSSAppException{
		try {
	...
	private static ObservationVO getObservationResult(HL7OBXType hl7OBXType, LabResultProxyVO labResultProxyVO, EdxLabInformationDT edxLabInformationDT) throws NEDSSAppException {
		...
		try {
			...
			Collection<Object> actIdDTColl =  new ArrayList<Object>();
			 ...
			 if(obsIdentifier!=null){     
				...
				if(observationDT.getAltCd()!=null || observationDT.getAltCdDescTxt()!=null){
				 ...
				 if( observationDT.getCdSystemCd()!=null && observationDT.getCdSystemCd().equals(EdxELRConstants.ELR_LOINC_CD)){
			...
			if(hl7OBXType.getAbnormalFlagsArray()!=null){
			...
			if(hl7OBXType.getObservationResultStatus()!=null){
				ELRXRefDAOImpl dao = new ELRXRefDAOImpl();
				String toCode = dao.findToCode("ELR_LCA_STATUS", hl7OBXType.getObservationResultStatus(), "ACT_OBJ_ST");
			...
			if(hl7XONTypeName!=null){
				OrganizationVO producerOrg =getPerformingFacility(hl7XONTypeName,observationVO.getTheObservationDT().getObservationUid(),labResultProxyVO, edxLabInformationDT);
				//edxLabInformationDT.setNextUid((edxLabInformationDT.getNextUid()-1));
			 ...
			 for(int i=0; i<methodArray.length; i++){
				...
				observationVO.getTheObservationDT().setMethodCd(method.getHL7Identifier());
				observationVO.getTheObservationDT().setMethodDescTxt(method.getHL7Text());
				break;
			}
			 String str=cdv.getDescForCode("OBS_METH",observationVO.getTheObservationDT().getMethodCd());
			if(observationVO.getTheObservationDT().getMethodCd()!=null && (str==null || (str!=null && str.trim().equals("")))){
			
				logger.warn("ObservationResultRequest.getObservationResult warning: Method code could not be teranslated. Please check!!!");
				edxLabInformationDT.setObsMethodTranslated(false);
			}
	...
	private static OrganizationVO getPerformingFacility(HL7XONType performingFacilityType, long observationUid, LabResultProxyVO labResultProxyVO,EdxLabInformationDT edxLabInformationDT) throws NEDSSAppException{
		...
		try {
			...
			entityIdDT.setRootExtensionTxt(performingFacilityType.getHL7OrganizationIdentifier());
			...
			if(performingFacilityType.getHL7AssigningAuthority()!=null){
				entityIdDT.setAssigningAuthorityCd(performingFacilityType.getHL7AssigningAuthority().getHL7UniversalID());
				entityIdDT.setAssigningAuthorityIdType(performingFacilityType.getHL7AssigningAuthority().getHL7UniversalIDType());
			}
			if(performingFacilityType.getHL7AssigningAuthority()!=null && performingFacilityType.getHL7AssigningAuthority().getHL7NamespaceID()!=null && performingFacilityType.getHL7AssigningAuthority().getHL7NamespaceID().equals(EdxELRConstants.ELR_CLIA_CD))
			...
			roleDT.setRoleSeq(new Integer(1));
			...
			organizationNameDT.setNmTxt(performingFacilityType.getHL7OrganizationName());
	...
	private static void formatValue( String text, HL7OBXType hl7OBXType,ObservationVO observationVO,EdxLabInformationDT edxLabInformationDT, String elementName) throws NEDSSAppException{
		String type = "";
		try {
			...
			if(type!=null){
				...
				}else if(type.equals(EdxELRConstants.ELR_STRING_CD)
						|| type.equals(EdxELRConstants.ELR_TEXT_CD)){
					ObsValueTxtDT  obsValueTxtDT= new ObsValueTxtDT();
					StringTokenizer st = new StringTokenizer(text, "^");
					int i=0;
					if(observationVO.getTheObsValueTxtDTCollection()==null)
