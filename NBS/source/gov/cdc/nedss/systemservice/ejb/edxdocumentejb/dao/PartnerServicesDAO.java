 ...
 */
	...
	throws NEDSSDAOSysException, NEDSSSystemException {
		...
		String SELECT_PARTNER_SERVICES_ORA = " and ((Public_health_case.ACTIVITY_FROM_TIME BETWEEN TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS') AND TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS')) OR (PUBLIC_HEALTH_CASE.LAST_CHG_TIME  BETWEEN TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS') AND TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS'))) "
				+ "  order by Public_health_case.ACTIVITY_FROM_TIME";
		...
		if (oracle)
			selectPartnerServices = SELECT_PARTNER_SERVICES_BASE + SELECT_PARTNER_SERVICES_ORA;
		else 
		...
		throws NEDSSDAOSysException, NEDSSSystemException {
			...
			String SELECT_900_BASE = 
			  "Select public_health_case.public_health_case_uid \"publicHealthCaseUid\" , public_health_case.prog_area_cd \"programAreaCd\", "
				...
				boolean oracle = propertyUtil.getDatabaseServerType().equals(NEDSSConstants.ORACLE_ID);
				if (oracle)
					selectPartnerServices = SELECT_900_BASE + SELECT_PARTNER_SERVICES_ORA;
				else 
