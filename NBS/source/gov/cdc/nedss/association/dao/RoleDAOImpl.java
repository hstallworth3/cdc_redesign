...
public class RoleDAOImpl extends BMPBase
{
  ...
  private static final String SELECT_BY_PK = "SELECT max(role_seq) from ROLE where subject_entity_uid = ? and cd = ?";
  private static final String SELECT_BY_SUBJECT_SCOPING_CD = "SELECT max(role_seq) from ROLE where subject_entity_uid = ? and cd = ? and scoping_entity_uid=?";
...
*/
   ...
   public int loadCountBySubjectCdComb(RoleDT roleDT) throws NEDSSDAOSysException, NEDSSSystemException
   {
        //PreparedStatement preparedStmt = getPreparedStatement(sqlStatement);
        ResultSet resultSetCode = null;
        RoleDT dt = new RoleDT();
   ...
   try
    ...
    //  This code is for accessing a stored procedure.
      ...
      Integer roleSequence1 = new Integer(role_Sequence1);
      ...
      Integer roleSequence2 = new Integer(role_Sequence2);
          ...
          while(iter.hasNext())
          {
              ...
              if(roleDT1.isItDirty()==true)
              {
                  ...
                  else
                  {
                      ...
                      Integer role_seq = roleDT1.getRoleSeq();
