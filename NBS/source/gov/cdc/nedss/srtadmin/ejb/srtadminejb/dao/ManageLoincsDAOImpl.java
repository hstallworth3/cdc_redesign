...
public class ManageLoincsDAOImpl extends DAOBase{
	...
	private static final String UPDATE_LOINC_SQL = "UPDATE "
		...
		+ " WHERE loinc_cd=?";
	private static final String UPDATE_LOINC_ORA = "UPDATE "
		+ NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".loinc_code"
		//+ " SET	seq_num=?, "
		+ " SET component_name=?,"
		+ " property=?, " + "time_aspect=?,"
	...
	public static final String CREATE_LOINC_ORA="INSERT INTO "
		+ NEDSSConstants.SYSTEM_REFERENCE_TABLE
		+ ".loinc_code(loinc_cd, component_name,"
		+ " property, time_aspect,"
		+ " system_cd, scale_type,"
...
public void createLoincs(LoincDT loincDT){
	...
	String codeSql = CREATE_LOINC_SQL;
	 if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID)) {
			codeSql = CREATE_LOINC_ORA;
        	String st =  "upper(LOINC_CD) = upper('" + loincDT.getLoincCd() + "')";
        	SRTAdminUtilDAOImpl.getInstance().checkDuplicate("Loinc_Code", st);     			
	 }
