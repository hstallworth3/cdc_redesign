...
public class CodeValueGeneralDAOImpl extends DAOBase{
	...
	private static final String GET_EXISTING_CODE_SET_ORA = "SELECT code_set_nm \"codeSetNm\" "+
    	"FROM "+ NEDSSConstants.SYSTEM_REFERENCE_TABLE +
    	".code_value_general" ;
	...
	private static final String RETRIEVE_CODE_SET_VAL_GEN_FIELDS_ORA="SELECT "
		+" code_set_nm \"codeSetNm\","
		+" code \"code\","
		+" code_desc_txt \"codeDescTxt\","
		+" code_short_desc_txt \"codeShortDescTxt\","
	...
	private static final String UPDATE_CODE_VAL_GEN_SQL = "UPDATE "
		...
		+ " WHERE code=?"
		+ " and code_set_nm=?";
	private static final String UPDATE_CODE_VAL_GEN_ORA = "UPDATE "
		+ NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".code_value_general"
		+ " SET code_desc_txt=?,"
		+ " code_short_desc_txt=?, "
		+ "code_system_cd=?,"
	...
	public static final String CREATE_CODE_VALUE_GENERAL_CODE_ORA="INSERT INTO "
		+ NEDSSConstants.SYSTEM_REFERENCE_TABLE
		+ ".Code_value_general(code_set_nm, code, "
		+ " code_desc_txt, code_short_desc_txt,"
	    + " code_system_cd, code_system_desc_txt, "
	...
	private static final String TEMP_CVG_DUPL_CODE_ORA = "select count(*) from "+ NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".code_value_general where upper(CODE_SET_NM) = upper(?) AND upper(code) = upper(?) ";
	...
	private static final String CREATE_CODESET_ORA = "INSERT INTO "+ NEDSSConstants.SYSTEM_REFERENCE_TABLE
		+ ".Codeset(code_set_nm, assigning_authority_cd, assigning_authority_desc_txt, code_set_desc_txt, code_system_cd, code_system_desc_txt, class_cd, effective_from_time, effective_to_time, is_modifiable_ind, nbs_uid, source_version_txt, source_domain_nm, status_cd, status_to_time,code_set_group_id" 
		+ "admin_comments, value_set_nm, ldf_picklist_ind_cd, value_set_code, value_set_type_cd,	value_set_oid,	value_set_status_cd, value_set_status_time,	parent_is_cd,add_time,	add_user_id) "
		+ " VALUES(?,?,?,?,?,?,?,'code_value_general',?,?,'Y','',NULL,NULL,'A',?,?,?,?,?,?,?,?,?,?,?,?,?) ";
	
	private static final String SELECT_CODE_SET_GROUP_ID_SQL = "SELECT MAX(code_set_group_id) FROM "+ NEDSSConstants.SYSTEM_REFERENCE_TABLE + "..CODESET where CODE_SET_NM=? ";
	private static final String SELECT_CODE_SET_GROUP_ID_ORA = "select MAX(code_set_group_id) FROM "+ NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".CODESET where CODE_SET_NM=? ";
	
	
	private static final String GET_CODE_DESC_TXT_SQL = "SELECT code_set_desc_txt FROM "+ NEDSSConstants.SYSTEM_REFERENCE_TABLE + "..CODESET where CODE_SET_NM=? ";
	private static final String GET_CODE_DESC_TXT_ORA = "select code_set_desc_txt FROM "+ NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".CODESET where CODE_SET_NM=? ";
	...
	private static final String UPDATE_CODE_VAL_GEN_OTH_CODE_ORA = "UPDATE "
		+ NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".code_value_general"
		+ " SET code=?"
		+ " WHERE concept_code=? and code_set_nm=?";
...
public Collection<Object> retrieveCodeSetValGenFields( String searchParams) throws NEDSSDAOSysException, NEDSSSystemException {
        ...
        if (NEDSSConstants.ORACLE_ID.equalsIgnoreCase(propertyUtil.getDatabaseServerType()))
			codeSql = RETRIEVE_CODE_SET_VAL_GEN_FIELDS_ORA;
...
public void updateCodeValueGeneral(CodeValueGeneralDT codeValGenDT) throws NEDSSDAOSysException, NEDSSSystemException {
    ...
    if (NEDSSConstants.ORACLE_ID.equalsIgnoreCase(propertyUtil.getDatabaseServerType()))
		codeSql = UPDATE_CODE_VAL_GEN_ORA;
...
public void insertCodeValueGeneral(CodeValueGeneralDT codeValGenDT) {
	...
	String codeSql = CREATE_CODE_VALUE_GENERAL_CODE_SQL;
	 if (NEDSSConstants.ORACLE_ID.equalsIgnoreCase(propertyUtil.getDatabaseServerType()))
			codeSql = CREATE_CODE_VALUE_GENERAL_CODE_ORA;
...
public void UpdateCodeValueGeneralForOth(CodeValueGeneralDT codeValGenDT)
{
    ...
    if (NEDSSConstants.ORACLE_ID.equalsIgnoreCase(propertyUtil.getDatabaseServerType()))
		codeSql = UPDATE_CODE_VAL_GEN_OTH_CODE_ORA;
...
private Integer duplicateCodesetNm(CodeValueGeneralDT codeValGenDT) {
	...
	if (NEDSSConstants.ORACLE_ID.equalsIgnoreCase(propertyUtil.getDatabaseServerType()))
		codeSql = TEMP_CVG_DUPL_CODE_ORA;
	try {			
		paramList.add(codeValGenDT.getCodeSetNm());
		paramList.add(codeValGenDT.getCode());
