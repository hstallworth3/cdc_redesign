...
public class SnomedCodeDAOImpl extends DAOBase{
	...
	private static final String FIND_EXISTING_SNOMED_ORA = "SELECT snomed_cd\"snomedCd\""
		+",snomed_desc_txt \"snomedDescTx\" "
		+", source_concept_id\"sourceConceptId\""
		+",source_version_id\"sourceVersionId\""
		+",pa_derivation_exclude_cd\"paDerivationExcludeCd\""
	...
	private static final String UPDATE_EXISTING_SNOMED_ORA = "UPDATE "
		+ NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".Snomed_code "
		+" SET snomed_desc_txt = ?," + "source_concept_id = ?,"
		+ " source_version_id = ? "
		+ ", pa_derivation_exclude_cd = ? "
	...
	private static final String CREATE_SNOMED_CODE_ORA= "INSERT INTO "
		+  NEDSSConstants.SYSTEM_REFERENCE_TABLE
	    + ".Snomed_code(	snomed_cd , "
	    + " snomed_desc_txt , "
	    + " source_concept_id , "
...
public Collection<Object> getSnomedDTCollection(String whereClause) throws NEDSSDAOSysException, NEDSSSystemException {
        ...
        if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
			codeSql = FIND_EXISTING_SNOMED_ORA;
...
public void updateSnomedCode(SnomedDT snomedDt)
{
	String codeSql=UPDATE_EXISTING_SNOMED_SQL;
	 ...
	 if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
			codeSql = UPDATE_EXISTING_SNOMED_ORA;
...
public void CreateSnomedCode( SnomedDT snomedDt)
{
	...
	if(propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID)) {
		codeSql=CREATE_SNOMED_CODE_ORA;
    	String st =  "upper(SNOMED_CD) = upper('" + snomedDt.getSnomedCd() + "')";
    	SRTAdminUtilDAOImpl.getInstance().checkDuplicate("Snomed_Code", st);   		
	}
