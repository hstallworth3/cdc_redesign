			...
			for (int i = 1; i <= columns; i++) {
				...
				if (columnValue != null) {
																				  ...
																				  // only
					{
						...
						columnValueClass = Class.forName("java.lang.String");
																				  ...
																				  // only
					{
						...
						java.sql.Blob blob = (java.sql.Blob) columnValue;
						BufferedReader is = new BufferedReader(
					...
					if (parmMethodMap == null) {
						...
						if (columnValueClass.getName().equalsIgnoreCase(
							...
							&& propertyUtil.getDatabaseServerType() != null
							&& (!propertyUtil.getDatabaseServerType()
						.equalsIgnoreCase(NEDSSConstants.ORACLE_ID))) //For SQL Server
						{
			...
			if (methodMap == null) {
				...
				for (int i = 0; i < methods.length; i++) {
					...
					if (methodName.startsWith("set") && parmTypes.length == 1) {
						...
						/*
						 * For Orcale Long, Integer and Double all gives
						 * BigDecimal
						 */
						if (parmTypes[0].equals(Long.class)
							...
							parmMethodMap.put(java.math.BigDecimal.class,
									method);
