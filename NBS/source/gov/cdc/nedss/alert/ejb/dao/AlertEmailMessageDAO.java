...
public class AlertEmailMessageDAO extends DAOBase {
	static final LogUtils logger = new LogUtils(AlertEmailMessageDAO.class
			.getName());
	...
	public Collection<Object>  getAlertConditionList(LabResultProxyVO labResultProxyVO, NBSSecurityObj securityObj){
		...
		if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
		{
			...
			dbType="nbs_srte..";
			 ...
			 String SELECT_CONDITION_CODE_SQL = "select condition_cd from " +dbType+ "loinc_condition where loinc_cd='";
			 String SNOMED_CONDITION_SQL="SELECT CONDITION_CD FROM " +dbType+ "SNOMED_CONDITION WHERE SNOMED_CD='";
			 String MAPPING_CONDITION_SQL="select condition_cd from " +dbType+ "labtest_progarea_mapping where lab_test_cd='";
			 String LAB_RESULT_SQL="select default_condition_cd  from " +dbType+ "lab_result where lab_result_cd='";
			 String NONSNOMED_CONDITION_SQL=" select condition_cd  from " +dbType+ "lab_result_snomed sn,  " +dbType+ "snomed_condition cond where "
