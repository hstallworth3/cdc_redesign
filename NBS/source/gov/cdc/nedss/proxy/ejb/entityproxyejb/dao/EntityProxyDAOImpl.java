    ...
    */
     ...
     */
     private String getCode(EntityLocatorParticipationDT entityLocatorParticipationDT) throws Exception
     {
        ...
        String code = null;
        String aQuery = null;
        if (PropertyUtil.getInstance().getDatabaseServerType() != null && PropertyUtil.getInstance().getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
        {
            aQuery = "SELECT state_nm FROM " +
                        NEDSSConstants.SYSTEM_REFERENCE_TABLE +
                        ".State_code WHERE state_cd = '" + stateCd + "'";
        }
        else
        {
            aQuery = "SELECT state_nm FROM " +
        ...
        try
        {
            ...
            if(stateNm != null)
            {
              if(cityDescTxt != null)
              {
                ...
                if (PropertyUtil.getInstance().getDatabaseServerType() != null && PropertyUtil.getInstance().getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
                {
                    aQuery = "SELECT code FROM " + NEDSSConstants.SYSTEM_REFERENCE_TABLE +
                     ".City_code_value WHERE upper(code_desc_txt )= ('" + codeDescTxt + "')";
                }
                else
                {
                    aQuery = "SELECT code FROM " + NEDSSConstants.SYSTEM_REFERENCE_TABLE +
