...
public class CTContactSummaryDAO  extends DAOBase  {
	...
	private final String SELECT_MPRPAT_NAMED_BY_CONTACT_COLLECTION_FOR_MANAGE1 = "select subjectPHC.cd \"subjectPhcCd\","
		...
		+" from ct_contact "
        +" left outer join public_health_case contactPHC on ct_contact.CONTACT_ENTITY_PHC_UID=contactPHC.PUBLIC_HEALTH_CASE_UID" 
        +" inner join public_health_case currentPHC on currentPHC.PUBLIC_HEALTH_CASE_UID = ?"
        +" inner join public_health_case subjectPHC on ( ct_contact.SUBJECT_ENTITY_PHC_UID=subjectPHC.PUBLIC_HEALTH_CASE_UID ";
        private final String SELECT_MPRPAT_NAMED_BY_CONTACT_COLLECTION_FOR_MANAGE3 = " ) inner join person contactPerson on ct_contact.CONTACT_ENTITY_UID=contactPerson.person_uid"
        +" inner join person subjectPerson on CT_CONTACT.SUBJECT_ENTITY_UID=subjectPerson.person_uid where ct_contact.record_status_cd='ACTIVE' and contactPerson.person_parent_uid=? and (contact_entity_phc_uid is null or contact_entity_phc_uid = ?) and subjectPHC.cd = currentPHC.cd ";        
	...
	private final String SELECT_MPRPAT_NAMED_BY_CONTACT_COLLECTION="select DISTINCT contact.local_id \"contactPhcLocalId\", subject.cd \"subjectPhcCd\","
		...
		+" subjectCM.FLD_FOLL_UP_DISPO \"sourceDispositionCd\", subjectCM.PAT_INTV_STATUS_CD \"sourceInterviewStatusCd\","		
		...
		+" from ct_contact "
		+" left outer join public_health_case subject on ct_contact.SUBJECT_ENTITY_PHC_UID=subject.PUBLIC_HEALTH_CASE_UID "
		+" left outer join case_management subjectCM on ct_contact.SUBJECT_ENTITY_PHC_UID=subjectCM.PUBLIC_HEALTH_CASE_UID "
		+" left outer join public_health_case contact on ct_contact.CONTACT_ENTITY_PHC_UID=contact.PUBLIC_HEALTH_CASE_UID "
		+" inner join person contactpat on ct_contact.CONTACT_ENTITY_UID=contactpat.person_uid "
		+" inner join person subjectrevision on ct_contact.subject_entity_uid=subjectrevision.person_uid "
		+" inner join person contactrevision on contactrevision.person_parent_uid=contactpat.person_parent_uid where   ct_contact.record_status_cd='ACTIVE' and contactpat.person_parent_uid=";
	...
	private final String SELECT_MPRPAT_OTHER_NAMED_BY_CONTACT_COLLECTION="select DISTINCT contact.local_id \"contactPhcLocalId\", subject.cd \"subjectPhcCd\","
			...
			+" left outer join public_health_case subject on ct_contact.SUBJECT_ENTITY_PHC_UID=subject.PUBLIC_HEALTH_CASE_UID "
			+" left outer join case_management subjectCM on ct_contact.SUBJECT_ENTITY_PHC_UID=subjectCM.PUBLIC_HEALTH_CASE_UID "
			+" left outer join public_health_case contact on ct_contact.third_party_entity_phc_uid=contact.PUBLIC_HEALTH_CASE_UID "
			+" inner join person otherpat on ct_contact.THIRD_PARTY_ENTITY_UID=otherpat.person_uid "
			+" inner join person subjectrevision on ct_contact.subject_entity_uid=subjectrevision.person_uid "
			+" inner join person contactrevision on contactrevision.person_parent_uid=otherpat.person_parent_uid where   ct_contact.record_status_cd='ACTIVE' and otherpat.person_parent_uid=";
	...
	private final String SELECT_MPR_CONTACT_NAMED_BY_PATIENT_COLLECTION1 ="select subject.cd \"subjectPhcCd\","
        ...
        +" from ct_contact "
        +" left outer join public_health_case subject on ct_contact.SUBJECT_ENTITY_PHC_UID=subject.PUBLIC_HEALTH_CASE_UID "
        +" left outer join interview ixs on ct_contact.named_during_interview_uid=ixs.interview_uid "
        +" left outer join case_management subjectCM on ct_contact.SUBJECT_ENTITY_PHC_UID=subjectCM.PUBLIC_HEALTH_CASE_UID "
        +" left outer join public_health_case contact on (ct_contact.CONTACT_ENTITY_PHC_UID=contact.PUBLIC_HEALTH_CASE_UID ";
        
	private final String SELECT_MPR_CONTACT_NAMED_BY_PATIENT_COLLECTION3 = " )inner join person on ct_contact.CONTACT_ENTITY_UID=person.person_uid "
        +" inner join person contactrevision on contactrevision.person_uid= ct_contact.CONTACT_ENTITY_UID "
        +" inner join person subjectpat on CT_CONTACT.SUBJECT_ENTITY_UID=subjectpat.person_uid "
	...
	+" from ct_contact " 
	+"  left outer join public_health_case subject on (ct_contact.SUBJECT_ENTITY_PHC_UID=subject.public_health_case_uid) " 
	+"  left outer join case_management cm on (ct_contact.CONTACT_ENTITY_PHC_UID=cm.public_health_case_uid) "
	+"  left outer join case_management subjectCM on (ct_contact.SUBJECT_ENTITY_PHC_UID=subjectCM.public_health_case_uid) "
	+"  left outer join interview ixs on ct_contact.named_during_interview_uid=ixs.interview_uid "
	+" 	inner join person on ct_contact.CONTACT_ENTITY_UID=person.person_uid "
	+" 	inner join person subjectperson on ct_contact.SUBJECT_ENTITY_UID=subjectperson.person_uid "
	...
	private final String SELECT_PHCPAT_OTHER_NAMED_BY_CONTACT_COLLECTION="select ct_contact.named_On_Date \"namedOnDate\", ct_contact.CT_Contact_uid \"ctContactUid\", ct_contact.local_Id \"localId\", "
			...
			+" from ct_contact " 
			+"  left outer join public_health_case subject on (ct_contact.SUBJECT_ENTITY_PHC_UID=subject.public_health_case_uid) " 
			+"  left outer join case_management cm on (ct_contact.THIRD_PARTY_ENTITY_PHC_UID=cm.public_health_case_uid) "
			+"  left outer join case_management subjectCM on (ct_contact.SUBJECT_ENTITY_PHC_UID=subjectCM.public_health_case_uid) "
			+"  left outer join interview ixs on ct_contact.named_during_interview_uid=ixs.interview_uid "
			+" 	inner join person on ct_contact.THIRD_PARTY_ENTITY_UID=person.person_uid "
			+" 	inner join person subjectperson on ct_contact.SUBJECT_ENTITY_UID=subjectperson.person_uid "
	...
	private final String SELECT_PHCPAT_NAMED_BY_PATIENT_COLLECTION1="select subject.cd \"subjectPhcCd\", " 
		...
		+"  left outer join interview ixs on ct_contact.named_during_interview_uid=ixs.interview_uid "
		+" 	left outer join public_health_case contact on (ct_contact.CONTACT_ENTITY_PHC_UID=contact.PUBLIC_HEALTH_CASE_UID ";
		private final String SELECT_PHCPAT_NAMED_BY_PATIENT_COLLECTION3= " ) inner join person on ct_contact.SUBJECT_ENTITY_UID=person.person_uid  " 
		+" inner join person personcontact on ct_contact.CONTACT_ENTITY_UID=personcontact.person_uid "
