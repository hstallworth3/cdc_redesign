	...
	public boolean publishPage(String invFormCd) throws IOException, InterruptedException {
		...
		if(nbsDirectoryPath.exists()) {
			...
			if(nbsTmpDeploy.length() > 0) {
				nbsDirectoryPath = new File(tmpDir + File.separator + "vfs" + File.separator + "deployment" + File.separator + nbsTmpDeploy + File.separator + nbsTmpExpWAR);
				...
				// Source Dir 
				if(invFormCd.equals(""))
					srcDir = new File(System.getProperty("nbs.dir") + SOURCE_FOLDER_IN_WAR + File.separator ) ;
				else{
				    srcDir = new File(System.getProperty("nbs.dir") + SOURCE_FOLDER_IN_WAR + File.separator + invFormCd + File.separator) ;
				    zipFile =  new File(System.getProperty("nbs.dir") + SOURCE_FOLDER_IN_WAR + File.separator +invFormCd + File.separator + invFormCd+".zip") ;
				}
				    logger.info("Source Directory for page publish :"+srcDir);
				// Destination Dir
				if(invFormCd.equals(""))
					destDir = new File(nbsDirectoryPath + TARGET_FOLDER_IN_WAR + File.separator );
				else
				   destDir = new File(nbsDirectoryPath + TARGET_FOLDER_IN_WAR + File.separator + invFormCd + File.separator); 	
				logger.info("Destination Directory for page publish :"+destDir);
				//if(!destDir.exists())
				//	destDir.mkdir();
				//	_copyProcess(srcDir, destDir);
			    ...
			    while(it.hasNext()){
			    	...
			    	srcDir = new File(System.getProperty("nbs.dir") + SOURCE_FOLDER_IN_WAR + File.separator + nbsPageDT1.getFormCd() + File.separator) ;
