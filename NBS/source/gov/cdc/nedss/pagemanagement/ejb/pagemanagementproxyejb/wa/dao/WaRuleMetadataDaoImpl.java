...
public class WaRuleMetadataDaoImpl extends BMPBase{
	...
	throws NEDSSSystemException{
		...
		String INSERT_RULE_METATADATA_ORACLE="INSERT INTO WA_rule_metadata(wa_template_uid,rule_cd,rule_expression,err_msg_txt,source_question_identifier," +
				...
				"javascript_function, javascript_function_nm, source_values,logic, user_rule_id,target_type) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,EMPTY_CLOB(),?,?,?,?,?)";
		...
		try
		{
			...
			preparedStmt = dbConnection.prepareStatement(INSERT_RULE_METATADATA_ORACLE);
			...
			//preparedStmt.setString(i++, "EMPTY_CLOB()");//14
		...
		{
			logger.fatal("Error while inserting into Rule_meta_data TABLE, exception = " + ex);
			logger.fatal("waRuleMetadataDT DT is "+waRuleMetadataDT.toString());
			logger.fatal("waRuleMetadataDT resultCount is "+resultCount);
			ex.printStackTrace();
			throw new NEDSSSystemException(ex.toString());
		...
		try{
			...
			String sqll="select javascript_function from WA_rule_metadata where WA_rule_metadata_uid= "+waRuleMetadataDT.getWaRuleMetadataUid() +" for update nowait"; 
			rss=statement.executeQuery(sqll);
			if(rss.next()){				
				java.sql.Clob clob= (java.sql.Clob)rss.getClob("javascript_function");
				clob.setString(1, waRuleMetadataDT.getJavascriptFunction());
				rss.close();
			}
	...
	throws NEDSSSystemException{
		...
		if(waRuleMetadataDT.getRuleCd().equalsIgnoreCase("Date Compare")){
			...
			String UPDATE_RULE_METATADATA_ORACLE="UPDATE WA_rule_metadata set wa_template_uid=?, rule_cd=?, rule_expression=?, err_msg_txt=?, source_question_identifier=?, " +
			...
			if (propUtil.getDatabaseServerType() != null &&
				...
				{
					dbConnection = getConnection();
					preparedStmt = dbConnection.prepareStatement(UPDATE_RULE_METATADATA_ORACLE);
				...
				try{
					...
					String sqll="select javascript_function from WA_rule_metadata where WA_rule_metadata_uid= "+waRuleMetadataDT.getWaRuleMetadataUid() +" for update nowait";
					preparedStmt = dbConnection.prepareStatement(sqll);
					rss=preparedStmt.executeQuery();
					if(rss.next()){				
						java.sql.Clob clob= (java.sql.Clob)rss.getClob("javascript_function");
						clob.setString(1, waRuleMetadataDT.getJavascriptFunction());
						rss.close();
	...
	public Collection<Object>  getUiMetatdataElementsDropDown(Long templateUid, String type) throws NEDSSSystemException {
		...
		if(type.equalsIgnoreCase("DATE")){
			whereclause= " and data_type in ('DATE', 'DATETIME') and standard_nnd_ind_cd='F' and display_ind='T' and nbs_UI_component_uid in (1001,1006,1007,1008,1009,1013,1017)";
		}
			...
			whereclause= " and data_type in ('CODED') and standard_nnd_ind_cd='F' and nbs_UI_component_uid in (1001,1006,1007,1008,1009,1013,1017)";
		}
		else{
			//whereclause= " and standard_nnd_ind_cd='F' and display_ind='T' and nbs_UI_component_uid in (1001,1006,1007,1008,1009,1013,1017)";
			whereclause= " and standard_nnd_ind_cd='F' and nbs_UI_component_uid in (1001,1006,1007,1008,1009,1013,1017)";
		}  
		...
		{
			sqlQuery ="select distinct wa_ui_metadata.code_set_group_id \"codeSetGroupId\", order_nbr \"orderNbr\", local_id \"localId\", question_identifier \"questionIdentifier\", nbs_UI_component_uid  \"nbsUIComponentUid\", question_label \"questionLabel\", " +
			" question_group_seq_nbr \"questionGroupSeqNbr\" from wa_ui_metadata " + dbServer + " where wa_template_uid= ? and (question_identifier = 'INV169' or (upper(cs.class_cd) = upper('code_value_general') and wa_ui_metadata.code_set_group_id = cs.code_set_group_id and display_ind='T')) " +whereclause + " order by  question_label";
		}
		else{
			sqlQuery ="select wa_ui_metadata.code_set_group_id \"codeSetGroupId\", order_nbr \"orderNbr\", local_id \"localId\", question_identifier \"questionIdentifier\", nbs_UI_component_uid \"nbsUIComponentUid\", question_label \"questionLabel\", " +
				" question_group_seq_nbr \"questionGroupSeqNbr\" from wa_ui_metadata  where wa_template_uid= ? " +whereclause + " order by  question_label";
		}			
	...
	public Collection<Object>  getUiMetaQuestionDropDown(Long templateUid, String source, String mode, String ruleId, String ruleCode) throws NEDSSSystemException {
		...
		if (ruleCode != null && ruleCode.equalsIgnoreCase("Require If"))
			additionalPredicate = " and required_ind = 'F' ";
		...
		if(flag){
		   sqlQuery="select order_nbr \"orderNbr\", local_id \"localId\", question_identifier \"questionIdentifier\"," +
			...
			sqlQuery="select order_nbr \"orderNbr\", local_id \"localId\", question_identifier \"questionIdentifier\"," +
			        " nbs_UI_component_uid  \"nbsUIComponentUid\", question_label \"questionLabel\"  from wa_ui_metadata  " +					 
					" where wa_template_uid="+templateUid +" and question_identifier not in ('"+ source +"') "+
					//" and nbs_ui_component_uid not in (1016)" +
					" and standard_nnd_ind_cd='F' and display_ind='T' " + 
					" and nbs_UI_component_uid in (1001,1006,1007,1008,1009,1013,1017)" +
	...
	public Long  maxRuleId ( String tableName) throws NEDSSSystemException
	{
		...
		try
		{
			...
			String databaseType = PropertyUtil.getInstance().getDatabaseServerType();
			 if(databaseType != null && databaseType.equalsIgnoreCase(NEDSSConstants.ORACLE_ID)){
				 sql="select  "+tableName+"_uid from "+tableName+" where ROWNUM =1 order by "+tableName+"_uid desc";
			 }
			  else
			 {
	...
	public void insertWaRulesHist(WaRuleMetadataDT waRuleMetadataDT) throws NEDSSSystemException
	{
		try{
			 String databaseType = PropertyUtil.getInstance().getDatabaseServerType();
			 if(databaseType != null && databaseType.equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
				 insertWaRulesHistOracle(waRuleMetadataDT);
			 else
	...
	public void insertWaRulesHistOracle(WaRuleMetadataDT waRuleMetadataDT) throws NEDSSSystemException
	{
		...
		try
		{
			String INSERT_RULE_METATADATA_HIST_ORACLE="INSERT INTO WA_rule_metadata_hist(wa_rule_metadata_uid,wa_template_uid,rule_cd,rule_expression,err_msg_txt,source_question_identifier," +
			"target_question_identifier ,record_status_cd,record_status_time,add_time,add_user_id,last_chg_time,last_chg_user_id,rule_desc_txt, " +
			"javascript_function, javascript_function_nm, user_rule_id,logic,source_values,target_type) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,EMPTY_CLOB(),?,?,?,?,?)";
			dbConnection = getConnection();
			preparedStmt = dbConnection.prepareStatement(INSERT_RULE_METATADATA_HIST_ORACLE);
			int i = 1;
			preparedStmt.setLong(i++, waRuleMetadataDT.getWaRuleMetadataUid().longValue()); //0
			if(waRuleMetadataDT.getWaTemplateUid()==null)
				preparedStmt.setNull(i++, Types.INTEGER);
		...
		{
			logger.fatal("SQLException while inserting " +
					"a new entry in Rule_metadata_hist table: \n"+sqlex.getMessage(), sqlex);
			logger.fatal("waRuleMetadataDT DT is "+waRuleMetadataDT.toString());
			logger.fatal("waRuleMetadataDT resultCount is "+resultCount);
			throw new NEDSSDAOSysException( sqlex.toString() );
		}
		...
		{
			logger.fatal("Error while inserting into Rule_metadata_hist TABLE, exception = " + ex.getMessage(), ex);
			logger.fatal("waRuleMetadataDT DT is "+waRuleMetadataDT.toString());
			logger.fatal("waRuleMetadataDT resultCount is "+resultCount);
			throw new NEDSSSystemException(ex.toString());
		}
		...
		try{
			//get another connection to update the clob javascript function 
			dbConnection = getConnection(); 
			
			String sqll="select javascript_function from WA_rule_metadata_hist where WA_rule_metadata_uid= "+waRuleMetadataDT.getWaRuleMetadataUid() +" for update nowait";
			preparedStmt = dbConnection.prepareStatement(sqll);
			rss=preparedStmt.executeQuery();
			if(rss.next()){				
				java.sql.Clob clob= (java.sql.Clob)rss.getClob("javascript_function");
		...
		{
			logger.fatal("SQLException while inserting " +
					"a new entry in Rule_meta_data_hist table: \n"+sqlex.getMessage(), sqlex);
			logger.fatal("waRuleMetadataDT DT is "+waRuleMetadataDT.toString());
			logger.fatal("waRuleMetadataDT resultCount is "+resultCount);
			throw new NEDSSDAOSysException( sqlex.toString() );
		}
		...
		{
			logger.fatal("Error while inserting into rule_metadata_hist TABLE, id = " + ex.getMessage(), ex);
			logger.fatal("waRuleMetadataDT DT is "+waRuleMetadataDT.toString());
			logger.fatal("waRuleMetadataDT resultCount is "+resultCount);
			throw new NEDSSSystemException(ex.toString());
		}
