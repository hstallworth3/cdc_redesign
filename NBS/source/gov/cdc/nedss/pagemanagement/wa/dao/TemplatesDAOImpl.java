...
public class TemplatesDAOImpl extends DAOBase{
	...
	private static final String UPDATE_TEMPLATE_STATUS = "UPDATE WA_template SET record_status_cd=?, "
     ...
     private static final String SELECT_ACTIVITY_LOGS_BY_ALGORITHMNAME_ORACLE = "Select "+ SELECT_ACTIVITY_LOGS_BY_ALGORITHMNAME+ " where ROWNUM <= 1 and edx.doc_type = '"+NEDSSConstants.LAB_11648804+"' or edx.doc_type = '"+NEDSSConstants.PHC_236+"' and imp_exp_ind_cd ='I' "
			    + "and algorithm_name = ? order by record_status_time desc";
	...
	private static final String SELECT_CODE_SET_NM_COLL_SQL = "SELECT  distinct CODE_SET_NM \"codesetNm\",code_set_group_id \"value\" FROM " + NEDSSConstants.SYSTEM_REFERENCE_TABLE + "..CODESET ";
	private static final String SELECT_CODE_SET_NM_COLL_ORA = "SELECT  distinct CODE_SET_NM \"codesetNm\",code_set_group_id \"value\" FROM " + NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".CODESET ";
	 	...
	 	try {
			...
			String databaseType = propertyUtil.getDatabaseServerType();
			 if(databaseType != null && databaseType.equalsIgnoreCase(NEDSSConstants.ORACLE_ID)){
				 selectSqlString = SELECT_ACTIVITY_LOGS_BY_ALGORITHMNAME_ORACLE;
			 }
			  else
			  {
...
public ArrayList<Object> getDsmActivityLogDetailCollection(Long activityUid) throws NEDSSDAOSysException, NEDSSSystemException {
    ...
    catch(Exception ex)
    {
        ...
        logger.fatal("Exception while selecting " +
                        "a activityLog ; uid = " + activityUid.toString(), ex);
  ...
  try{
	...
	try {
		...
		logger.fatal("SQLException while obtaining database connection "
				+ "for fetchInsertSql : TemplatesDAOImpl", nsex);
      ...
      if (propertyUtil.getDatabaseServerType() != null &&
    	  ...
    	  //sqlQuery = "Select * from "+strTableName+ " where "+ colName +" in (" + qIDList +") and wa_template_uid = "+waTemplateUid ; 
      ...
      try {
		...
		throw new NEDSSDAOSysException("SQLException while fetching records "
				+ "fetchInsertSql: TemplatesDAOImpl sql Query is :"+sqlQuery +"Exception is :" +se.getMessage());
	}
 }catch (Exception se) {
	 ...
	 // dt.setEDXActivityLogDTDetails(eDXActivityLogDTDetails);	
	  edxActivityLogDAOImpl.insertExportEDXActivityLog(dt);
		throw new NEDSSDAOSysException("SQLException while fetching records "
				+ "fetchInsertSql: TemplatesDAOImpl " +se.getMessage());
	}         
 finally {
		closeResultSet(rs);				
   ...
   /**
	...
	throws NEDSSDAOSysException,NEDSSSystemException {
		...
		try {
			...
			logger.fatal("SQLException while obtaining database connection "
					+ "for InsertImportSql : TemplatesDAOImpl", nsex);
			throw new NEDSSSystemException(nsex.getMessage());
		}
	  		    ...
	  		    if(i==5){
					...
					Long  waRuleMetadataDTUid = null;
				  		...
				  		for(int len =0;len< insertSql1.length;len++){  
				  			...
				  			if (propertyUtil.getDatabaseServerType() != null &&
							propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID)){
				  			    sql=  JspBeautifier.replace(sql,"ReplaceWithCurrentDate", "sysdate");
				 		  	    sql= JspBeautifier.replace(sql,"'$$JSFUN"+jsFunction+"##JSFUN'", "EMPTY_CLOB()"); 
				  			}
				  			else{
				  			...
				  			try{
								...
								if (propertyUtil.getDatabaseServerType() != null &&
									...
									logger.debug("resultCount in waRuleMetadataDTUid is " + resultCount);
									Statement statement = dbConnection.createStatement();
									String sqll="select javascript_function from WA_rule_metadata where WA_rule_metadata_uid= "+waRuleMetadataDTUid.longValue() +" for update nowait"; 
									ResultSet rss=statement.executeQuery(sqll);
									jsFunction = jsFunction.replaceAll("''","'");
									if(rss.next()){				
										java.sql.Clob clob= (java.sql.Clob)rss.getClob("javascript_function");
										clob.setString(1,jsFunction);
										rss.close();
									}
									preparedStmt2.close();
									statement.close();
									...
									if(i==5){
										if(e.getMessage().contains("duplicate key")){
											   closeStatement(preparedStmt);
											   logger.fatal("unique key constraint while adding following code set to wa_rule_metadata table table "+sql);
											   ...
											   e.printStackTrace(myPrinter);
							                    String stackTraceStr = writerStr.toString();
										   logger.fatal("fatal error occured while inserting following code set to wa_rule_metadata table. rolling back the whole transaction ");
	  		     ...
	  		     if(i==3 || i==4){	
	  		       ...
	  		       for(int len =0;len< insertSql1.length;len++){  
	  		    	 String insertSql = "" ;
		  		       ...
		  		       if (propertyUtil.getDatabaseServerType() != null &&
						   propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
				  		  insertSql=  JspBeautifier.replace(insertSql,"ReplaceWithCurrentDate", "sysdate");
		  		       else
						...
						try{
							...
							if(i==3){
								if(e.getMessage().contains("duplicate key") || e.getMessage().contains("unique constraint")){
									   ...
									   logger.fatal("unique key constraint while adding following code set to wa_nnd_metadata  table "+insertSql);
								       continue;
								   }
								   else	{  
									   e.printStackTrace(myPrinter);
					                    String stackTraceStr = writerStr.toString();
									   dt.setEdxActivityLogUid(activityLogUid);
									   logger.fatal("fatal error occured while inserting following code set to wa_nnd_metadata table. rolling back the whole transaction ");
								      dt.setSourceUid(null);
							    	  dt.setTargetUid(new Long(templateUid));
							    	  dt.setDocType("Template");
							    	  dt.setDocName(templateNm);
							...
							if(i==4){
								if(e.getMessage().contains("duplicate key") || e.getMessage().contains("unique constraint")){											
									   logger.fatal("unique key constraint while adding following code set to wa_rdb_metadata  table "+insertSql);
								       continue;
								   }
								   else	{  
									   e.printStackTrace(myPrinter);
					                    String stackTraceStr = writerStr.toString();
								   logger.fatal("fatal error occured while inserting following code set to wa_rdb_metadata table. rolling back the whole transaction ");
								   dt.setEdxActivityLogUid(activityLogUid);   
								   dt.setSourceUid(null);
							    	  dt.setTargetUid(new Long(templateUid));
							    	  dt.setDocType("Template");
					   ...
					   eDXActivityLogDTDetails.add(ddt);
						...
						if(i==0){
							if(e.getMessage().contains("duplicate key") || e.getMessage().contains("unique constraint") ){
								   ...
								   logger.fatal("unique key constraint while adding following code set to wa_question  table "+insertSql);
								   ddt = new EDXActivityDetailLogDT();
								   ddt.setEdxActivityLogUid(activityLogUid);
								   ddt.setRecordId(pairs.getKey().toString().substring(0, pairs.getKey().toString().indexOf(":QT:")));
								   ddt.setRecordType(pairs.getKey().toString().substring(pairs.getKey().toString().indexOf(":QT:")+4,pairs.getKey().toString().indexOf(":QN:")));
								   ...
								   e.printStackTrace(myPrinter);
				                    String stackTraceStr = writerStr.toString();
							   logger.fatal("fatal error occured while inserting following code set to wa_question table. rolling back the whole transaction ");
							   dt.setEdxActivityLogUid(activityLogUid);   
							   dt.setSourceUid(null);
						...
						if(i==1){
							if(e.getMessage().contains("duplicate key") || e.getMessage().contains("unique constraint")){
								   //closeStatement(preparedStmt);
								   logger.fatal("unique key constraint while adding following code set to wa_template  table "+insertSql);
								   ...
								   e.printStackTrace(myPrinter);
				                    String stackTraceStr = writerStr.toString();
							   logger.fatal("fatal error occured while inserting following code set to wa_template table. rolling back the whole transaction ");
							   dt.setEdxActivityLogUid(activityLogUid);  
							   dt.setSourceUid(null);
						    	  dt.setTargetUid(null);
						    	  dt.setDocType("Template");
						...
						if(i==2){
							...
							if(e.getMessage().contains("duplicate key") || e.getMessage().contains("unique constraint")){
								   logger.fatal("unique key constraint while adding following code set to wa_ui_metadata  table "+insertSql);
								   UiInsert = false;
								   continue;
							   }
							   else	{   
								   e.printStackTrace(myPrinter);
				                    String stackTraceStr = writerStr.toString();
							   logger.fatal("fatal error occured while inserting following code set to wa_ui_metadata table. rolling back the whole transaction ");
							   dt.setEdxActivityLogUid(activityLogUid);   
							   dt.setSourceUid(null);
						    	  dt.setTargetUid(new Long(templateUid));
						    	  dt.setDocType("Template");
	...
	throws NEDSSDAOSysException,NEDSSSystemException {
		...
		try {
			...
			logger.fatal("SQLException while obtaining database connection "
					+ "for InsertVocabSql : TemplatesDAOImpl", nsex);
			throw new NEDSSSystemException(nsex.getMessage());
		}
	    	 ...
	    	 //for(int i=0;i<cdMap.size();i++) {
		    	 Iterator<?> it = ((Map<?, ?>) aList.get(1)).entrySet().iterator();//codeset collection 
					...
					try{
					  	    ...
					  	    while (it1.hasNext()) {
					  		  ...
					  		  if (propertyUtil.getDatabaseServerType() != null &&
						         ...
						         try{
							  	      ...
							  	      //  System.out.println(pairs.getKey() + " = " + pairs.getValue());
								  		...
								  		String sql="";
									  		for(int len =0;len< insertSql1.length;len++){  
									  			 if (propertyUtil.getDatabaseServerType() != null &&
														   propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
									  			sql=  JspBeautifier.replace(insertSql1[len],"ReplaceWithCurrentDate", "sysdate");	
									  			 else
											   ...
											   try{
												   ...
												   closeStatement(preparedStmt);
												   if(e.getMessage().contains("duplicate key") || e.getMessage().contains("unique constraint")){
													   logger.fatal("unique key constraint while adding following code set to code_value_general table "+sql);
													   ddt = new EDXActivityDetailLogDT();
													   ddt.setEdxActivityLogUid(activityLogUid);
												   ...
												   logger.fatal("fatal error occured while inserting following code set to code_value_general table. rolling back the whole transaction ");
												      dt.setEdxActivityLogUid(activityLogUid);
											    	  dt.setSourceUid(null);
											    	  dt.setTargetUid(null);
											    	  dt.setDocType("Template");
						        	 ...
						        	 closeStatement(preparedStmt);
									   if(e.getMessage().contains("duplicate key") || e.getMessage().contains("unique constraint")){
										   logger.fatal("unique key constraint while adding following code set to codeset table "+insertSql);
										   ddt = new EDXActivityDetailLogDT();
										   ddt.setEdxActivityLogUid(activityLogUid);
										   ddt.setRecordId(pairs.getKey().toString().substring(0, pairs.getKey().toString().indexOf(":VSN:")));
										   ddt.setRecordName(pairs.getKey().toString().substring(pairs.getKey().toString().indexOf(":VSN:")+5,pairs.getKey().toString().indexOf(":CT:")));
										   ...
										   break;
										     ...
										     e.printStackTrace(myPrinter);
						                    String stackTraceStr = writerStr.toString();
									   logger.fatal("fatal error occured while inserting following code set to codeset_group_metadata table. rolling back the whole transaction ");
									      dt.setEdxActivityLogUid(activityLogUid);
								    	  dt.setSourceUid(null);
								    	  dt.setTargetUid(null);
								    	  dt.setDocType("Template");
						...
						closeStatement(preparedStmt);
						   if(e.getMessage().contains("duplicate key") || e.getMessage().contains("unique constraint")){
							   logger.fatal("unique key constraint while adding following code set to codeset_group_metadata table "+insertSql);
							   ddt = new EDXActivityDetailLogDT();
							   ddt.setEdxActivityLogUid(activityLogUid);
							   ddt.setRecordId(pairs.getKey().toString().substring(0, pairs.getKey().toString().indexOf(":VSN:")));
							   ddt.setRecordName(pairs.getKey().toString().substring(pairs.getKey().toString().indexOf(":VSN:")+5,pairs.getKey().toString().indexOf(":CT:")));
	...
	throws NEDSSDAOSysException,NEDSSSystemException {
		...
		try {
			...
			logger.fatal("SQLException while obtaining database connection "
					+ "for  fetchQIdentifiers4Template  : TemplatesDAOImpl", nsex);
			throw new NEDSSSystemException(nsex.getMessage());
		}
	      //  logic to check if code has seperate table
	      if (propertyUtil.getDatabaseServerType() != null &&
	          propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID)){
	    	  sqlQuery = " Select question_identifier,code_set_group_id from wa_ui_metadata where wa_template_uid = "+templateUid;
	      }
	      else{
	    	  sqlQuery = " Select question_identifier,code_set_group_id from wa_ui_metadata where wa_template_uid = "+templateUid;
	      }
	      try {
			...
			throw new NEDSSDAOSysException("SQLException while selecting "
					+ "fetchQIdentifiers4Template: TemplatesDAOImpl " +se.getMessage());
	...
	public Map<Object,Object> getCodeSetNmColl() throws NEDSSDAOSysException, NEDSSSystemException {
	    ...
	    try
	    {
		    ...
		    if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
				codeSql = SELECT_CODE_SET_NM_COLL_ORA;
	...
	throws NEDSSDAOSysException,NEDSSSystemException {
		...
		try {
	      ...
	      if (propertyUtil.getDatabaseServerType() != null &&
	          propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID)){
	    	  //sqlQuery = " Select question_identifier,code_set_group_id from wa_ui_metadata where wa_template_uid = "+templateUid;
	    	  sqlQuery =  "Select code_set_group_id from wa_question where question_identifier in(select question_identifier from wa_ui_metadata where wa_template_uid = "+templateUid +") minus  select code_set_group_id from wa_ui_metadata where wa_template_uid = "+templateUid;
	    	  
	      }
	      else{
	    	  sqlQuery =  "Select code_set_group_id from wa_question where question_identifier in(select question_identifier from wa_ui_metadata where wa_template_uid = "+templateUid +") EXCEPT  select code_set_group_id from wa_ui_metadata where wa_template_uid = "+templateUid;
	      }
	      try {
			preparedStmt = dbConnection
					.prepareStatement(sqlQuery);
			//preparedStmt.setString(1,templateUid);
			resultSet = preparedStmt.executeQuery();
			...
			.fatal(
					...
					se.getMessage(), se);
			throw new NEDSSDAOSysException("SQLException while selecting "
					+ "fetchAdditonalCodeSets: TemplatesDAOImpl " +se.getMessage());
		} catch (Exception reuex) {
			logger
					.fatal(
							"Error in result set handling while selecting jsp files: TemplatesDAOImpl.",
...
throws NEDSSDAOSysException,NEDSSSystemException {
	...
	try {
      ...
      if (propertyUtil.getDatabaseServerType() != null &&
          propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID)){
    	  sqlQuery =  "(select CAST(unit_value AS Number) as unit_value from wa_ui_metadata where wa_template_uid =  "+templateUid +" and unit_type_cd = 'CODED'  minus ( select code_set_group_id from wa_ui_metadata where wa_template_uid =  "+templateUid +"  UNION Select code_set_group_id from wa_question where question_identifier in(select question_identifier from  wa_ui_metadata where wa_template_uid =  "+templateUid +" )))";
    	  sqlQuery = sqlQuery + " UNION (select CAST(unit_value AS Number) from wa_question where wa_question_uid in (select wa_question_uid from wa_ui_metadata where wa_template_uid = "+templateUid +") and unit_type_cd='CODED' minus ( select CAST(unit_value AS Number) from wa_ui_metadata where wa_template_uid =  "+templateUid +" and unit_type_cd='CODED' union  select code_set_group_id from wa_ui_metadata where wa_template_uid =  "+templateUid +"  UNION Select code_set_group_id from wa_question where question_identifier in (select question_identifier from  wa_ui_metadata where wa_template_uid =  "+templateUid +" )))";
      }
      else{
    	  sqlQuery =  "(select unit_value from wa_ui_metadata where wa_template_uid =  "+templateUid +" and unit_type_cd = 'CODED'  EXCEPT ( select code_set_group_id from wa_ui_metadata where wa_template_uid =  "+templateUid +"  UNION Select code_set_group_id from wa_question where question_identifier in(select question_identifier from  wa_ui_metadata where wa_template_uid =  "+templateUid +" )))";
    	  sqlQuery = sqlQuery + " UNION (select unit_value from wa_question where wa_question_uid in (select wa_question_uid from wa_ui_metadata where wa_template_uid = "+templateUid +") and unit_type_cd='CODED' except ( select unit_value from wa_ui_metadata where wa_template_uid =  "+templateUid +" and unit_type_cd='CODED' union  select code_set_group_id from wa_ui_metadata where wa_template_uid =  "+templateUid +"  UNION Select code_set_group_id from wa_question where question_identifier in (select question_identifier from  wa_ui_metadata where wa_template_uid =  "+templateUid +" )))";
      }
      try {
		preparedStmt = dbConnection
				.prepareStatement(sqlQuery);
