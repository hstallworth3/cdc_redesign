...
public class PartnerServicesHelper {
	...
	(
		...
		while (clientIter.hasNext()) {
		 ...
		 if (siteIdHash.isEmpty()) { //must be at least 1 site
			...
			theSite.setSiteId(PartnerServicesConstants.PS_DEFAULT_SITE_ID);
			theSite.setSiteTypeValueCode(PartnerServicesConstants.SITE_TYPES_MAP.get(PartnerServicesConstants.PS_DEFAULT_SITE_ID));
			theSite.setZip(PartnerServicesConstants.PS_DEFAULT_SITE_ZIP);
		 }
	...
	private static void populateSiteIfNotInList(
		...
		try {
			...
			while (interviewSummaryListIter.hasNext()) {
				...
				if (PartnerServicesConstants.SITE_TYPES_MAP.containsKey(siteType))
				...
				theSite.setSiteTypeValueCode(PartnerServicesConstants.SITE_TYPES_MAP.get(siteType));
	...
	private static void populateClient(
		...
		String birthYear = PartnerServicesConstants.PS_DEFAULT_YEAR; //1800
		if (birthDate != null)
		...
		if (ethnicityCd != null) {
			if (PartnerServicesConstants.RACE_CATAGORY_MAP.containsKey(ethnicityCd))  {
				client.setEthnicity(PartnerServicesConstants.RACE_CATAGORY_MAP.get(ethnicityCd));
			} else {
				client.setEthnicity(PartnerServicesConstants.RACE_CATAGORY_MAP.get("U")); //unknown
			}
		} else client.setEthnicity(PartnerServicesConstants.RACE_CATAGORY_MAP.get("U")); //unknown
		...
		if (patientAdr != null) {
			if (patientAdr.getStateCd() != null && PartnerServicesConstants.STATE_MAP.containsKey(patientAdr.getStateCd())) {
						client.setStateOfResidence(PartnerServicesConstants.STATE_MAP.get(patientAdr.getStateCd()));
			} else {
				//per Jit, use NBS_STATE_CD for default if missing
				if (PartnerServicesConstants.STATE_MAP.containsKey(defaultStateCd))
					client.setStateOfResidence(PartnerServicesConstants.STATE_MAP.get(defaultStateCd)); 
			}
		} else client.setStateOfResidence(PartnerServicesConstants.STATE_MAP.get(defaultStateCd));
		
		String currentSexCd = patientVO.thePersonDT.getCurrSexCd();
		//some fields we need are only in the MPR
		String birthSexCd = patientMprVO.thePersonDT.getBirthGenderCd();
		if (birthSexCd != null) {
			if (PartnerServicesConstants.BIRTH_GENDER_MAP.containsKey(birthSexCd)) {
				client.setBirthGenderValueCode(PartnerServicesConstants.BIRTH_GENDER_MAP.get(birthSexCd));
			}
		} else if (currentSexCd != null && (currentSexCd.equals("M") || currentSexCd.equals("F"))){
			client.setBirthGenderValueCode(PartnerServicesConstants.BIRTH_GENDER_MAP.get(currentSexCd));
		//required field so if not present, set to Unknown
		} else {
			client.setBirthGenderValueCode(PartnerServicesConstants.BIRTH_GENDER_MAP.get("U"));
		}
		//set current gender 
		...
		if (prefferredGenderCd != null && PartnerServicesConstants.CURRENT_SEX_MAP.containsKey(prefferredGenderCd))
			client.setCurrentGenderValueCode(PartnerServicesConstants.CURRENT_SEX_MAP.get(prefferredGenderCd));
		else if (unknownReasonCd != null && PartnerServicesConstants.CURRENT_SEX_MAP.containsKey(unknownReasonCd))
			client.setCurrentGenderValueCode(PartnerServicesConstants.CURRENT_SEX_MAP.get(unknownReasonCd));
		else if (additionalGender != null && additionalGender.trim().isEmpty()) {
			client.setCurrentGenderValueCode(PartnerServicesConstants.CURRENT_SEX_MAP.get("89"));
			client.setOtherCurrentGender(additionalGender);
		} else if (currentSexCd != null && PartnerServicesConstants.CURRENT_SEX_MAP.containsKey(currentSexCd)) {
			client.setCurrentGenderValueCode(PartnerServicesConstants.CURRENT_SEX_MAP.get(currentSexCd));
		} else client.setCurrentGenderValueCode(PartnerServicesConstants.CURRENT_SEX_MAP.get("R")); //default to refused to ans
		if (drivingPhcProxyVO != null) {
	...
	private static void populateClientRiskProfile(
		...
		if (completedRiskProfile != null && PartnerServicesConstants.NO_CLIENT_RISK_FACTORS.containsKey(completedRiskProfile)) {
			clientRiskProfile.setNoClientRiskFactors(PartnerServicesConstants.NO_CLIENT_RISK_FACTORS.get(completedRiskProfile));
		}
		
		String injDrugUse = getQuestionAnswer("STD114", phcProxyVO.getPageVO(), NEDSSConstants.CASE);
		if (injDrugUse == null || !PartnerServicesConstants.INJECTION_DRUG_USE_MAP.containsKey(injDrugUse))
			injDrugUse = "UNK";
		...
		if (injDrugUse != null && PartnerServicesConstants.INJECTION_DRUG_USE_MAP.containsKey(injDrugUse)) 
			clientRiskProfile.setInjectionDrugUse(PartnerServicesConstants.INJECTION_DRUG_USE_MAP.get(injDrugUse));
		
			
		String shareInjDrugUse = getQuestionAnswer("NBS232", phcProxyVO.getPageVO(), NEDSSConstants.CASE);
		if (shareInjDrugUse == null)
			shareInjDrugUse = "UNK";
		if (shareInjDrugUse != null && PartnerServicesConstants.SHARED_INJECTION_DRUG_USE_MAP.containsKey(shareInjDrugUse)) 
			clientRiskProfile.setShareDrugInjectionEquipment(PartnerServicesConstants.SHARED_INJECTION_DRUG_USE_MAP.get(shareInjDrugUse));
		if (interviewSummaryDTColl != null && !interviewSummaryDTColl.isEmpty()) {
			...
			if (patientInterviewStatus != null && patientInterviewStatus.equals("I")) //interviewed but no interviews
				clientRiskProfile.setDateCollectedForRiskProfile(PartnerServicesConstants.PS_DEFAULT_DATE); // set to dummy date 01/01/1900
		}
		...
		if (completedRiskProfile != null && (completedRiskProfile.equals("66") || completedRiskProfile.equals("77")) && (sexWithMaleLast12.equals("N") || sexWithMaleLast12.equals("O") || sexWithMaleLast12.equals("R") || sexWithMaleLast12.equals("U") || sexWithMaleLast12.equals("D")))
			sexWithMaleLast12 = "UNK";;
		if (sexWithMaleLast12 != null && PartnerServicesConstants.WithMaleLast12.containsKey(sexWithMaleLast12)) 
			vaginalOrAnalSexInLast12Months.setWithMale(PartnerServicesConstants.WithMaleLast12.get(sexWithMaleLast12));
		...
		if (completedRiskProfile != null && (completedRiskProfile.equals("66") || completedRiskProfile.equals("77")) && (sexWithFemaleLast12.equals("N") || sexWithFemaleLast12.equals("O") || sexWithFemaleLast12.equals("R") || sexWithFemaleLast12.equals("U") || sexWithFemaleLast12.equals("D")))
			sexWithFemaleLast12 = "UNK";;
		if (sexWithFemaleLast12 != null && PartnerServicesConstants.WithFemaleLast12.containsKey(sexWithFemaleLast12)) 
			vaginalOrAnalSexInLast12Months.setWithFemale(PartnerServicesConstants.WithFemaleLast12.get(sexWithFemaleLast12));
		...
		if (sexWithMaleLast12 != null && PartnerServicesConstants.WithTransLast12.containsKey(sexWithTransLast12)) 
			vaginalOrAnalSexInLast12Months.setWithTransgender(PartnerServicesConstants.WithTransLast12.get(sexWithTransLast12 ));
		...
		if (sexWithMaleLast12 != null && PartnerServicesConstants.NoCondom.containsKey(sexNoCondomLast12)) 
			vaginalOrAnalSexInLast12Months.setVaginalOrAnalSexWithoutCondomPS(PartnerServicesConstants.NoCondom.get(sexNoCondomLast12));
		...
		if (sexWithIDULast12 != null && PartnerServicesConstants.WithIDULast12.containsKey(sexWithIDULast12)) 
			vaginalOrAnalSexInLast12Months.setVaginalOrAnalSexWithIDUPS(PartnerServicesConstants.WithIDULast12.get(sexWithIDULast12));
		...
		try {
			...
			if ((hadSexWithAnonymousPartner != null && !hadSexWithAnonymousPartner.isEmpty() && PartnerServicesConstants.OUY_MAP.containsKey(hadSexWithAnonymousPartner)) ||
				(hadSexWithKnownIDU != null && !hadSexWithKnownIDU.isEmpty() && PartnerServicesConstants.OUY_MAP.containsKey(hadSexWithKnownIDU)) ||
				(hadSexWhileIntoxicatedHigh != null && !hadSexWhileIntoxicatedHigh.isEmpty() && PartnerServicesConstants.OUY_MAP.containsKey(hadSexWhileIntoxicatedHigh)) ||
				(exchangedMoneyForSex != null && !exchangedMoneyForSex.isEmpty() && PartnerServicesConstants.OUY_MAP.containsKey(exchangedMoneyForSex))) {
				...
				if ((hadSexWithAnonymousPartner != null && !hadSexWithAnonymousPartner.isEmpty()))
					if (PartnerServicesConstants.OUY_MAP.containsKey(hadSexWithAnonymousPartner)) {
						otherRisks.addOtherRiskFactor(PartnerServicesConstants.OTHER_RISK_MAP.get("8")); //with stranger
					}
				if ((hadSexWithKnownIDU != null && !hadSexWithKnownIDU.isEmpty())) {
					if (PartnerServicesConstants.OUY_MAP.containsKey(hadSexWithKnownIDU)) {
					 otherRisks.addOtherRiskFactor(PartnerServicesConstants.OTHER_RISK_MAP.get("15")); //Unprotected vaginal/anal sex with a person who is an IDU
					}
				}
				if ((hadSexWhileIntoxicatedHigh != null && !hadSexWhileIntoxicatedHigh.isEmpty())) {
					if (PartnerServicesConstants.OUY_MAP.containsKey(hadSexWhileIntoxicatedHigh)) {
					 otherRisks.addOtherRiskFactor(PartnerServicesConstants.OTHER_RISK_MAP.get("2")); //intoxicated
					}
				}
				if ((exchangedMoneyForSex != null && !exchangedMoneyForSex.isEmpty())) {
					if (PartnerServicesConstants.OUY_MAP.containsKey(exchangedMoneyForSex)) {
					 otherRisks.addOtherRiskFactor(PartnerServicesConstants.OTHER_RISK_MAP.get("1"));
					}//for money
				}
			}
		} catch (Exception ex) {
	...
	private static boolean isPartnerCase(Collection<Object> contactSummaryCollection) {
		...
		while (itr.hasNext()) {
			...
			if (contactSummaryDT.isPatientNamedByContact() && 
					...
					(contactSummaryDT.getSourceDispositionCd().equalsIgnoreCase(PartnerServicesConstants.PREV_POS) ||
							contactSummaryDT.getSourceDispositionCd().equalsIgnoreCase(PartnerServicesConstants.PREV_NEG_NEW_POS) ||
							contactSummaryDT.getSourceDispositionCd().equalsIgnoreCase(PartnerServicesConstants.NO_PREV_NEW_POS)) &&
							contactSummaryDT.getContactReferralBasisCd() != null &&
							contactSummaryDT.getContactReferralBasisCd().startsWith("P"))
				return true;
		}
    ...
    private static void populateIndexPatientInformation(
		...
		if (initialFollowup != null && initialFollowup.equalsIgnoreCase("SF"))
			partnerServiceCase.setReportedToSurveillance(PartnerServicesConstants.REPORTED_TO_SURVEILLANCE_MAP.get("Y"));
		else 
			partnerServiceCase.setReportedToSurveillance(PartnerServicesConstants.REPORTED_TO_SURVEILLANCE_MAP.get("N"));
		...
		String patientInterviewStatus = phcProxyVO.getPublicHealthCaseVO().getTheCaseManagementDT().getPatIntvStatusCd();
		if (patientInterviewStatus != null && PartnerServicesConstants.ATTEMPT_TO_LOCATE_OUTCOME_MAP.containsKey(patientInterviewStatus)) {
			attemptToLocate.setAttemptToLocateOutcome(PartnerServicesConstants.ATTEMPT_TO_LOCATE_OUTCOME_MAP.get(patientInterviewStatus));
    	} else {
    		attemptToLocate.setAttemptToLocateOutcome(PartnerServicesConstants.ATTEMPT_TO_LOCATE_OUTCOME_MAP.get("U")); //Unable
    	}
		...
		if (attemptToLocateResult != null && attemptToLocateResult.toString().equals("1")) {  //1 = unable to locate
			...
			if (reasonUnableToLocate != null && PartnerServicesConstants.REASON_UNABLE_TO_LOCATE_MAP.containsKey(reasonUnableToLocate)) {
				attemptToLocate.setReasonForUnsuccessfulAttempt(PartnerServicesConstants.REASON_UNABLE_TO_LOCATE_MAP.get(reasonUnableToLocate));
			} else {
				if (patientInterviewStatus != null && patientInterviewStatus.equalsIgnoreCase("D")) //deceased
					attemptToLocate.setReasonForUnsuccessfulAttempt(PartnerServicesConstants.REASON_UNABLE_TO_LOCATE_MAP.get("01")); //Deceased
				else if (patientInterviewStatus != null && patientInterviewStatus.equalsIgnoreCase("J"))
					attemptToLocate.setReasonForUnsuccessfulAttempt(PartnerServicesConstants.REASON_UNABLE_TO_LOCATE_MAP.get("02")); //OOJ
				else if (patientInterviewStatus == null || (patientInterviewStatus != null && !patientInterviewStatus.equalsIgnoreCase("I"))) 
					attemptToLocate.setReasonForUnsuccessfulAttempt(PartnerServicesConstants.REASON_UNABLE_TO_LOCATE_MAP.get("89")); //Other Specify - but no place to specify in 2.1 schema
			}
		}		
	...
	private static void populateElicitPartnerInformationIfPresent(
		...
		if (totalNumberOfNamed != 0 ) {
			...
			try {
			elicitPartner.setTimePeriodForRecallInMonths(new BigInteger(PartnerServicesConstants.ELICIT_PARTNER_TIME_PERIOD)); //12
    ...
    private static void populateIndexSessionInformation(
    	...
    	while (interviewSummaryIterator.hasNext()) {
    		...
    		if (siteId == null || siteId.isEmpty() || siteType == null || siteType.isEmpty()) {
    			...
    			siteId = PartnerServicesConstants.PS_DEFAULT_SITE_ID;
    		}
	 ...
	 */
		private static void getTheRaces(PersonVO personVO, TypeRaces races) {
			...
			if(personVO.getThePersonRaceDTCollection()!=null){
				...
				while(ite.hasNext()){
					...
					if( recordStatusCd != null  && recordStatusCd.equalsIgnoreCase(NEDSSConstants.RECORD_STATUS_ACTIVE)) {
						if (PartnerServicesConstants.RACE_MAP.containsKey(raceDT.getRaceCd())) {
							Race theRace = races.addNewRace();
							theRace.setRaceValueCode(PartnerServicesConstants.RACE_MAP.get(raceDT.getRaceCd()));
						}
					}
				} //has next
			} //collection not null
			...
			if (!raceFound) { //race is required, if no races present add an 'Unknown Race'
				Race theRace = races.addNewRace();
				if (refusedToAnswer)
					theRace.setRaceValueCode(PartnerServicesConstants.RACE_MAP.get(NEDSSConstants.REFUSED_TO_ANSWER));
				else 
					theRace.setRaceValueCode(PartnerServicesConstants.RACE_MAP.get(NEDSSConstants.UNKNOWN));
			}
			return;
	   }
	...
	private static void populatePartnerInformation(
		...
		try {
			...
			if (referralBasisCd != null && PartnerServicesConstants.PARTNER_TYPE_MAP.containsKey(referralBasisCd))
				partner.setPartnerType(PartnerServicesConstants.PARTNER_TYPE_MAP.get(referralBasisCd));
			//**********Attempt to Locate********//
			...
			if (attemptToLocateResult != null && attemptToLocateResult.toString().equals("1")) {  //1 = unable to locate
				...
				if (reasonUnableToLocate != null && PartnerServicesConstants.REASON_UNABLE_TO_LOCATE_MAP.containsKey(reasonUnableToLocate)) {
					...
					if (patientInterviewStatus != null && patientInterviewStatus.equalsIgnoreCase("D")) //deceased
						...
						attemptToLocate.setReasonForUnsuccessfulAttempt(PartnerServicesConstants.REASON_UNABLE_TO_LOCATE_MAP.get("02")); //OOJ
					else attemptToLocate.setReasonForUnsuccessfulAttempt(PartnerServicesConstants.REASON_UNABLE_TO_LOCATE_MAP.get("89")); //Other Specify - but no place to specify in 2.1 schema
				}
		...
		try {
			...
			if (previousHivTest != null && previousHivTest.equals("Y") && dateOfLastHivTest == null)
				dateOfLastHivTest = PartnerServicesConstants.PS_DEFAULT_DATE;
			String selfReportedHivTest = getQuestionAnswer("STD106", phcProxyVO.getPageVO(), NEDSSConstants.CASE);
			if ((partnerNotifiability != null && PartnerServicesConstants.PARTNER_NOTIFIABILITY_MAP.containsKey(partnerNotifiability)) 
					|| (actualNotificationMethod != null && PartnerServicesConstants.PARTNER_NOTIFIABILITY_MAP.containsKey(partnerNotifiability))
					|| (previousHivTest != null && PartnerServicesConstants.PARTNER_PREVIOUS_HIV_TEST_MAP.containsKey(previousHivTest)) 
					|| dateOfLastHivTest != null || (selfReportedHivTest != null && PartnerServicesConstants.PARTNER_SELF_REPORTED_HIV_TEST_MAP.containsKey(selfReportedHivTest))) {
				...
				if (previousHivTest != null && PartnerServicesConstants.PARTNER_PREVIOUS_HIV_TEST_MAP.containsKey(previousHivTest)) 
					exposureNotification.setPreviousHIVTest(PartnerServicesConstants.PARTNER_PREVIOUS_HIV_TEST_MAP.get(previousHivTest));
				if (dateOfLastHivTest != null)
					exposureNotification.setDateOfLastHIVTest(dateOfLastHivTest);
				if (selfReportedHivTest != null && PartnerServicesConstants.PARTNER_SELF_REPORTED_HIV_TEST_MAP.containsKey(selfReportedHivTest))
					exposureNotification.setSelfReportedHIVTestResult(PartnerServicesConstants.PARTNER_SELF_REPORTED_HIV_TEST_MAP.get(selfReportedHivTest));
				if (partnerNotifiability != null && PartnerServicesConstants.PARTNER_NOTIFIABILITY_MAP.containsKey(partnerNotifiability))
					exposureNotification.setPartnerNotifiability(PartnerServicesConstants.PARTNER_NOTIFIABILITY_MAP.get(partnerNotifiability));
				else 
					exposureNotification.setPartnerNotifiability(PartnerServicesConstants.PARTNER_NOTIFIABILITY_MAP.get("88")); //required field, put Other.
				if (actualNotificationMethod != null && partnerNotifiability != null &&
						partnerNotifiability.equals("6") &&  PartnerServicesConstants.PARTNER_ACTUAL_NOTIFICATION_METHOD_MAP.containsKey(actualNotificationMethod))
					exposureNotification.setActualNotificationMethod(PartnerServicesConstants.PARTNER_ACTUAL_NOTIFICATION_METHOD_MAP.get(actualNotificationMethod));
			}
		} catch (Exception ex) {
		...
		try {
			...
			if (referralDate == null && attemptToLocateResult != null && attemptToLocateResult.toString().equals("2")) { //located but no default, per Dougan default to 01/01/1900
				referralDate = PartnerServicesConstants.PS_DEFAULT_DATE;
			}
			if (referralDate != null || 
					(referredToHivTesting != null && PartnerServicesConstants.REFERRED_TO_HIV_TESTING_MAP.containsKey(referredToHivTesting)) ||
					(referredToMedicalCare != null && PartnerServicesConstants.REFERRED_TO_MEDICAL_CARE_MAP.containsKey(referredToMedicalCare)) ||
					(firstMedicalCareAppointment != null && PartnerServicesConstants.FIRST_MEDICAL_APPOINTMENT_MAP.containsKey(firstMedicalCareAppointment)) ||
					(hivTestPerformed != null && PartnerServicesConstants.HIV_TEST_PERFORMED_MAP.containsKey(hivTestPerformed)) ||
					(hivTestResult != null && PartnerServicesConstants.HIV_TEST_RESULT_MAP.containsKey(hivTestResult)) ||
					(hivTestResultProvided != null && PartnerServicesConstants.HIV_TEST_RESULTS_PROVIDED_MAP.containsKey(hivTestResultProvided)) ) {
				...
				if (referredToHivTesting != null && PartnerServicesConstants.REFERRED_TO_HIV_TESTING_MAP.containsKey(referredToHivTesting))
					referral.setReferredToHIVTesting(PartnerServicesConstants.REFERRED_TO_HIV_TESTING_MAP.get(referredToHivTesting));
				if (referredToMedicalCare != null && PartnerServicesConstants.REFERRED_TO_MEDICAL_CARE_MAP.containsKey(referredToMedicalCare))
					referral.setReferredToMedicalCare(PartnerServicesConstants.REFERRED_TO_MEDICAL_CARE_MAP.get(referredToMedicalCare));
				if (firstMedicalCareAppointment != null && PartnerServicesConstants.FIRST_MEDICAL_APPOINTMENT_MAP.containsKey(firstMedicalCareAppointment))
					referral.setFirstMedicalCareAppointment(PartnerServicesConstants.FIRST_MEDICAL_APPOINTMENT_MAP.get(firstMedicalCareAppointment));
				if (hivTestPerformed != null && PartnerServicesConstants.HIV_TEST_PERFORMED_MAP.containsKey(hivTestPerformed))
					referral.setHIVTestPerformed(PartnerServicesConstants.HIV_TEST_PERFORMED_MAP.get(hivTestPerformed));
				if (hivTestResult != null && PartnerServicesConstants.HIV_TEST_RESULT_MAP.containsKey(hivTestResult))
					referral.setHIVTestResult(PartnerServicesConstants.HIV_TEST_RESULT_MAP.get(hivTestResult));
				if (hivTestResultProvided != null && PartnerServicesConstants.HIV_TEST_RESULTS_PROVIDED_MAP.containsKey(hivTestResultProvided))
					referral.setHIVTestResultsProvided(PartnerServicesConstants.HIV_TEST_RESULTS_PROVIDED_MAP.get(hivTestResultProvided));
			}
    ...
    private static String findCaseNumberForPHC(PageActProxyVO phcProxyVO) {
		...
		if (phcProxyVO.getPublicHealthCaseVO().getTheActIdDTCollection() != null && 
			...
			while (actIdIter.hasNext()) {
				...
				if ((actIdDt.getTypeCd().equalsIgnoreCase(PartnerServicesConstants.LEGACY_ID_TYPE))
	...
	private static void addDefaultSite(Sites sites, HashMap<String, String> siteIdHash) {
		//Dummy site added for Interviews that don't specify Site Type or Zip
			...
			noNamespace.XpemsPSDataDocument.XpemsPSData.Sites.Site theSite = sites.addNewSite();
			theSite.setSiteId(PartnerServicesConstants.PS_DEFAULT_SITE_ID);
			theSite.setSiteTypeValueCode(PartnerServicesConstants.SITE_TYPES_MAP.get(PartnerServicesConstants.PS_DEFAULT_SITE_TYPE));
			theSite.setZip(PartnerServicesConstants.PS_DEFAULT_SITE_ZIP);
			siteIdHash.put(PartnerServicesConstants.PS_DEFAULT_SITE_ID, PartnerServicesConstants.PS_DEFAULT_SITE_TYPE);
			return;
	}
}
