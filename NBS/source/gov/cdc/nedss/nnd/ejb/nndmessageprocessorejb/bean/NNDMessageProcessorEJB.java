	...
	public void buildAndWriteInvestigationMessage(NotificationDT notificationDT,
		...
		try {
			...
			try {
			    ...
			    phcVO = actController.getPublicHealthCase(actRelationshipDT.getTargetActUid(),  nbsSecurityObj);
			   publicHealthCaseDT=phcVO.getThePublicHealthCaseDT();
			}
			catch (Exception e) {
				e.printStackTrace();
			...
			TreeMap conditionMap = notificationConditionCodeDAOImpl.getEntityIdentiferForNNDConditions(NEDSSConstants.INDIVIDUAL);
...
if(notificationDT.getNndInd()!=null && notificationDT.getNndInd().equals(NEDSSConstants.YES)){
   ...
   try {
     ...
     investigationProxyVO = investigationProxy.getInvestigationProxy(
   ...
   try {
     ...
     if (morbidityProxyVOList != null) {
	   ...
	   logger.fatal(
				  " Exception while getting ObservattionProxyEJB");
     ce.printStackTrace();
     NNDException nnde = new NNDException(ce.getMessage() +
   ...
   try {
     ...
     Collection<Object>  vaccinationSumVOColl = investigationProxyVO.
     ...
     for (Iterator<Object> anIterator = vaccinationSumVOColl.iterator();
			   anIterator.hasNext(); ) {

			VaccinationSummaryVO vacSumVO = (VaccinationSummaryVO) anIterator.next();
			interventionUid = vacSumVO.getInterventionUid();
			...
			if(interventionProxy == null) {
			  ...
			  Object object = nedssUtils.lookupBean(sBeanJndiName);
			  InterventionProxyHome interventionProxyHome =
			      (InterventionProxyHome) PortableRemoteObject.narrow(
			      object, InterventionProxyHome.class);
			  logger.debug(
			      "Found InterventionProxyHome: " +
			      interventionProxyHome);
			  interventionProxy = interventionProxyHome.create();
			}//end of
			vaccinationProxyVO = interventionProxy.getVaccinationProxy(
			    interventionUid, nbsSecurityObj);
			if(vaccinationProxyVO.getTheMaterialVO() == null || vaccinationProxyVO.getTheObservationVOCollection() == null ){
			...
			vaccinationProxyVOList.add(vaccinationProxyVO);
			 ...
			 if (vaccinationProxyVOList != null) {
					messageProcessorVO.setTheVaccinationProxyVOCollection(
					    vaccinationProxyVOList);
			 }
          }
...
/**
    ...
    try {
		...
		if ( notificationDT.getCd() != null && notificationDT.getCd().equals(NEDSSConstants.AGGREGATE_NOTIFICATION_CD) ) { 
			...
			try {
				...
				logger.fatal("While trying to create Aggregate Summary Notification Message for Notification uid:  " + notificationDT.getNotificationUid() + ", the Cn_transportq_out update failed due to data concurrent exception.");
  ...
  private void updateNotificationComplete(NotificationDT notificationDT,
    ...
    try {
      ...
      logger.fatal("concurrent access is not allowed!");
      NNDException nnde = new NNDException(
   ...
   */
	...
	public Collection<Object> getNETSSTransportQOutDTCollection(Short mmwrYear, Short mmwrWeek, Boolean includePriorYear, NBSSecurityObj nbsSecurityObj) throws java.rmi.RemoteException, NEDSSSystemException {
		...
		{
			logger.error("Exception retrieving NETSS records for " + mmwrYear + " " + mmwrWeek + " " +e.getMessage());
			e.printStackTrace();
			throw new NEDSSSystemException("Error retrieving NETSS Collection: " + e.getMessage());
		}		
