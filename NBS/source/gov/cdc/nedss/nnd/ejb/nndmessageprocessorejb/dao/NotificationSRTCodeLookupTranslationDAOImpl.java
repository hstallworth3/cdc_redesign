...
public class NotificationSRTCodeLookupTranslationDAOImpl extends DAOBase{
	...
	private static String SELECT_CODESET_INFO_ORACLE = "SELECT code_set_nm \"codeSetNm\", class_cd \"classCd\" from " + 
	NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".Codeset where code_set_group_id = ?";
	...
	private static String SELECT_SRTCODE_INFO_ORACLE = "SELECT concept_preferred_nm \"codedValueDescription\", code_system_cd \"codedValueCodingSystem\", concept_code \"codedValue\" FROM "+
	NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".";
	...
	private static String SELECT_SRT_CODE_TRANSLATION_PHIN_SQL = "SELECT to_code \"codedValue\", to_code_desc_txt \"codedValueDescription\", to_code_system_cd \"codedValueCodingSystem\" FROM "+
			NEDSSConstants.SYSTEM_REFERENCE_TABLE + "..";
	private static String SELECT_SRT_CODE_TRANSLATION_PHIN_ORACLE = "SELECT to_code \"codedValue\", to_code_desc_txt \"codedValueDescription\", to_code_system_cd \"codedValueCodingSystem\" FROM "+
	NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".";
	...
	NEDSSConstants.SYSTEM_REFERENCE_TABLE + "..";
	private static String SELECT_SRT_CODE_TRANSLATION_NBS_ORACLE = "SELECT from_code \"codedValue\", from_code_desc_txt \"codedValueDescription\", to_code_system_cd \"codedValueCodingSystem\" FROM "+
	NEDSSConstants.SYSTEM_REFERENCE_TABLE + ".";
	...
	private void retrieveCodeSetInfo(Coded codeToLookup) throws NEDSSSystemException {
        ...
        if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
        	sql = SELECT_CODESET_INFO_ORACLE;
        else
		...
		try
		{
			NNDPamCaseDataDTCollection  = (ArrayList<Object> )preparedStmtMethod(newCaseNotificationDataDT, queryArugments, sql, NEDSSConstants.SELECT);
		}
		 catch (Exception ex) {
	    	String errString = "NotificationSRTCodeLookupTranslationDAOImpl.retrieveCodeSetInfo:  Failed to retrieve codeset name for code_set_group_id = " + 
	    						codeToLookup.getCodesetGroupId() +
	   ...
	   Iterator<Object>  it  = NNDPamCaseDataDTCollection.iterator();
		while (it.hasNext()) {
			...
			{
		    	newCaseNotificationDataDT = (CaseNotificationDataDT)object;
	...
	public void retrieveSRTCodeInfo(Coded codeToLookup) throws NEDSSSystemException
	{
        ...
        if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
        	sql = SELECT_SRTCODE_INFO_ORACLE;
        else
        	sql = SELECT_SRTCODE_INFO_SQL;
		
		try
		{
			NNDPamCaseDataDTCollection  = (ArrayList<Object> )preparedStmtMethod(
												newCaseNotificationDataDT, 
												queryArugments, 
												sql +
												codeToLookup.getCodesetTableName() +
												SELECT_SRTCODE_INFO_WHERECLAUSE, 
	...
	private void retrieveNBSCodetoPHINCodeTranslation(CaseNotificationDataDT codeToTranslate) throws NEDSSSystemException
	{
		...
		String sql;
        if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
        	sql = SELECT_SRT_CODE_TRANSLATION_PHIN_ORACLE;
        else 
	...
	private void retrievePHINCodetoNBSCodeTranslation(CaseNotificationDataDT codeToTranslate) throws NEDSSSystemException
	{
		...
		String sql;
		if (propertyUtil.getDatabaseServerType() != null && propertyUtil.getDatabaseServerType().equalsIgnoreCase(NEDSSConstants.ORACLE_ID))
        	sql = SELECT_SRT_CODE_TRANSLATION_NBS_ORACLE;
        else 
