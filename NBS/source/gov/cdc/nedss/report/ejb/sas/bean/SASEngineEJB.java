...
public class SASEngineEJB implements SessionBean {
	...
	private void initConnection() throws Exception {
		try {
			...
			if (sasarch.equalsIgnoreCase("PC")) {
				Connection.setServerArchitecture(connection, sasarch);
			} else {
				Connection.setServerArchitecture(connection, sasarch);
				connection.setUsernamePrompt(propUtil.getUserNamePrompt());
				connection.setUsername(propUtil.getUserName());
				connection.setPasswordPrompt(propUtil.getPassWordPrompt());
				connection.setPassword(propUtil.getPassWord());
				connection.setPort(2323);
				connection.setCommandPrompt(propUtil.getCommandPrompt());
				String sasCommand = propUtil.getProperties().getProperty(
						"SASCOMMAND");
				connection.setCommand(sasCommand);
				logger.debug("The new Sas command is :" + sasCommand);

			}
			logger.debug("The connection details are Username:"
					+ connection.getUsername());
			logger.debug("The connection details are UsernamePrompt:"
					+ connection.getUsernamePrompt());
			logger.debug("The connection details are PasswordPrompt:"
					+ connection.getPasswordPrompt());
