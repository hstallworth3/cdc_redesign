...
<html>
    <head>
      ...
      /*  function _setAttributes(headerNm, link, colId) 
       ...
       {
       	document.forms[0].action ='/nbs/ManageTemplates.do?method=ManageTemplatesLib&initLoad=true';
       	document.forms[0].submit();
       }
							    ...
							    <table style="width:100%;">
									<tr>
									    <td style="text-align:right"  id="srtLink"> 
									        ...
									        <input type="hidden" id="ActionMode" value="${manageTemplateForm.actionMode}"/>
									    </td>
									</tr>
								...
								<% }%>
								 ...
								 <% if (request.getAttribute("ConfirmMesgCreate") != null) { %>
									  <div class="infoBox success">
									  	<%=(String)request.getAttribute("ConfirmMesgCreate")%>&nbsp;<a href='<%=request.getAttribute("clickHereLk")%>' >Click Here</a> to access the Add Page screen.<br/>
									  </div>    
						           <% } %>
						           <% if (request.getAttribute("ConfirmMesg") != null) { %>
									  <div class="infoBox success" id="success1">
						            ...
						            <% if (request.getAttribute("confirmation") != null && request.getAttribute("confirmation").toString().equals("uploaderror")) {
						            	 if(request.getAttribute("status") != null && request.getAttribute("status").toString().equals("templateAlreadyExists")) {%>
					                        <div class="infoBox errors" id="error1">
						                        Import of <b><%=request.getAttribute("srcTemplateNm") == null ? "" : request.getAttribute("srcTemplateNm")%> </b> failed. Template  with the given  template name  <b><%=request.getAttribute("srcTemplateNm") == null ? "" : request.getAttribute("srcTemplateNm")%> </b> being imported already exists in the system.   
					                    ...
					                    <%}else if(request.getAttribute("status") != null && request.getAttribute("status").toString().equals("filenotfound")) {%>
					                     ...
					                     <%}else{ %>     
					                        <div class="infoBox errors" id="error3">
						                        Import of <b><%=request.getAttribute("srcTemplateNm") == null ? "" : request.getAttribute("srcTemplateNm")%> </b> failed. Please see import log for more details.    
					                ...
					                <td style="padding:0.5em;">					                   
					                    <div id="addAttachmentBlock" class="boxed">
				                           <table class="formTable" style=" width:100%; margin:0px;">
									        ...
									        <!-- File Attachment -->
									        <tr>
									            <td class="fieldName" id="chooseFileLabel"> <span style="color:red; font-weight:bold;">*</span>Template Name:</td>
		                                    ...
		                                    <tr>
			                                    <td align="center">
			                                      <display:table name="manageList" class="dtTable" style="margin-top:0em;"  id="parent"  pagesize="${manageTemplateForm.attributeMap.queueSize}"   requestURI="/ManageTemplates.do?method=ManageTemplatesLib&existing=true" sort="external" export="true" excludedParams="stringQueueCollection answerArrayText(SearchText1) answerArrayText(SearchText2) answerArray(LASTUPDATED) answerArray(LASTUPDATEDBY) answerArray(SOURCE) answerArray(STATUS)  method">
														...
														<display:column property="viewLink" title="" media="html" style="width:4%;text-align:center;"/>
