...
<html>
	...
	/* 	function cancelForm()
        ...
        {
			document.forms[0].action ="/nbs/PortPage.do?method=submitSaveAndContinue&isQuestionPage=true";
			document.forms[0].submit();
        } 
	    ...
	    function makeMSelects() {
			$j("#pStatus").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
			$j("#fromID").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
			$j("#fromLabel").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
			$j("#fromDtTyp").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
			$j("#pMap").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
			$j("#toID").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
			$j("#toLab").text({actionMode: '<%= request.getAttribute("ActionMode") %>'});
			$j("#toDtTyp").multiSelect({actionMode: '<%= request.getAttribute("ActionMode") %>'});
	    }
...
function tabBrowsing() {      
 ...
 <body onload="attachIcons();makeMSelects();disableSubmit();showCount();displayFilterAndSortIcons();autocompTxtValuesForJSP();tabBrowsing();startCountdown();reconcileSelects('availableConditions','selectedConditions')">
				...
				<div  style="text-align:right; margin-bottom:8px;">
    				...
    				<div style="text-align: right;"><i> <span class="boldRed">*</span> Indicates a Required Field</i></div>
	      			<div class="grayButtonBar" style="text-align: right;">
	      				<input type="button"  name="SaveandContinue" value="Save and Continue" id=" " onclick="saveContinue();"/>
        			  ...
        			  <!-- To Display Map Name -->
        			   <div class="grayButtonBar" style="text-align:left;font-weight:bold;font-size:14px">
        				...
        				<div style="text-align:right">
        				     <span ><font color="red"><b>Questions Remaining To Be Mapped:</b>
                                ...
                                <td align="center">
                                    ...
                                    excludedParams="answerArrayText(SearchText1) answerArrayText(SearchText2)  answerArrayText(SearchText3) answerArrayText(SearchText4) answerArray(STATUS) answerArray(MAP) answerArray(FRMDATATYPE) answerArray(TODATATYPE) answerArray(PORTCOND) answerArray(PORTDATE) answerArray(EVENTTYPE) answerArray(MAPPINGSTATUS) currentQuestion.blockIdNbr currentQuestion.answerGroupSeqNbr currentQuestion.fieldMappingRequired currentQuestion.codeMappingRequired currentQuestion.fromQuestionId currentQuestion.legacyBlockInd currentQuestion.toQuestionId method">
										...
										<display:column title="" style="width:2%;text-align:center;" media="html">
												 &nbsp;&nbsp;&nbsp;<img src="page_white_text.gif" class="cursorHand" style= "cursor:pointer" title="View"
                                    	...
                                    	<display:column title="" style="width:2%;text-align:center;" media="html">
                            				 ...
                            				 &nbsp;&nbsp;&nbsp;<img src="page_white_edit.gif" class="cursorHand" style= "cursor:pointer" title="Map"
                     	...
                     	<br/>
                     	<br/>		
                     	<div id="pageMappingAttributes" style="background:#CEE3F6">
							<br/>
							<table width="90%" style="table-layout:fixed" align="center">
								<tr>
									...
									<td>
										<html:select name="portPageForm" property="currentQuestion.toQuestionId"  styleId="toQuestionList"  style="width:350" onchange="populateToFields(this)">
										<html:option value=""/>
								...
								<tr>
									...
									<td>
										<html:select name="portPageForm" property="currentQuestion.fieldMappingRequired" styleId="questionMappingReq" onchange="disableEnableTOFields(this.value);">
									...
									<td>
										<html:select name="portPageForm" property="currentQuestion.codeMappingRequired" styleId="answerMappingReq">
											<html:optionsCollection property="codedValue(YN)" value="key" label="value" />
										</html:select>
									</td>
								</tr>
								<tr>
									...
									<td>
										<html:text name="portPageForm" styleId="repeatingBlockNumber" property="currentQuestion.blockIdNbr" disabled="true" ></html:text>
									</td>
								</tr>
								<tr>
    				...
    				<div class="grayButtonBar" style="text-align: right;">
            				...
            				<input type="Button" align="right" name="SaveandContinue2" value="Save and Continue" onclick="saveContinue();"/> 
    ...
    <input type="hidden" id="totCnt" value="<%= request.getAttribute("totalCount")%>"/>
    <input type="hidden" id="mapReqCnt" value="<%= request.getAttribute("mapReqCount")%>"/>
