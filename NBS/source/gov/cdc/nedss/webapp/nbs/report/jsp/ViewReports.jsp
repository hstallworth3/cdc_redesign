		                                ...
		                                <tr>
		                                   <td colspan="2">
		                                       <% 
		                                           ...
		                                           if (reportTypeKey.equals("private")||reportTypeKey.equals("Reporting_Facility")) {
	                                           ...
	                                           <display:table name="sectionReports" class="<%= dtTableClass %>" id="<%= dtIndex %>">
	                                               <display:column title="" style="width:4em; text-align:center;">
	                                                    <% if (((ReportDT)pageContext.getAttribute(dtIndex)).isRunPermission() && ((ReportDT)pageContext.getAttribute(dtIndex)).getReportTypeCode().equals("JAVA_REPORT_HTML")) { %>
                                                             <a href="#" onclick="openReportPopup(<%=((ReportDT)pageContext.getAttribute(dtIndex)).getReportUid()%>, <%=((ReportDT)pageContext.getAttribute(dtIndex)).getDataSourceUid()%>);">Run</a>
                                                        <% }%><%else if (((ReportDT)pageContext.getAttribute(dtIndex)).isRunPermission()) { %>
                                                             <a href="javascript:runReport('<%=((ReportDT)pageContext.getAttribute(dtIndex)).getShared()%>',<%=((ReportDT)pageContext.getAttribute(dtIndex)).getReportUid()%>,<%=((ReportDT)pageContext.getAttribute(dtIndex)).getDataSourceUid()%>)"> Run </a>
                                                        <% }%><% else{%>
                                                          <span style="color:#BBB;">Run</span>
                                                         <%}%>

	                                               </display:column>
	                                               <%if (reportTypeKey.equalsIgnoreCase("private")||reportTypeKey.equalsIgnoreCase("Reporting_Facility") ){%>  
	                	                           <display:column title="" style="width:4em; text-align:center;">
	                                               <% if ((reportTypeKey.equalsIgnoreCase("private")||reportTypeKey.equalsIgnoreCase("Reporting_Facility")) && ((ReportDT)pageContext.getAttribute(dtIndex)).isDeletePermission()) { %>
	                                                        <a href="javascript:deleteReport('<%=((ReportDT)pageContext.getAttribute(dtIndex)).getShared()%>',<%=((ReportDT)pageContext.getAttribute(dtIndex)).getReportUid()%>,<%=((ReportDT)pageContext.getAttribute(dtIndex)).getDataSourceUid()%>)"> Delete </a>
								<% } else if((reportTypeKey.equalsIgnoreCase("private")||reportTypeKey.equalsIgnoreCase("Reporting_Facility")) && ! ((ReportDT)pageContext.getAttribute(dtIndex)).isDeletePermission()){%>
									<span style="color:#BBB;">Delete</span>
								<%}%>
	                                                   </display:column>
	                                               <%}%>
	                                               <display:column title="Report Title" class="hoverDescLink">
	                                                    <a href="javascript:void(0)" title="<%=((ReportDT)pageContext.getAttribute(dtIndex)).getDescTxt()%>"> <%=((ReportDT)pageContext.getAttribute(dtIndex)).getReportTitle()%> </a>
	                                               </display:column>
