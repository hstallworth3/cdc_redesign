...
function mergePersons()
{
	...
	for(var j=0;j<inputs.length;j++)
	{
	    ...
	    if(inputs.item(j).id=="survivor" && inputs.item(j).checked==true && inputs.item(j-1).id=="merge" && inputs.item(j-1).checked==false)
		  {     //errorText = "You selected a PatientId as the Survivor but you unchecked the associated Merge checkbox. Please recheck the merge checkbox or choose another Survivor.\n";
