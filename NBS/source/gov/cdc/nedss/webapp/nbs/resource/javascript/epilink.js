function validateEpilink()
{
	...
	if(sev != null && sev.value.length != 10 ) {
        errors[index++] = "Current Epi Link ID format should be alphanumeric and 10 digits in length ";
        getElementByIdOrByName("epilinkId1").style.color="990000";
		isError = true;
	}
	if(sev2 != null && sev2.value.length != 10) {
        errors[index++] = "New Epi Link ID format should be alphanumeric and 10 digits in length. ";
        getElementByIdOrByName("epilinkId1").style.color="990000";
		isError = true;
	}
