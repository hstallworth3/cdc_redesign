	...
	private void updateActivityLogLinks(ArrayList resultedTestList, String searchType)
	{
		...
		while (ite.hasNext()) {
			if(NEDSSConstants.TEST_TYPE_LOCAL.equals(searchType))
			{
				...
				String url = "<a href=\"javascript:populateResultedTestName('" + resultedTestVO.getLabTestDescription() + "','" + resultedTestVO.getLabTestCd() + "');\">Select</a>"; 
			...
			}else if(NEDSSConstants.TEST_TYPE_LOINC.equals(searchType))
			{
				...
				String url = "<a href=\"javascript:populateResultedTestName('" + loincResultDT.getLoincComponentName() + "','" + loincResultDT.getLoincCd() + "');\">Select</a>"; 
