...
//Page 4			
	...
	private static void fillPDForm(PDAcroForm acroForm, HttpServletRequest req) throws NEDSSAppException {
		...
		try {
				...
				getMappedValues(InterviewFieldsMap, req);
				...
				populatePlaceRecord(proxyVO, req.getSession());
	...
	public static Map<String, String>  getContactInformationFormFields(PageActProxyVO theProxyVO, HttpServletRequest request) throws NEDSSAppException {
		...
		while (conSumIter.hasNext()) {
			...
			try {
				...
				if(coProxyVO!=null)
					try {
						coinfectionInvList = PageLoadUtil.getSpecificCoinfectionInvList(contactsPHCProxyVO.getPublicHealthCaseVO().getThePublicHealthCaseDT().getPublicHealthCaseUid(), request);
	 ...
	 * @param session
	 */
	private static void populatePlaceRecord(PageActProxyVO proxyVO, HttpSession session) {
