...
public class UpdatedNotifications extends DispatchAction 
{
            ...
            HttpServletResponse response) throws IOException, ServletException
    {
		...
		try {
			...
			if(forceEJBcall) {
	            ...
	            updatedNotificationsForm.getAttributeMap().put("DSUpdatedNotificationList",updatedNotifications);
			}
			else {
				updatedNotifications = (ArrayList<Object> ) updatedNotificationsForm.getAttributeMap().get("DSUpdatedNotificationList");
	    	}
	        ...
	        updatedNotificationsForm.setNotifications(updatedNotifications);
	        updatedNotificationsForm.initializeDropDowns();
            ...
            HttpServletResponse response) throws IOException, ServletException
    {
		...
		if (notificationVOs != null) {
			...
			notificationVOs = (ArrayList<Object> ) notificationsForm.getNotifications();
		}
		notificationsForm.getAttributeMap().put("DSUpdatedNotificationList", notificationVOs);
    		...
    		HttpServletRequest request) 
    {
		...
		try {
			...
			ArrayList<Object> currentNotifications = (ArrayList<Object> ) notificationsForm.getNotifications();
			...
			if (updated.getCaseClassCdTxt() != null) {
				if (updated.getCaseClassCdTxt().trim().length() > 0) {
					if (updated.getCaseClassCdTxt().indexOf("*") >= 0) {
						String decoratedText = "<span style=\"font-weight:bold;color:#FF0000;\">" + updated.getCaseClassCdTxt() + "</span>";
