...
public class ContactTracingAction extends DispatchAction {
	...
	throws IOException, ServletException {
        ...
        try {
               coinfectionInvList = PageLoadUtil.getSpecificCoinfectionInvList( new Long(investigationUID), request);
        ...
        logger.info("Total coinfection investigation found --------------------- "+coinfectionInvList.size());

        
		if(investigationUID!=null && investigationUID.length()>0 && coinfectionInvList.size()>1){
