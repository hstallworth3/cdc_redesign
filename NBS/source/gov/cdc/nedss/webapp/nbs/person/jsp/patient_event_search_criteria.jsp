<?xml version="1.0" encoding="UTF-8"?>
  ...
  document.onkeydown = function(event)
	...
	function clearForm(){
		...
		if(document.getElementById("tabs0head0").getAttribute("class")=="ongletTextEna"){//Patient Search tab active
				...
				initializeMultiSelects();
				   ...
				   // alert(allEnabledSearchIpElts.length);
			        for (var i = 0; i < allEnabledSearchIpElts.length; i++) {
			            ...
			            && $j(allEnabledSearchIpElts[i]).attr("type") != 'button' && jQuery.trim($j(allEnabledSearchIpElts[i]).attr("value")) !="" ) {
							 ...
							 if($j(allEnabledSearchIpElts[i]).attr("id") == 'patientDOB1' && $j(allEnabledSearchIpElts[i]).attr("value") != "") {
										...
										$j("#patientDOB1").focus();
										getElementByIdOrByName("DEM105L").style.color="red";
										 erroronTab1= true; 
										}
							 ...
							 //id type and id value validation starts here....
							  if(($j(allEnabledSearchIpElts[i]).attr("id") == 'DEM222'  && $j(allEnabledSearchIpElts[i]).attr("value") != "") &&
							     ...
							     errorText = "ID Value must have a value. Please correct the data and try again.";
									   errorMsgArray.push(errorText);
									   $j("#patientDOB2").focus();
									   getElementByIdOrByName("DEM223L").style.color="red";
					...
					if($j("#ETYPE").attr("value")!=""){
				        // atleastOne = true;
				         erroronTab2= true; 
				         if ($j('#COND').attr('value') != "" 
				        		|| $j('#COND').attr('value') != ""
				        		|| ($j('#PROGRAMAREA').attr('value') != "" || $j('#PROGRAMAREALAB').attr('value') != "" || $j('#PROGRAMAREAMORB').attr('value') != "" || $j('#PROGRAMAREACASE').attr('value') != "" || $j('#PROGRAMAREAINVEST').attr('value') != "" )
				        		|| $j('#JURISD').attr('value') != ""
				        		|| $j('#pregnantList').attr('value') != ""
				        		|| $j('#ESR200').attr('value') != "" 
			        			|| $j('#ESR100').attr('value') != "" 
				        		|| (($j('#ELECTRONIC').attr('checked')==false && !$j('#ELECTRONIC').attr('disabled') )
				        		|| ($j('#MANUAL').attr('checked')==false && !$j('#MANUAL').attr('disabled') ))
				        		...
				        		|| ($j('#newInitial').attr('checked')==false
	        		...
	        		if($j("#ESR100").attr("value")!="" && $j("#ESR101").attr("value")=="") {
			        	...
			        	//atleastOne = true;
			        	 ...
			        	 getElementByIdOrByName("ESR101L").style.color="red";	
			        	 erroronTab2= true;        	
			        ...
			        if($j("#ESR200").attr("value")!="") {
			        	 ...
			        	 if($j("#DateOp").attr("value")=="=" && $j("#from_date").attr("value")=="") {
					        	...
					        	//atleastOne = true;
					        	 ...
					        	 getElementByIdOrByName("dateLabel").style.color="red";	
					        	 erroronTab2= true;        	
...
if($j("#EventProviderFacility").attr("value")!="" && ($j("#INV185").text().length==0 && $j("#INV186").text().length==0 && $j("#INV208").text().length==0 && $j("#INV209").text().length==0 && $j("#INV187").text().length==0 )) {			        	
			        	...
			        	//atleastOne = true;
			        	 ...
			        	 getElementByIdOrByName("ProviderFacility").style.color="red";	
			        	 erroronTab2= true;        	
			      ...
			      //  alert("deep");
			       ...
			       /*  if((!atleastOne) && atleastOneEvent) {
					...
					getElementByIdOrByName("EIDOP_textbox").value==""||
					getElementByIdOrByName("DateOp_textbox").value==""){
		    ...
		    // alert(errorMsgArray.length);
		     ...
		     //  alert("3");
		      ...
		      if("<%=request.getAttribute("formHref")%>" != null){
		          document.personSearchForm.action =   "<%=(String)request.getAttribute("formHref")%>?ContextAction=Submit";
		          }else{		        
            ...
            if ($j(allEnabledSearchIpElts[i]).attr("type") != 'button' 
            		&&  $j(allEnabledSearchIpElts[i]).attr("id") != 'EIDOP' 
            		 ...
            		 && $j(allEnabledSearchIpElts[i]).attr("id") != 'ETYPE'
            			 && $j(allEnabledSearchIpElts[i]).attr("id") != 'resultOperatorList'  && $j(allEnabledSearchIpElts[i]).attr("id") != 'codedResultOrganismOperatorList') {
                $j(allEnabledSearchIpElts[i]).val("");
            }
        }
 ...
 * checkPermissions: 
  ...
  function checkPermissions(){
	 ...
	 var permissionLab = ${personSearchForm.attributeMap.permissionLab};
	 var permissionMorb = ${personSearchForm.attributeMap.permissionMorb};
	 var permissionCase = ${personSearchForm.attributeMap.permissionCase};
	 var permissionInvestigation = ${personSearchForm.attributeMap.permissionInvestigation};
...
%> 
    ...
    <body class onload="startCountdown();autocompTxtValuesForJSP();handleRefineSearch();handleAdvancedSearch();checkPermissions();setReportType();showHideSections();showHideEventId();showHideEventFields();setTitle();maintainEventProviderFacilityOnRefine();">
        <div id="pageview"></div>
