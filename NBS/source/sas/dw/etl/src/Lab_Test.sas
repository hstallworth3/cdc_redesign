...
PROC  SQL;
create table updated_observation_List as select * from rdbdata.updated_observation_List;
create table updated_lab_test_list as select lab_test_uid, lab_test_key from nbs_rdb.lab_test where lab_test_uid in (select observation_uid from rdbdata.updated_observation_List);
quit;
PROC  SQL;
...
proc sql;
create table s_edx_document as select EDX_Document_uid, act_uid, add_time from nbs_ods.EDX_Document, rdbdata.s_updated_lab where  EDX_Document.act_uid=s_updated_lab.observation_Uid order by add_time desc;
quit;
...
observation_uid
from nbs_ods.participation, nbs_rdb.UPDATED_OBSERVATION_LIST obs 
where 
...
select tst.*, obs.Cd_desc_txt 'Parent_Test_Nm' as Parent_test_nm
from nbs_ods.observation as obs, lab_test2 as tst
where tst.parent_test_pntr = obs.observation_uid;
/*Setting SPECIMEN_ADD_TIME &  SPECIMEN_LAST_CHANGE_TIME*/
proc sql;
create index lab_test_uid on lab_test3(lab_test_uid);
quit;
proc sql;
create table lab_test4 as 
 ...
 from lab_test3 as tst
	...
	left join nbs_ods.observation as obs1
