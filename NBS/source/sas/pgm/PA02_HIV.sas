...
a.INVESTIGATOR_FL_FUP_KEY, a.INVESTIGATOR_FL_FUP_QC, c.PROVIDER_LAST_NAME, c.PROVIDER_FIRST_NAME, INVESTIGATOR_DISP_FL_FUP_KEY
...
a.INVESTIGATOR_FL_FUP_KEY = c.PROVIDER_KEY
where b.INV_CASE_STATUS in ('Probable','Confirmed') 
and a.INVESTIGATOR_FL_FUP_KEY ~=1 and  datepart(a.FL_FUP_DISPO_DT) ge datepart(a.FL_FUP_INVESTIGATOR_ASSGN_DT)
...
proc sql;
create table s_p  as 
select PROVIDER_LAST_NAME,  INVESTIGATOR_FL_FUP_KEY , count (distinct inv_local_id) as var_s_p  
from &in
...
select PROVIDER_LAST_NAME,  INVESTIGATOR_DISP_FL_FUP_KEY as INVESTIGATOR_FL_FUP_KEY , count (distinct inv_local_id) as var_ae_p  
from &in1
where FL_FUP_DISPOSITION is not null and INVESTIGATOR_DISP_FL_FUP_KEY ~= INVESTIGATOR_FL_FUP_KEY
group by INVESTIGATOR_DISP_FL_FUP_KEY, PROVIDER_LAST_NAME
order by INVESTIGATOR_DISP_FL_FUP_KEY, PROVIDER_LAST_NAME;quit; 
...
data final (keep = PROVIDER_LAST_NAME_new colname colval colval2 colval3 colval4 pname_l);
length PROVIDER_LAST_NAME_new $50.;
set ind_5;
PROVIDER_LAST_NAME_new=left(trim(PROVIDER_LAST_NAME));
pname_l=lowcase(PROVIDER_LAST_NAME_new);
run;

proc sort DATA = final ;
by pname_l PROVIDER_LAST_NAME_new;
run;
...
TITLE2 bold f= Calibri h=12pt  j=c "WORKER: SUMMARY OF #byval(PROVIDER_LAST_NAME_new)	";
...
style(column)={font_size=10pt font_face="Calibri" cellspacing=0};
by pname_l PROVIDER_LAST_NAME_new;
columns  PROVIDER_LAST_NAME_new colname colval colval2 colval3 colvaL4 colvaL5 ;

define PROVIDER_LAST_NAME_new / ORDER=data page noprint ;
