...
group by a.FIRST_NM, a.LAST_NM, a.ADD_USER_ID 
...
select distinct PROGRAM_AREA_CD from nbs_rdb.STD_HIV_DATAMART
;
quit;
...
from v_event_metric 
where EVENT_TYPE in ('LabReport','MorbReport') and prog_area_cd in (select PROGRAM_AREA_CD from PROG_AREA) 
group by FIRST_NM, LAST_NM, ADD_USER_ID 
...
from v_event_metric 
where EVENT_TYPE in ('CONTACT') and prog_area_cd in (select PROGRAM_AREA_CD from PROG_AREA) group by FIRST_NM, LAST_NM, ADD_USER_ID order by ADD_USER_ID
;
quit;
