...
,           substr(b.CTT_SEX_EXP_FREQ,1,1) as named_freq 
,           substr(b.CTT_NDLSHARE_EXP_FREQ,1,1) as named_ndl_shr_freq
...
G.PATIENT_KEY,
				datepart(b.CTT_FIRST_SEX_EXP_DT) as bb_first_dt format = mmddyy10. , 
datepart(b.CTT_FIRST_NDLSHARE_EXP_DT) as bb_first_dt2 format = mmddyy10.
,b.CTT_SEX_EXP_FREQ as bb_freq ,
b.CTT_NDLSHARE_EXP_FREQ as bb_ndl_freq
,datepart(b.CTT_LAST_SEX_EXP_DT) as bb_last_exp_dt format = mmddyy10.,
datepart(b.CTT_LAST_NDLSHARE_EXP_DT) as bb_ndl_sh_dt format = mmddyy10.,
datepart(f.IX_DATE) as bb_inter_dt format = mmddyy10.,
substr(b.CTT_REFERRAL_BASIS,1,2) as named_ref
...
proc sql;
	create table name_by_init as 
		select distinct G.PATIENT_KEY,
			...
			,           substr(b.CTT_SEX_EXP_FREQ,1,1) as freq_OP 
			,          substr(b.CTT_NDLSHARE_EXP_FREQ,1,1) as named_ndl_shr_freq
...
datepart(c.SYM_SIGN_SX_OBVTN_ONSET_DT) as Sym_On_dte format = mmddyy10.,
