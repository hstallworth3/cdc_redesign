...
public class PA01_CaseManagement_Report_ClosedDate_STD {
	...
	public void Populate_STDReport() throws Exception {
		...
		log = new Log("PA01_CaseManagement_Report_ClosedDate_STD", Log_File_Path);
		df = new DataFile(Data_File_Path, "PA01_CaseMgmt_ClosedDate_STD");
		...
		for (int col = 1; col <= colCount; col++) {
			...
			try {
				...
				while (itr.hasNext()) {
					index = index + 1;
					if (tableRows.get(index).getText().contains("PA01 Case Management Report (Closed Date) - STD "))
						break;
				}
