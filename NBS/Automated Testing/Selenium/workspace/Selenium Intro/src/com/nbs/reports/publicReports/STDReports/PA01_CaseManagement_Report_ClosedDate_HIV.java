...
public class PA01_CaseManagement_Report_ClosedDate_HIV {
	...
	public void Populate_STDReport() throws Exception {
		...
		log = new Log("PA01_CaseManagement_Report_ClosedDate_HIV", Log_File_Path);
		df = new DataFile(Data_File_Path, "PA01_CaseMgmt_ClosedDate_HIV");
		...
		for (int col = 1; col <= colCount; col++) {
			...
			try {
				...
				while (itr.hasNext()) {
					index = index + 1;
					if (tableRows.get(index).getText().contains("PA01 Case Management Report (Closed Date) - HIV "))
						break;
				}
				...
				if (!comUtils.isElementDisplayed(By.id("error1"))) {
					...
					if (comUtils.isElementDisplayed(By.xpath("//*[@id='pageContainer1']/xhtml:div[2]/xhtml:div[4]"))) {
						...
						if (driver.findElement(By.xpath("//*[@id='pageContainer1']/xhtml:div[2]/xhtml:div[4]")).getText()
								.contains("CASE MANAGEMENT REPORT") || driver.findElement(By.xpath("//*[@id='pageContainer1']/xhtml:div[2]/xhtml:div[4]")).getText()
								.contains("Summary of ALL Worker") )
