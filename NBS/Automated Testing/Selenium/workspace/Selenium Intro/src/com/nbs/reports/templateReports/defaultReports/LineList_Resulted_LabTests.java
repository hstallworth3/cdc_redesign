...
public class LineList_Resulted_LabTests {
	...
	@Test
	public void Populate_LineList_Resulted_LabTests_Report() throws Exception {
		comUtils.setUpDriver(driver);
		log = new Log("LineList_Resulted_LabTests_Report", Log_File_Path);
		df = new DataFile(Data_File_Path, "LineList_Resulted_LabTests");
		...
		for (int col = 1; col <= colCount; col++) {
			try {
				...
				// Line List of Resulted Lab Tests
				int index = 0;
				...
				while (itr.hasNext()) {
					index = index + 1;
					if (tableRows.get(index).getText().contains("Line List of Resulted Lab Tests"))
						break;
				}
				...
				if (!comUtils.isElementDisplayed(By.id("error1"))) {
					...
					if (comUtils.isElementDisplayed(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td/p"))) {
						...
						if (driver.findElement(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td")).getText()
								.contains("Custom Report For Table: LAB100"))
