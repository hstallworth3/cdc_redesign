...
public class Notification_ActivityLog_LineListing {
	...
	@Test
	public void Populate_Notifi_ActivityLog_LineList_Report() throws Exception {
		comUtils.setUpDriver(driver);
		log = new Log("Notification_ActivityLog_LineListing_Report", Log_File_Path);
		df = new DataFile(Data_File_Path, "Notifi_ActivityLog_LineList");
		...
		for (int col = 1; col <= colCount; col++) {
			try {
				...
				while (itr.hasNext()) {
					index = index + 1;
					if (tableRows.get(index).getText().contains("Notification Activity Log Line Listing"))
						break;
				}
				...
				if (!comUtils.isElementDisplayed(By.id("error1"))) {
					...
					if (comUtils.isElementDisplayed(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td/p"))) {
						...
						if (driver.findElement(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td")).getText()
								.contains("Custom Report For Table: nnd_activity_log"))
