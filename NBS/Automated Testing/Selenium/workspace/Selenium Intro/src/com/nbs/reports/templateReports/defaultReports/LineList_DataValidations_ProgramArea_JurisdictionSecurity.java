...
public class LineList_DataValidations_ProgramArea_JurisdictionSecurity {
	...
	@Test
	public void Populate_LineList_DataValidation_PA_JS() throws Exception {
		comUtils.setUpDriver(driver);
		log = new Log("LineList_DataValidation_PA_JSLineList_DataValidation_PA_JS_Report", Log_File_Path);
		df = new DataFile(Data_File_Path, "LineList_DataValidation_PA_JS");		
		...
		for(int col=1; col <=colCount; col++){	
			try {
				...
				//Click on Run - Get the index of the Run button with Text -  Line List of Data Validations with Program Area and Jurisdiction Security
				int index = 0;
				...
				while(itr.hasNext()) {
					index = index+1;
					if (tableRows.get(index).getText().contains("Line List of Data Validations with Program Area and Jurisdiction Security"))
						break;	
				}
				...
				if (!(comUtils.isElementDisplayed(By.id("error1")))){
					...
					if (comUtils.isElementDisplayed(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td/p"))){					                          
						...
						if (driver.findElement(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td")).getText().contains("Custom Report For Table: DATA_VALIDATION"))
							log.writeSubSection("Report executed successfully","",true);
						else 
							log.writeSubSection("Report did NOT execute successfully1","",false);
					}
