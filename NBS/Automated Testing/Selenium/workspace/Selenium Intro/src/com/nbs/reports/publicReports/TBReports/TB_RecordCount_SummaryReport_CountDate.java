...
public class TB_RecordCount_SummaryReport_CountDate {
	...
	@Test
	public void Populate_TB_RecordCount_SR_CD() throws Exception {
		comUtils.setUpDriver(driver);
		log = new Log("TB_RecordCount_SR_CD_Report", Log_File_Path);
		df = new DataFile(Data_File_Path, "TB_RecordCount_SR_CD");		
		...
		for(int col=1; col <=colCount; col++){	
			try {
				...
				//Click on Run - Get the index of the Run button with Text -  TB Record Count - Summary Report by Count Date
				int index = 0;
				...
				if (!(comUtils.isElementDisplayed(By.id("error1")))){
					...
					if (comUtils.isElementDisplayed(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td/p"))){
						...
						if (driver.findElement(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td")).getText().contains("TB"))//Needs to be changed. Have not yet found any Report to display.
