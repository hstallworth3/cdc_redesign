...
public class PA01_CaseManagement_InterviewAssignDate_HIV {
	...
	public void Populate_STDReport() throws Exception {
		...
		log = new Log("PA01_CaseManagement_InterviewAssignDate_HIV", Log_File_Path);
		df = new DataFile(Data_File_Path, "PA01_CaseMgmt_Interview_HIV");
		...
		for (int col = 1; col <= colCount; col++) {
			...
			try {
				...
				while (itr.hasNext()) {
					index = index + 1;
					if (tableRows.get(index).getText().contains("PA01 Case Management Report (Interview Assign Date) - HIV"))
						break;
				}
