...
public class CaseInvestigation_RejectedNNDs_ProgramArea_JurisdictionSecurity {
	...
	@Test
	public void Populate_CaseInves_RejectNNDs_PA_JS_Report() throws Exception {
		comUtils.setUpDriver(driver);
		log = new Log("CaseInves_RejectNNDs_PA_JS_Report", Log_File_Path);
		df = new DataFile(Data_File_Path, "CaseInves_RejectNNDs_PA_JS");
		...
		for (int col = 1; col <= colCount; col++) {
			try {
				...
				// Case Investigation with Rejected NNDs with Program Area and
				...
				while (itr.hasNext()) {
					index = index + 1;
					if (tableRows.get(index).getText().contains(
							"Case Investigation with Rejected NNDs with Program Area and Jurisdiction Security"))
						break;
				}
				...
				if (!comUtils.isElementDisplayed(By.id("error1"))) {
					...
					if (comUtils.isElementDisplayed(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td/p"))) {
						...
						if (driver.findElement(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td")).getText()
								.contains("Custom Report For Table: REJECTED_NND"))
