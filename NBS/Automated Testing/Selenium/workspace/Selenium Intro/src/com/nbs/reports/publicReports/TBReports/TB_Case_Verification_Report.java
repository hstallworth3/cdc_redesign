...
public class TB_Case_Verification_Report {
	...
	@Test
	public void Populate_TB_Case_Verification_Report() throws Exception {
		comUtils.setUpDriver(driver);
		log = new Log("TB_Case_Verification_Report", Log_File_Path);
		df = new DataFile(Data_File_Path, "TB_Case_Verification_Report");		
		...
		for(int col=1; col <=colCount; col++){	
			try {
				...
				//Click on TB Report Section under Template Reports (in some environment Template10 is available and in some environments Template8 is available)
				int env_index =0;
				for (int i = 1;i<=10;i++){
				...
				driver.findElement(By.xpath("//*[@id='Public"+env_index+"']/thead/tr/th/a/img")).click();//Need to change it according to diff environment
				...
				//Click on Run - Get the index of the Run button with Text -  Line List of Hepatitis Investigations
				int index = 0;
				...
				while(itr.hasNext()) {
					index = index+1;
					if (tableRows.get(index).getText().contains("TB Case Verification Report"))
						break;	
				}
				...
				if (!(comUtils.isElementDisplayed(By.id("error1")))){
					...
					if (comUtils.isElementDisplayed(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td"))){
						...
						if (driver.findElement(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td")).getText().contains("TB Case Verification Report")){
