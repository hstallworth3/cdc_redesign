...
public class CA01_ChalkTalk_Report_Case {
	...
	public void Populate_STDReport() throws Exception {
		...
		log = new Log("CA01_ChalkTalk_Report_Case", Log_File_Path);
		df = new DataFile(Data_File_Path, "CA01_ChalkTalk_Report_Case");
		...
		for (int col = 1; col <= colCount; col++) {
			...
			try {
				...
				String EpiLinkID = std_Report.getFieldValue("Epi Link ID", col);
				...
				if((DIAGNOSIS_CD_SelectAll).equalsIgnoreCase("Yes") && (DIAGNOSIS_CD_IncludeNulls).equalsIgnoreCase("Yes"))
					log.write("Epi Link ID: " + EpiLinkID + ";" + "DIAGNOSIS_CD: "+"Select All;"+ "   DIAGNOSIS_CD: "+ "Include Nulls;");  
				else if((DIAGNOSIS_CD_SelectAll).equalsIgnoreCase("Yes")) 
					log.write("Epi Link ID: " + EpiLinkID + ";" + "DIAGNOSIS_CD: "+"Select All;");
				else if((DIAGNOSIS_CD_IncludeNulls).equalsIgnoreCase("Yes")) 
					log.write("Epi Link ID: " + EpiLinkID + ";" + "  DIAGNOSIS_CD:  " + DIAGNOSIS_CD+ ";" + "DIAGNOSIS_CD:  " + "Include Nulls");
				else
					log.write("Epi Link ID: " + EpiLinkID + ";" + "  DIAGNOSIS_CD:  " + DIAGNOSIS_CD+ ";" );
				...
				while (itr.hasNext()) {
					index = index + 1;
					if (tableRows.get(index).getText().contains("CA01 Chalk Talk Report: Case"))
						break;
				}
				...
				if (!comUtils.isElementDisplayed(By.id("error1"))) {
					...
					Thread.sleep(2000);
					if (comUtils.isElementDisplayed(By.xpath("//*[@id='pageContainer1']/xhtml:div[2]/xhtml:div[5]"))) {

						System.out.println(
								driver.findElement(By.xpath("//*[@id='pageContainer1']/xhtml:div[2]/xhtml:div[5]")).getText());
						
						if (driver.findElement(By.xpath("//*[@id='pageContainer1']/xhtml:div[2]/xhtml:div[5]")).getText()
								.contains("CHALK TALK REPORT"))
