...
public class Deduplication_ActivityLog_LineListing {
	...
	public void Populate_SummaryReport() throws Exception {
		...
		log = new Log("DeduplicationActivityLog_LL", Log_File_Path);
		df = new DataFile(Data_File_Path, "Dedup_ActivityLog_LL");		
		...
		for(int col=1; col <=colCount; col++){	
			try {
				...
				//Click on Run - Get the index of the Run button with Text -  Deduplication Activity Log Line Listing
				int index = 0;
				...
				while(itr.hasNext()) {
					index = index+1;
					if (tableRows.get(index).getText().contains("Deduplication Activity Log Line Listing"))
						break;	
				}
				...
				if (!(comUtils.isElementDisplayed(By.id("error1")))){
					...
					if (comUtils.isElementDisplayed(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td/p"))){					                          
						...
						if (driver.findElement(By.xpath("html/body/div[1]/table[1]/tbody/tr[1]/td")).getText().contains("Custom Report For Table: deduplication_activity_log"))
